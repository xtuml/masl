...
namespace ThreadTimer
{
  ...
  bool ThreadTimer::initialise()
  {
    ...
    SWA::PluginRegistry::getInstance().registerFlagSetter(getName(),"Active",[this](bool flag){setActive(flag);});
    SWA::PluginRegistry::getInstance().registerFlagGetter(getName(),"Active",[this](){return isActive();});
    SWA::PluginRegistry::getInstance().registerFlagSetter(getName(),"Show Detail",[this](bool flag){setTimeActions(flag);});
    SWA::PluginRegistry::getInstance().registerFlagGetter(getName(),"Show Detail",[this](){return isTimeActions();});
    SWA::PluginRegistry::getInstance().registerPropertySetter(getName(),"Threshold (ms)",[this](const std::string& millis){setThreshold(millis);});
    SWA::PluginRegistry::getInstance().registerPropertyGetter(getName(),"Threshold (ms)",[this](){return getThreshold();});
  ...
  {
    stream << std::format("{:60} Real : {:5}ms User : {:5}ms Sys : {:5}ms", name, (SWA::Duration::real() - startReal).millis(), (SWA::Duration::user() - startUser).millis(),(SWA::Duration::system() - startSystem).millis());
  }
