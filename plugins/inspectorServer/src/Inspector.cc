...
bool initialise() {
    if (ooaPort ||
        SWA::CommandLine::getInstance().optionPresent(PortNoOption)) {
        try {
            int ooaPortNo = std::stoi(SWA::CommandLine::getInstance().getOption(
                PortNoOption, (ooaPort ? ooaPort : "")));
...
struct Init {
    Init() {
        SWA::CommandLine::getInstance().registerOption(SWA::NamedOption(
            PortNoOption,
            std::string("Inspector Port Number") +
                (ooaPort ? std::string(" (") + ooaPort + ")" : ""),
            false, "portNo", true, false));
...
Inspector::Inspector(int port)
    : requestChannel(20000 + port), infoChannel(30000 + port),
      consoleRedirect(40000 + port),
      connectListener(
          requestChannel.getServerFd(),
          [this](int fd) { return connectCallback(fd); },
          SWA::Process::getInstance().getActivityMonitor()),
      requestListener([this](int fd) { return requestCallback(fd); },
                      SWA::Process::getInstance().getActivityMonitor()),
      traceLines(false), traceBlocks(false), traceExceptions(false),
      traceEvents(false), stepLines(false), stepBlocks(false),
      stepExceptions(false), stepEvents(false), stepToNext(false),
      paused(false), idle(true), backlogTime(),
      backlogTimer(SWA::ListenerPriority::getNormal(),
                   [this](int overrun) { return backlogCallback(overrun); }) {
    ...
    commandLookup.emplace(GET_PROCESS_DATA,
                          [this]() { return getProcessData(); });
    commandLookup.emplace(REDIRECT_CONSOLE,
                          [this]() { return redirectConsole(); });
    commandLookup.emplace(RUN_DOMAIN_SERVICE,
                          [this]() { return runDomainService(); });
    commandLookup.emplace(RUN_TERMINATOR_SERVICE,
                          [this]() { return runTerminatorService(); });
    commandLookup.emplace(RUN_OBJECT_SERVICE,
                          [this]() { return runObjectService(); });
    commandLookup.emplace(FIRE_EVENT, [this]() { return fireEvent(); });
    commandLookup.emplace(SCHEDULE_EVENT, [this]() { return scheduleEvent(); });
    commandLookup.emplace(CANCEL_TIMER, [this]() { return cancelTimer(); });
    commandLookup.emplace(TRACE_LINES, [this]() { return setTraceLines(); });
    commandLookup.emplace(TRACE_BLOCKS, [this]() { return setTraceBlocks(); });
    commandLookup.emplace(TRACE_EXCEPTIONS,
                          [this]() { return setTraceExceptions(); });
    commandLookup.emplace(TRACE_EVENTS, [this]() { return setTraceEvents(); });
    commandLookup.emplace(STEP_LINES, [this]() { return setStepLines(); });
    commandLookup.emplace(STEP_BLOCKS, [this]() { return setStepBlocks(); });
    commandLookup.emplace(STEP_EXCEPTIONS,
                          [this]() { return setStepExceptions(); });
    commandLookup.emplace(STEP_EVENTS, [this]() { return setStepEvents(); });
    commandLookup.emplace(ENABLE_TIMERS,
                          [this]() { return setEnableTimers(); });
    commandLookup.emplace(CONTINUE_EXECUTION,
                          [this]() { return continueExecution(); });
    commandLookup.emplace(PAUSE_EXECUTION,
                          [this]() { return pauseExecution(); });
    commandLookup.emplace(STEP_EXECUTION, [this]() { return stepExecution(); });
    commandLookup.emplace(SET_BREAKPOINT, [this]() { return setBreakpoint(); });
    commandLookup.emplace(GET_INSTANCE_DATA,
                          [this]() { return getInstanceData(); });
    commandLookup.emplace(GET_SELECTED_INSTANCE_DATA,
                          [this]() { return getSelectedInstanceData(); });
    commandLookup.emplace(GET_SINGLE_INSTANCE_DATA,
                          [this]() { return getSingleInstanceData(); });
    commandLookup.emplace(GET_RELATED_INSTANCE_DATA,
                          [this]() { return getRelatedInstanceData(); });
    commandLookup.emplace(GET_EVENT_QUEUE,
                          [this]() { return getEventQueue(); });
    commandLookup.emplace(GET_TIMER_QUEUE,
                          [this]() { return getTimerQueue(); });
    commandLookup.emplace(GET_LOCAL_VARIABLES,
                          [this]() { return getLocalVariables(); });
    commandLookup.emplace(GET_STACK, [this]() { return getStack(); });
    commandLookup.emplace(GET_ASSIGNER_STATE,
                          [this]() { return getAssignerState(); });
    commandLookup.emplace(RUN_SCHEDULE, [this]() { return runSchedule(); });
    commandLookup.emplace(GET_INSTANCE_COUNT,
                          [this]() { return getInstanceCount(); });
    commandLookup.emplace(CREATE_INSTANCE_POPULATION,
                          [this]() { return createInstancePopulation(); });
    commandLookup.emplace(CREATE_SINGLE_INSTANCE,
                          [this]() { return createSingleInstance(); });
    commandLookup.emplace(UPDATE_SINGLE_INSTANCE,
                          [this]() { return updateSingleInstance(); });
    commandLookup.emplace(DELETE_SINGLE_INSTANCE,
                          [this]() { return deleteSingleInstance(); });
    commandLookup.emplace(CREATE_RELATIONSHIPS,
                          [this]() { return createRelationships(); });
    commandLookup.emplace(CREATE_SUPERSUBTYPES,
                          [this]() { return createSuperSubtypes(); });
    commandLookup.emplace(INVOKE_PLUGIN_ACTION,
                          [this]() { return invokePluginAction(); });
    commandLookup.emplace(GET_PLUGIN_FLAG,
                          [this]() { return getPluginFlag(); });
    commandLookup.emplace(GET_PLUGIN_PROPERTY,
                          [this]() { return getPluginProperty(); });
    commandLookup.emplace(SET_PLUGIN_FLAG,
                          [this]() { return setPluginFlag(); });
    commandLookup.emplace(SET_PLUGIN_PROPERTY,
                          [this]() { return setPluginProperty(); });
...
void Inspector::backlogCallback(int overrun) {
    try {
        ...
        std::cerr << "Lost Inspector Connection (" << e.what() << ")"
                  << std::endl;
...
void Inspector::processRequest() {
    try {
        ...
        std::map<InspectorRequestId, CommandFunction>::const_iterator
            foundCommand =
...
bool Inspector::scheduleEvent() {
    ...
    int objectId;
    int eventId;
    requestChannel >> timerId >> expiry >> period >> domainId >> objectId >>
        eventId;
    ...
    if (event)
        SWA::EventTimers::getInstance().scheduleTimer(timerId, expiry, period,
                                                      event);
...
bool Inspector::setBreakpoint() {
    ...
    int actionId;
    int lineNo;
    requestChannel >> actionType >> domainId >> objectId >> actionId >>
        lineNo >> set;

    if (set) {
        return breakpoints
            .insert(SWA::StackFrame(SWA::StackFrame::ActionType(actionType),
                                    domainId, objectId, actionId, lineNo))
            .second;
    } else {
        return breakpoints.erase(
            SWA::StackFrame(SWA::StackFrame::ActionType(actionType), domainId,
                            objectId, actionId, lineNo));
    }
}
...
bool Inspector::createInstancePopulation() {
    ...
    GenericObjectHandler &handler = ProcessHandler::getInstance()
                                        .getDomainHandler(domainId)
                                        .getGenericObjectHandler(objectId);
...
bool Inspector::deleteSingleInstance() {
    ...
    ProcessHandler::getInstance()
        .getDomainHandler(domainId)
        .getGenericObjectHandler(objectId)
        .deleteInstance(requestChannel, instanceId);
    return true;
}
...
bool Inspector::createRelationships() {
    ...
    DomainHandler &handler =
        ProcessHandler::getInstance().getDomainHandler(domainId);
...
bool Inspector::getInstanceData() {
    ...
    ProcessHandler::getInstance()
        .getDomainHandler(domainId)
        .getGenericObjectHandler(objectId)
        .writePopulation(requestChannel);
...
bool Inspector::getSingleInstanceData() {
    ...
    ProcessHandler::getInstance()
        .getDomainHandler(domainId)
        .getGenericObjectHandler(objectId)
        .writeInstance(requestChannel, instanceId);
...
void Inspector::outputTrace(std::string prefix, bool showLineNo,
                            std::string message) const {
    std::cout << "Inspector: "
              << std::string(
                     (SWA::Stack::getInstance().getStackFrames().size()) * 2,
                     ' ')
              << prefix;
    if (!SWA::Stack::getInstance().getStackFrames().empty()) {
        std::cout << SWA::NameFormatter::formatStackFrame(
            SWA::Stack::getInstance().top(), showLineNo);
        if (message.size() > 0) {
            std::cout << " : ";
        }
    }
...
bool Inspector::getPluginFlag() {
    ...
    requestChannel << SWA::PluginRegistry::getInstance().getFlag(pluginName,
                                                                 flagName);
...
bool Inspector::getPluginProperty() {
    ...
    requestChannel << SWA::PluginRegistry::getInstance().getProperty(
...
bool Inspector::setPluginProperty() {
    ...
    SWA::PluginRegistry::getInstance().setProperty(pluginName, propertyName,
                                                   value);
...
void Inspector::enteredCatch() {
    ...
    if (stepExceptions || stepBlocks || stepLines || stepToNext ||
        hitBreakpoint())
        pause();
}
...
std::string getInstanceText(int domainId, int objectId,
                            SWA::IdType instanceId) {
...
void Inspector::transitioningState(int domainId, int objectId, int instanceId,
    ...
    if (traceEvents)
        outputTrace("* ", true,
                    "State Transition for " +
                        getInstanceText(domainId, objectId, instanceId) +
                        " from " +
                        SWA::NameFormatter::formatStateName(domainId, objectId,
                                                            oldState) +
                        " to " +
                        SWA::NameFormatter::formatStateName(domainId, objectId,
                                                            newState));
...
void Inspector::transitioningAssignerState(int domainId, int objectId,
                                           int oldState, int newState) {
    while (requestChannel.ready())
        processRequest();
    ...
    if (traceEvents)
        outputTrace("* ", true,
                    "Assigner State Transition from " +
                        SWA::NameFormatter::formatStateName(domainId, objectId,
                                                            oldState) +
                        " to " +
                        SWA::NameFormatter::formatStateName(domainId, objectId,
...
void Inspector::generatingEvent(const std::shared_ptr<SWA::Event> &event) {
    ...
    if (traceEvents) {
        std::string message = "Generating Event " +
                              SWA::NameFormatter::formatEventName(
                                  event->getDomainId(), event->getObjectId(),
                                  event->getEventId());
        if (event->getHasSource()) {
            message += " from " + getInstanceText(event->getDomainId(),
                                                  event->getSourceObjectId(),
                                                  event->getSourceInstanceId());
        }

        if (event->getHasDest()) {
            message += " to " + getInstanceText(event->getDomainId(),
                                                event->getObjectId(),
                                                event->getDestInstanceId());
        }
...
void Inspector::processingEvent(const std::shared_ptr<SWA::Event> &event) {
    ...
    if (traceEvents) {
        std::string message = "Processing Event " +
                              SWA::NameFormatter::formatEventName(
                                  event->getDomainId(), event->getObjectId(),
                                  event->getEventId());
        if (event->getHasSource()) {
...
void Inspector::settingTimer(int timerId, const SWA::Timestamp &timeout,
    ...
    if (traceEvents) {
        std::string message =
            std::format("Scheduling timer {} at {} period {} for event {}",
                        timerId, timeout, period,
                        SWA::NameFormatter::formatEventName(
                            event->getDomainId(), event->getObjectId(),
                            event->getEventId()));
        if (event->getHasSource()) {
...
void Inspector::firingTimer(int timerId, int overrun) {
    ...
    if (traceEvents) {
        outputTrace(
            "* ", false,
            std::format("Firing timer {}, overrun {}", timerId, overrun));
    }
}
