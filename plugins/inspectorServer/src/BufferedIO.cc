...
namespace Inspector
{
  ...
  {
    writeRaw(boost::endian::native_to_big(val));
  }
  ...
  {
      writeRaw(boost::endian::native_to_big(std::bit_cast<std::uint64_t>(val)));
  }
  ...
  void BufferedInputStream::read ( double& val )
  {
      std::uint64_t bits;
      readRaw(bits);
      boost::endian::big_to_native_inplace(bits);
      val = std::bit_cast<double>(bits);
