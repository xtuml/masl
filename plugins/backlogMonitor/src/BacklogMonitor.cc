...
BacklogMonitor::BacklogMonitor()
    : active(true), pollInterval(), reportThreshold(), lastBacklog(),
      expectedTime(),
      timer(SWA::ListenerPriority::getNormal(),
            [this](auto &&overrun) { timerCallback(overrun); }) {}
...
bool BacklogMonitor::initialise() {
    SWA::PluginRegistry::getInstance().registerPropertySetter(
        getName(), "Poll interval (secs)",
        [this](auto &&interval) { setPollInterval(interval); });
    SWA::PluginRegistry::getInstance().registerPropertySetter(
        getName(), "Report Threshold (secs)",
        [this](auto &&threshold) { setReportThreshold(threshold); });
    SWA::PluginRegistry::getInstance().registerPropertyGetter(
        getName(), "Poll interval (secs)",
        [this]() { return getPollInterval(); });
    SWA::PluginRegistry::getInstance().registerPropertyGetter(
        getName(), "Report Threshold (secs)",
        [this]() { return getReportThreshold(); });
    SWA::PluginRegistry::getInstance().registerFlagSetter(
        getName(), "Active", [this](auto &&active) { setActive(active); });
    ...
    SWA::CommandLine::getInstance().registerOption(
        SWA::NamedOption(PollIntervalOption,
                         "Poll interval for backlog reporting (0 = disabled)",
                         false, "interval", true, false));
    SWA::CommandLine::getInstance().registerOption(SWA::NamedOption(
        ReportThresholdOption, "Threshold for backlog reporting ", false,
...
void BacklogMonitor::startup() {
    setPollInterval(
        SWA::CommandLine::getInstance().getOption(PollIntervalOption, "60"));
    setReportThreshold(
        SWA::CommandLine::getInstance().getOption(ReportThresholdOption, "10"));
    setActive(active);
}
...
void BacklogMonitor::report(const SWA::Duration &backlog) const {
    ...
    static const SWA::Sequence<SWA::String> suffixes(suffixArray,
                                                     suffixArray + 3);

    std::clog << "Backlog : "
              << backlog.format(SWA::Duration::Hour, SWA::Duration::Second,
                                SWA::Duration::TowardsZero, true, 1, false, 0,
                                "", "", suffixes)
              << "\n"
              << std::flush;
}
