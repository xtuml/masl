...
std::string getListenerPort() {
    std::string port((getenv("EVENT_INSPECTOR_PORT") == 0
                          ? "8500"
                          : getenv("EVENT_INSPECTOR_PORT")));
    return port;
}
...
std::string getListenerHost() {
    std::string host((getenv("EVENT_INSPECTOR_HOST") == 0
                          ? ""
                          : getenv("EVENT_INSPECTOR_HOST")));
    return host;
}
...
EventInspectorWriter::EventInspectorWriter()
    : port(getListenerPort()), host(getListenerHost()),
      socketListener(host, std::stoi(port)), clientSocket(),
      connectionClient([this](int fd) { return clientCallback(fd); },
                       SWA::Process::getInstance().getActivityMonitor()),
      connectionListener([this](int fd) { return listenerCallback(fd); },
                         SWA::Process::getInstance().getActivityMonitor()) {
...
void EventInspectorWriter::write(const std::string &buffer) {
    if (clientSocket) {
        try {
            ...
            std::cerr << " EventInspectorWriter::write failed : " << se.report()
                      << " " << se.reason() << std::endl;
            disconnectClient();
        }
    }
...
bool EventInspectorWriter::listenerCallback(int listenFd) {
    ...
    // one.
    try {
        ClientSocketType::SocketDescriptor clientSocketFd =
            socketListener.acceptSocket();
        if (clientSocketFd.isValid()) {
            disconnectClient();
            clientSocket = std::shared_ptr<ClientSocketType>(
                new ClientSocketType(clientSocketFd));
        ...
        std::cerr << "Event Inspector writer - Caught unexpected Listener "
                     "SKT::SocketException : "
                  << se.report() << std::endl;
...
bool EventInspectorWriter::clientCallback(int clientFd) {
    ...
    try {
        ...
        std::cerr << "Event Inspector writer - Caught unexpected client read "
                     "SKT::SocketException : "
                  << se.report() << std::endl;
    }
