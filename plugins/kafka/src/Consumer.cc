...
namespace Kafka {
    ...
    void Consumer::run() {
        ...
        dispatcher.run(
            ...
            [this](cppkafka::Message msg) {
                // Queue the message to be handled on the main thread
                messageQueue.enqueue(msg);
                SWA::Process::getInstance().getIOContext().post(SWA::Process::getInstance()
                                                                    .wrapProcessingThread("Kafka Message", [this]() {
                                                                        handleMessages();
                                                                    }));
            }
            // TODO error handling
    ...
    void Consumer::handleMessages() {
        ...
        if (!messageQueue.empty()) {
            ...
            for (auto it = msgs.begin(); it != msgs.end(); it++) {
                ...
                Callable service = ProcessHandler::getInstance()
                                       .getServiceHandler(msg.get_topic())
                                       .getInvoker(std::vector<uint8_t>(msg.get_payload()));
    ...
    void Consumer::createTopics(std::vector<std::string> topics) {
        ...
        for (auto it = topics.begin(); it != topics.end(); it++) {
            ...
            if (rd_kafka_topic_result_error(terr[0]) &&
                rd_kafka_topic_result_error(terr[0]) != RD_KAFKA_RESP_ERR_TOPIC_ALREADY_EXISTS) {
