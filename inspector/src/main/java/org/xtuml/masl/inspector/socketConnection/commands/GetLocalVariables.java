...
public class GetLocalVariables extends CommandStub<LocalVarData[]>
{
  ...
  public LocalVarData[] execute ( final CommunicationChannel channel ) throws IOException
  {
    ...
    if ( (position.getSource() instanceof ObjectServiceMetaData && ((ObjectServiceMetaData)position.getSource()).getType() == ServiceType.Instance) )
    {
      final LocalVarData var = new LocalVarData("this", ((ObjectServiceMetaData)position.getSource()).getObject().getInstanceType());
      var.read(channel);
      result.add(var);
    }
    else if ( (position.getSource() instanceof StateMetaData && (((StateMetaData)position.getSource()).getType() == StateType.Normal || ((StateMetaData)position
                                                                                                                                                                .getSource()).getType() == StateType.Terminal)) )
    {
      final LocalVarData var = new LocalVarData("this", ((StateMetaData)position.getSource()).getObject().getInstanceType());
      var.read(channel);
      result.add(var);
    }
