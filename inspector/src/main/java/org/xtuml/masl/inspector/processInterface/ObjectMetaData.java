...
public abstract class ObjectMetaData
    implements Comparable<ObjectMetaData>
{
  ...
  public Node getXMLSchema ( final Document document )
  {
    ...
    final Element complexContent = document.createElementNS(XMLConstants.W3C_XML_SCHEMA_NS_URI, "xs:complexContent");
    ...
    if ( isActive() )
    {
      ...
      final Element currentStateDef = document.createElementNS(XMLConstants.W3C_XML_SCHEMA_NS_URI, "xs:simpleType");
  ...
  private void linkRelationships ()
  {
    ...
    for ( int i = 0; i < getRelationships().length; i++ )
    {
      ...
      if ( relMeta != null )
      {
        ...
        {
          // Simple Relationship
          if ( relMeta.getRightObject().equals(this) && relMeta.getLeftRole().equals(objRelMeta.getRolePhrase()) )
          {
            ...
            if ( relMeta.getRightMany()
                 || (!relMeta.getLeftMany() && relMeta.getRightConditional() && !relMeta.getLeftConditional()) )
            {
              rels.add(new FormalisedRelationship(relMeta, i, -1));
            }
          }
          else if ( relMeta.getLeftObject().equals(this) && relMeta.getRightRole().equals(objRelMeta.getRolePhrase()) )
          {
            // The current object is the left hand object of this relationship,
            // so add relationship to list if it can be formalised here.
            if ( !(relMeta.getRightMany() || (!relMeta.getLeftMany() && relMeta.getRightConditional() && !relMeta
                                                                                                                 .getLeftConditional())) )
            {
          ...
          else
          {
            ...
            System.err.println("leftObj=" + relMeta.getLeftObject().getName() + "leftRole=" + relMeta.getLeftRole());
            System.err.println("rightObj=" + relMeta.getRightObject().getName() + "rightRole=" + relMeta.getRightRole());
            System.err.println();
          }
        }
        else
