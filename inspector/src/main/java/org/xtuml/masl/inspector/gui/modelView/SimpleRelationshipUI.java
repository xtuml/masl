...
public class SimpleRelationshipUI extends ComponentUI
{
  ...
  public void installUI ( final JComponent c )
  {
    ...
    final RelationshipLineHeadUIResource leftLineHead = RelationshipLineHeadUIResource.getLineHead(rel.getLeftMany(),
                                                                                                   rel.getLeftConditional());
    final RelationshipLineHeadUIResource rightLineHead = RelationshipLineHeadUIResource.getLineHead(rel.getRightMany(),
  ...
  public void calcSizes ()
  {
    ...
    mainArrowHeight = arrowTextHeight + bottomHalfArrowHeight + Math.max(roleHeight, assocArrowLineMinLength + assocObjectHeight);
  ...
  public Dimension getPreferredSize ( final JComponent c )
  {
    ...
    if ( newMetrics != metrics )
    {
      ...
      int prefArrowLineWidth = Math.max(leftArrowHeadWidth + leftCardWidth, rightArrowHeadWidth + rightCardWidth) * 2
                               + gapWidth
                               + numberWidth
                               + assocArrowHeadWidth
                               + gapWidth;
      ...
      final int prefWholeArrowWidth = Math.max(Math.max(prefRoleLineWidth, prefArrowLineWidth), prefAssocObjectWidth);
