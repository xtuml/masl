...
class CollectionTableModel extends AbstractTableModel
{
  ...
  public int getPreferredColumnWidth ( final int col, final JTable parent )
  {
    ...
    if ( col == 0 )
    {
      headerWidth = headerRenderer.getTableCellRendererComponent(parent, "[88..88]", false, false, 0, 0).getPreferredSize().width;
    }
    else
    {
      headerWidth = headerRenderer.getTableCellRendererComponent(parent, getColumnName(col), false, false, 0, 0).getPreferredSize().width;
      TypeMetaData cellType;
      if ( data.getType().getCollectionDepth() == 0 && data.getType().getBasicType() == TypeMetaData.BasicType.Structure )
      {
