...
public abstract class DurationData extends DataValue<DurationData>
    implements Comparable<DurationData>
{
  ...
  public String toISO ()
  {
    ...
    if ( split.hasTime )
    {
      result = result
               + "T"
               + (split.hours > 0 ? split.hours + "H" : "")
               + (split.minutes > 0 ? split.minutes + "M" : "")
               + ((split.seconds > 0 || split.nanos > 0) ? split.seconds
                                                           + (split.nanos > 0 ? fractionalFormat.format(split.secondsFraction) : "")
                                                           + "S"
                                                        : "");
    }
    return result;
