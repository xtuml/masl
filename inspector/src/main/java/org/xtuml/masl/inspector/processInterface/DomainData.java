    ...
    implements org.xtuml.masl.inspector.processInterface.XMLSerializable
{
      ...
      if ( curNode.getNodeType() == Node.ELEMENT_NODE )
      {
        ...
        else
        {
          ...
          else
          {
            ...
            if ( ssMeta == null )
            {
              throw new IndexOutOfBoundsException("Relationship " + curNode.getNodeName() + " not found.");
            }
  ...
  public void writeToProcess () throws java.rmi.RemoteException
  {
    ...
    for ( final RelationshipMetaData relMeta : relData.keySet() )
    {
      ...
      for ( final RelationshipData link : linkData )
      {
        ...
        if ( rightObj == null )
        {
          System.out.println("Warning: " + rightObject.getName() + " id=" + link.getRightId() + " not found.");
          break;
        }
        ...
        if ( relMeta.getAssocObject() != null )
        {
          ...
          if ( assocObj == null )
          {
            System.out.println("Warning: " + assocObject.getName() + " id=" + link.getAssocId() + " not found.");
            break;
          }
    ...
    for ( final SuperSubtypeMetaData ssMeta : ssData.keySet() )
    {
      ...
      for ( final SuperSubtypeData link : ssLinkData )
      {
        ...
        if ( superObj == null )
        {
          System.out.println("Warning: " + superObject.getName() + " id=" + link.getSupertypeId() + " not found.");
          break;
        }
        final InstanceData subObj = subLookup.get(link.getSubtypeId());
        if ( subObj == null )
        {
          System.out.println("Warning: " + subObject.getName() + " id=" + link.getSubtypeId() + " not found.");
          break;
        }
      ...
      for ( int i = 0; i < objectMeta.getAttributes().length; ++i )
      {
        ...
        if ( attMeta.getType().getBasicType() == TypeMetaData.BasicType.Timer )
        {
          ...
          for ( final InstanceData localInstance : instances )
          {
            ...
            {
              final InstanceData dbInstance = ProcessConnection.getConnection().getInstanceData(objectMeta,
                                                                                                localInstance.getPrimaryKey());
  ...
  void relinkKey ( final DataValue<?> value )
  {
    if ( value instanceof InstanceIdData )
    {
      ...
      if ( instanceData == null )
      {
        System.out.println("Warning: " + instance.getMetaData().getName() + " id=" + instance.getId() + " not found.");
        instance.setId(null);
      }
      else
      {
