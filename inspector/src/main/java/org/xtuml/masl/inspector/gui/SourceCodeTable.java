...
class SourceCodeTable extends ToolTipTable
{
  ...
  private void init ()
  {
    ...
    getColumnModel().getColumn(SourceCodeTableModel.INDICATOR_COL).setCellRenderer(
                                                                                   new NoSelectTableCellRenderer()
      {
    ...
    getColumnModel().getColumn(SourceCodeTableModel.LINE_NO_COL).setCellRenderer(
                                                                                 new NoSelectTableCellRenderer()
      {
    ...
    getColumnModel().getColumn(SourceCodeTableModel.CODE_COL).setCellRenderer(
    ...
    registerKeyboardAction(toggleAction,
                           KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_B, Event.CTRL_MASK),
  ...
  private class MouseHandler extends MouseAdapter
  {
    ...
    {
      if ( Capability.SET_BREAKPOINT.isAvailable() &&
           (e.getModifiers() & InputEvent.BUTTON1_MASK) != 0 && e.getClickCount() > 1 )
      {
  ...
  public void scrollToLine ( final int line )
  {
    ...
    final Rectangle belowRow = getCellRect(Math.min(model.getRowCount() - 1, line - 1 + Preferences.getBelowContextLines()),
                                           0,
                                           true);
