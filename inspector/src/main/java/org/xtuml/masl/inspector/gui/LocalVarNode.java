...
public class LocalVarNode
{
  ...
  public String toString ()
  {
    ...
    else if ( value instanceof InstanceIdData )
    {
      ...
      {
        return name + " = " + data.getMetaData().getName() + " " + data.getMetaData().getInstanceIdentifier(data.getInstanceData());
      }
    }
    ...
    else if ( value instanceof CollectionData )
    {
      ...
      {
        return name + " = [" + ((CollectionData)value).getStartIndex() + ".." + ((CollectionData)value).getEndIndex() + "]";
      }
    }
  ...
  public LocalVarNode getChild ( final int index )
  {
    ...
    if ( child == null )
    {
      if ( value instanceof LocalVarData[] )
      {
        child = new LocalVarNode(index,
                                 ((LocalVarData[])value)[index].getName(),
                                 ((LocalVarData[])value)[index].getValue());
      }
      else if ( value instanceof StructureData )
      {
        child = new LocalVarNode(index,
                                 ((StructureData)value).getMetaData().getAttributes()[index].getName(),
                                 ((StructureData)value).getAttributes()[index]);
      }
      else if ( value instanceof CollectionData )
      {
        child = new LocalVarNode(index,
                                 "[" + (index + ((CollectionData)value).getStartIndex()) + "]",
                                 ((CollectionData)value).getData().get(index));
      }
      else if ( value instanceof DictionaryData )
      {
        final DataValue<?> key = new ArrayList<DataValue<?>>(((DictionaryData)value).getData().keySet()).get(index);
        child = new LocalVarNode(index,
                                 "[" + key + "]",
                                 ((DictionaryData)value).getData().get(key));
      }
      else if ( value instanceof TimerData )
      {
      ...
      else if ( value instanceof InstanceIdData )
      {
        ...
        {
          child = new LocalVarNode(index,
                                   meta.getAttributes()[index].getName(),
                                   data.getAttributes()[index]);
        }
        else
        {
          child = new LocalVarNode(index, "Current State", meta.getStates()[data.getCurrentState()].getName());
        }
      }
  ...
  {
    return !(value instanceof LocalVarData[] ||
             value instanceof StructureData || value instanceof DictionaryData ||
             value instanceof CollectionData || value instanceof InstanceIdData ||
             value instanceof TimerData || value instanceof EventData);
  }
