...
public abstract class AbstractFormModel implements FormModel {
    ...
    /**
     * Returns a default name for the field using spreadsheet conventions: A, B, C,
     * ... Z, AA, AB, etc. If <code>field</code> cannot be found, returns an empty
     * string.
     *
     ...
     *
     * @param fieldName string containing name of field to be located
     ...
     *
     * @param fieldIndex the field being queried
     ...
     *
     * @return all of this model's <code>FormModelListener</code>s or an empty array
     *         if no form model listeners are currently registered
     *
     ...
     * changed. The number of rows may also have changed and the <code>JForm</code>
     * should redraw the form from scratch. The structure of the form (as in the
     * order of the fields) is assumed to be the same.
     *
     * @see FormModelEvent
     ...
     * event and its <code>autoCreateFieldsFromModel</code> flag is set it discards
     * any form fields that it had and reallocates default fields in the order they
     ...
     * @param field field of cell which has been updated
     * @see FormModelEvent
     * @see EventListenerList
     */
    ...
    /**
     * Forwards the given notification event to all <code>FormModelListeners</code>
     * that registered themselves as listeners for this form model.
     *
     ...
     * model <code>m</code> for its form model listeners with the following code:
     *
     * <pre>
     *
