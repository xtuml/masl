...
class ObjectDetailsPane extends JTabbedPane {
    ...
    ObjectDetailsPane(final ObjectList objectList) {
        if (Preferences.getModellingMode() == Preferences.ModellingMode.UML) {
            addTab("Class Diagram",
                    new JScrollPane(new org.xtuml.masl.inspector.gui.modelView.ObjectDisplay(objectList)));
        } else {
            addTab("Information Model",
                    new JScrollPane(new org.xtuml.masl.inspector.gui.modelView.ObjectDisplay(objectList)));
        }
        ...
        instanceServiceList.getModel()
                .addListDataListener(new TabHider(getTabCount() - 1, instanceServiceList.getModel()));
