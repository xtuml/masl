...
class InstanceViewModel extends com.jrefinery.ui.SortableTableModel
{
  ...
  class InstanceComparator
      implements Comparator<InstanceData>
  {
  ...
  class InstanceListenerThread extends Thread
      implements InstanceDataListener
  {
  ...
  public int getPreferredColumnWidth ( final int col, final JTable parent )
  {
    ...
    switch ( getColumnType(col) )
    {
      case ATTRIBUTE:
        ...
        {
          for ( final String name : metaObj.getAttributes()[getAttributeNo(col)].getType().getEnumerate().getNames() )
          {
        ...
        break;
      case RELATIONSHIP:
        headerWidth = headerRenderer.getTableCellRendererComponent(parent,
                                                                   metaObj.getRelationships()[getRelationshipNo(col)].getNumber(),
                                                                   false,
                                                                   false,
                                                                   0,
                                                                   0).getPreferredSize().width;

        cellWidth = cellRenderer.getTableCellRendererComponent(parent, new String("\u21e2 99999"), false, false, 0, 0)
                                .getPreferredSize().width;
        break;
      case CURRENT_STATE:
        for ( final StateMetaData state : metaObj.getStates() )
        {
          cellWidth = Math.max(cellWidth, cellRenderer.getTableCellRendererComponent(parent, state.getName(), false, false, 0, 0)
                                                      .getPreferredSize().width);
        }
        break;
      case PK:
        headerWidth = headerRenderer.getTableCellRendererComponent(parent, getColumnName(col), false, false, 0, 0)
                                    .getPreferredSize().width;
        cellWidth = cellRenderer.getTableCellRendererComponent(parent, new Integer(10000), false, false, 0, 0).getPreferredSize().width;
        break;
      default:
        throw new IllegalStateException("Unrecognised column type");
    }
  ...
  public String getTitle ()
  {
    ...
    else
    {
      ...
      final String id = sourceInstance == null ? "!! Deleted !!" : sourceMeta.getInstanceIdentifier(sourceInstance);

      title = sourceMeta.getDomain().getName()
              + "::"
              + sourceMeta.getName()
              + " "
              + id
              + " "
              + metaRel.getDescription()
              + " ("
              + metaRel.getNumber()
              + ")";
    }
  ...
  public void sortByColumn ( final int col, boolean ascending )
  {
    ...
    { /* Ignore and let loader sort when complete */
    }
  }
    ...
    final Thread loader = new Thread(getTitle() + " Data Loader")
    {
      ...
      public void run ()
      {
        try
        {
          ...
          {
            ProcessConnection.getConnection().getRelatedInstanceData(sourceMeta, sourcePK, sourceRel, listener);
          }
        }
        catch ( final Exception e )
        {
