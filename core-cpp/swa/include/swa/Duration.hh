...
namespace SWA
{
  ...
  class Duration : private boost::totally_ordered<Duration,
                           boost::additive<Duration,
                           boost::multiplicative<Duration,int,
                           boost::multiplicative<Duration,long,
                           boost::multiplicative<Duration,long long,
                           boost::multiplicative<Duration,unsigned int,
                           boost::multiplicative<Duration,unsigned long,
                           boost::multiplicative<Duration,unsigned long long,
                           boost::multiplicative<Duration,double
                            > > > > > > > > >
  {
    private:
    ...
    public:
      ...
      enum DurationFields { Weeks     = 0x80, 
                            Days      = 0x40,
                            Hours     = 0x20,
                            Minutes   = 0x10,
                            Seconds   = 0x08,
      ...
      Duration ( DurationFields fields, 
                 int64_t value1, 
                 int64_t value2 = 0, 
                 int64_t value3 = 0, 
                 int64_t value4 = 0, 
      ...
      template<class T> static Duration fromWeeks   ( T value )  { return Duration(WEEKS  ,value); }
      template<class T> static Duration fromDays    ( T value )  { return Duration(DAYS   ,value); }
      template<class T> static Duration fromHours   ( T value )  { return Duration(HOURS  ,value); }
      template<class T> static Duration fromMinutes ( T value )  { return Duration(MINUTES,value); }
      template<class T> static Duration fromSeconds ( T value )  { return Duration(SECONDS,value); }
      template<class T> static Duration fromMillis  ( T value )  { return Duration(MILLIS ,value); }
      template<class T> static Duration fromMicros  ( T value )  { return Duration(MICROS ,value); }
      template<class T> static Duration fromNanos   ( T value )  { return Duration(NANOS  ,value); }
      ...
      String format_hms ( FormatField smallestField   = Second,
                          int         decimalPlaces   = 0,
                          bool        truncateDecimal = true,
                          bool        compactForm     = false ) const;

      String format     ( FormatField             largestField, 
      ...
      bool operator== ( const Duration& rhs ) const { return ticks == rhs.ticks; };
      long double operator/ ( const Duration& rhs ) const { return static_cast<long double>(ticks) / rhs.ticks; }
    ...
    private:
      ...
      {
        if ( scaled > (std::numeric_limits<Tick>::max())/scale ) throw SWA::ProgramError("Duration Overflow");
        ...
        if ( scaled < 0 && scaled < (std::numeric_limits<Tick>::min())/scale ) throw SWA::ProgramError("Duration Underflow");
  ...
  inline Duration::DurationFields operator| ( Duration::DurationFields field1, Duration::DurationFields field2 )
  {
    return Duration::DurationFields(static_cast<uint32_t>(field1) | static_cast<uint32_t>(field2));
  }
