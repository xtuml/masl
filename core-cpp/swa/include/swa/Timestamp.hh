...
namespace SWA
{
  ...
  class Timestamp : private boost::totally_ordered<Timestamp,boost::additive<Timestamp,Duration> >
  {
    ...
    public:
      enum TimestampFields { CalendarYear   = 0x8000, 
                             MonthOfYear    = 0x4000,
                             DayOfMonth     = 0x2000,
                             WeekYear       = 0x1000,
                             WeekOfYear     = 0x0800,
      ...
      template<class T>
      static Timestamp fromDaysSinceEpoch   ( T value )  { return Timestamp(DAYS  ,value); }

      template<class T>
      static Timestamp fromNanosSinceEpoch   ( T value )  { return Timestamp(NANOS  ,value); }

      template<class T>
      static Timestamp fromSecondsSinceEpoch   ( T value )  { return Timestamp(SECONDS  ,value); }
  ...
  private:
      ...
      {
        if ( scaled > (std::numeric_limits<Tick>::max())/scale ) throw SWA::ProgramError("Timestamp Overflow");
        if ( scaled < (std::numeric_limits<Tick>::min())/scale ) throw SWA::ProgramError("Timestamp Underflow");
        return static_cast<Tick>(scaled)*scale;
      }
      static Tick scaledToTicks ( double scaled, Units scale ) { return scaledToTicks(static_cast<long double>(scaled),scale); }
      ...
      template<class Iterator>
      static bool readDatePart ( Iterator& it, const Iterator& end, Timestamp::TimestampFields& fields, std::vector<int64_t>& values, bool& useSeparators );

      template<class Iterator>
      static Tick readTicksFromDecimal ( Iterator& it, const Iterator& end, size_t length, Units scale );

      template<class Iterator>
      static bool readTimePart ( Iterator& it, const Iterator& end, Timestamp::TimestampFields& fields, std::vector<int64_t>& values, bool useSeparators );
