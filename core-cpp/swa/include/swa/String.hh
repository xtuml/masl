...
namespace SWA
{
  ...
  class String : private boost::less_than_comparable<String, boost::equality_comparable<String > >
  {
    public:
      //*********************************************************
      ...
      String ( ) : data() {}
      template<class It>
      String ( It i, It j ) : data(i,j) {}

      iterator insert ( iterator p, value_type c ) { return data.insert(p,c); }
      template<class It>
      void insert ( iterator p, It i, It j ) { data.insert(p,i,j); }
      ...
      String& replace ( size_type idx, size_type len, const SWA::String& str, size_type str_idx, size_type str_num )
      {
      ...
      String& replace ( size_type idx, size_type len, const value_type* value_types, size_type value_types_len )
      {
        data.replace(idx,len,value_types,value_types_len);
        return *this;
      }

      String& replace ( iterator beg, iterator end, const value_type* value_types, size_type value_types_len )
      {
      ...
      String& replace ( iterator beg, iterator end, InputIterator newBeg, InputIterator newEnd )
      {
      ...
      String& insert ( size_type idx, const value_type* value_types, size_type value_types_len )
      {
      ...
      void resize( size_type num, value_type c )
      {
        data.resize(num,c);
      }
      ...
      explicit String ( const boost::iterator_range<const_iterator>& rhs ) : data(rhs.begin(),rhs.end()) {}

      operator const Container&() const { return data; }

      template<class V>
      operator Set<V>() const { return Set<V>(data.begin(),data.end()); }
      template<class V>
      operator Bag<V>() const { return Bag<V>(data.begin(),data.end()); }
      template<class V>
      operator Sequence<V>() const { return Sequence<V>(data.begin(),data.end()); }
      ...
      // Concatenation operator. Returns a new String.
      String operator + ( value_type rhs ) const
      {
        return data + rhs;
      }
      ...
      {
        std::string result;
        std::transform(data.begin(),data.end(),std::back_inserter(result),ToLower);
        return result;
      }
      ...
      {
        std::string result;
        std::transform(data.begin(),data.end(),std::back_inserter(result),ToUpper);
        return result;
      }
      ...
      const_iterator iteratorAt(size_type pos) const
      {
        return begin() + pos - 1;
      }

      const_iterator iteratorAfter(size_type pos) const
      {
        return begin() + pos;
      }

      iterator iteratorAt(size_type pos)
      {
        return begin() + pos - 1;
      }

      iterator iteratorAfter(size_type pos)
      {
        return begin() + pos;
      }
  ...
  private:
      ...
      { 
        if ( index < 1 ) throw SWA::ProgramError ("Attempt to read String element before start");
        if ( index > size() ) throw SWA::ProgramError ("Attempt to read String element after end");
      }
  };
