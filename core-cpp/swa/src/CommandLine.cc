...
namespace SWA
{
  NamedOption::NamedOption ( const std::string& name, const std::string& usageText, bool required, const std::string& paramDescription, bool paramRequired, bool multipleAllowed )
    : name(name),
  ...
  {
    if ( name[0] != '-' ) throw ProgramError ("Named Option must begin with '-'");
  }
  ...
  std::string NamedOption::getUsage() const
  {
    ...
    if ( !required ) usage = "[" + usage + "]";

    if ( multipleAllowed ) usage+= "...";
  ...
  {
    // Having any options after one that allows multiple values would make it impossible to determine which is which
    if ( !trailingOptions.empty() && trailingOptions.back().isMultiple() ) 
    {
      throw ProgramError("Only final trailing option can have multiple values. " + option.getUsage() + " supplied after " + trailingOptions.back().getUsage() + ".");
    }

    // Having an required option after an optional one would make it impossible to determine which is which
    if ( !trailingOptions.empty() && !trailingOptions.back().isRequired() && option.isRequired() ) 
    {
      throw ProgramError("Cannot have required trailing option after an optional one. " + option.getUsage() + " supplied after " + trailingOptions.back().getUsage() + ".");
    }

    // Check this option isn't already present
    for ( TrailingOptions::const_iterator it = trailingOptions.begin(), end = trailingOptions.end(); it != end; ++it )
    {
      ...
      if ( (*it)[0] == '-' )
      {
        ...
        if ( found != namedOptions.end() )
        {
          ...
          if ( option.hasParameter() )
          {
            ...
            {
              // Nothing supplied, check whether it was flagged as required
        ...
        else
        {
          ...
          if ( !lenient ) throw ProgramError(getUsage());
          std::string paramValue;
  ...
  class PairSecondIterator : public boost::input_iteratable<PairSecondIterator<InnerIterator>,typename InnerIterator::value_type::second_type*>
  {
    public:
  ...
  std::vector<std::string> CommandLine::getMultiOption ( const std::string& name, const std::string& defaultVal ) const
  {
    ...
    std::vector<std::string> result(PairSecondIterator<OptionValues::const_iterator>(range.first),
  ...
  int CommandLine::getIntOption ( const std::string& name, int defaultVal ) const    
  {
    ...
    return val.empty()?defaultVal:boost::lexical_cast<int>(val);
  }
