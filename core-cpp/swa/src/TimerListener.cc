...
namespace SWA
{
  ...
  TimerListener::TimerListener ( const ListenerPriority& priority, const Callback& callback )
    : timerId(),
      callback(callback),
      expiryTime(),
      interval(),
      active(false)
  {
    ...
    {
      throw SWA::ProgramError(::boost::make_tuple("Failed to create timer : ",strerror(errno)));
    }

    Process::getInstance().getActivityMonitor().addTimerCallback(timerId, boost::bind(&TimerListener::timerFired,this,_1));
  }
  ...
  void TimerListener::schedule ( const Timestamp& expiryTime, const Duration& interval )
  {
  ...
  void TimerListener::setTime(const timespec& expiryTime, const timespec& interval )
  {
    ...
    if ( timeout.it_value.tv_sec < 0 || ( timeout.it_value.tv_sec == 0 && timeout.it_value.tv_nsec == 0 ) )
    {
    ...
    if ( timer_settime(timerId,TIMER_ABSTIME,&timeout,0) )
    {
      ...
      {
        throw SWA::ProgramError(::boost::make_tuple("Failed to schedule timer : ",strerror(errno)));
      }
    }
  }
  ...
  void TimerListener::cancel ()
  {
    ...
    {
      throw SWA::ProgramError(::boost::make_tuple("Failed to cancel timer : ",strerror(errno)));
    }    
  ...
  void TimerListener::timerFired ( int32_t overrun )
  {
    ...
    { 
      ...
      else
      {
        ...
        // Reschedule the timer to make sure we don't miss expirations due to race conditions.
        setTime(expiryTime.getTimespec(), interval.getTimespec() );    
      }
    }
    else
