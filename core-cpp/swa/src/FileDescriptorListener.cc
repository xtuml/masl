...
namespace SWA
{
      ...
      monitor(monitor)
  {
    ...
    if ( valid() ) monitor.addFdCallback(fd,boost::bind(&FileDescriptorListener::callCallback,this,_1));
  }
  ...
  {
    if ( valid() ) monitor.removeFdCallback(fd);
    cancel();
    this->fd = fd;
    initFd();
    updateFdStatus(active);
  ...
  {
    if ( fcntl(fd,F_SETSIG,priority.getValue())) throw ProgramError(strerror(errno)); 
    ...
    if ( fcntl(fd,F_SETOWN,syscall(SYS_gettid)) ) throw ProgramError(strerror(errno)); 
  }
  ...
  void FileDescriptorListener::updateFdStatus( bool makeActive )
  {
    if ( valid() )
    {
      if ( makeActive )
      {
        ...
        if ( flags == -1 || fcntl(fd,F_SETFL, flags | O_ASYNC) ) throw ProgramError(strerror(errno));
      ...
      else
      {
        ...
        if ( flags == -1 )
        {
          // Ignore a bad file descriptor - we are trying to cancel it anyway! 
          if ( errno != EBADF ) throw ProgramError(strerror(errno));
        }
        else
        {
          if ( fcntl(fd,F_SETFL, flags & ~O_ASYNC) ) throw ProgramError(strerror(errno));
        }
      }
    }
  }
  ...
  void FileDescriptorListener::setPriority( const ListenerPriority& priority )
  {
    ...
    if ( valid() && fcntl(fd,F_SETSIG,priority.getValue())) throw ProgramError(strerror(errno));    
  }
