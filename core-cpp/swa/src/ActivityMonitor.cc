...
namespace SWA
{
  ...
  {
    // Defer initialisation of the activity monitor until it is actually required for
    // use. This is so that additional applications can link against the SWA library
    // and use thier own signal handling without having to use the real-time signal 
  ...
  uint64_t ActivityMonitor::addSignalCallback (int signal, const SignalCallback& callback )
  {
     ...
     signalCallbacks.insert(SignalCallbackTable::value_type(signal,std::pair<uint64_t,SignalCallback>(nextId,callback)));     
  ...
  void ActivityMonitor::removeSignalCallback (uint64_t id )
  {
    ...
    if ( signal > 0 && signalCallbacks.find(signal) == signalCallbacks.end() )
    {
      ...
      // When the signal was registered its orginal signal handler was stored off
      // so it could be replaced when the signal callbacl was removed. Therefore
      // remove the dummy default handler and place the orginal back.
      ...
      if (signalItr != orgSignalHandlers.end()){
         ...
         if (sigaction(signal,&sa,0) < 0){
             throw ProgramError(std::string("ActivityMonitor::removeSignalCallback sigaction failed : ") + strerror(errno));
         }
  ...
  const ActivityMonitor::FdCallback& ActivityMonitor::getFdCallback(int fd) const
  {
    ...
    if ( it != fdCallbacks.end() ) return it->second;
    else return ignoreFd;   
  }
  ...
  const ActivityMonitor::FdExtendedCallback& ActivityMonitor::getFdExtCallback(int fd) const
  {
    ...
    if ( it != fdExtCallbacks.end() ) return it->second;
    else return ignoreFdExt;   
  }
  ...
  const ActivityMonitor::NormalCallback& ActivityMonitor::getNormalCallback(int id) const
  {
    ...
    if ( it != normalCallbacks.end() ) return it->second;
    else return ignoreNormal;   
  }

  const void ActivityMonitor::processSignalCallbacks(int signal, int pid, int uid) const
  {
    std::pair<SignalCallbackTable::const_iterator,SignalCallbackTable::const_iterator> range = signalCallbacks.equal_range(signal);
  ...
  const ActivityMonitor::TimerCallback& ActivityMonitor::getTimerCallback(timer_t id) const
  {
    ...
    if ( it != timerCallbacks.end() ) return it->second;
    else return ignoreTimer;   
  }

  bool ActivityMonitor::pollActivity( int timeout_ms ) const
  {
    if (!initialised) {
        throw ProgramError("ActivityMonitor::pollActivity failed : monitor has not been initialised");
    }
  ...
  {
    if (!initialised) {
        throw ProgramError("ActivityMonitor::waitOnActivity failed : monitor has not been initialised");
    }
  ...
  void ActivityMonitor::processActivity( const siginfo_t& info ) const
  {
    switch ( info.si_code )
    {
      ...
      case SI_TIMER:
      {
        ...
        // associated signal event data is populated before the call to timer_create
        // the time id is held as a pointer in the union sigval field. Therefore access
        // it accordingly
      ...
      default:
      {
        ...
        {
          throw SWA::ProgramError("unrecognised signal type : " + boost::lexical_cast<std::string>(info.si_code));
        }
      }
    }
  }
