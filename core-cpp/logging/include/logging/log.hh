#ifndef CORE_CPP_LOG_HH
#define CORE_CPP_LOG_HH
...
namespace xtuml::logging {
    /**
     * A Wrapper around log4cplus and fmt to allow formatted logging without nasty macros.
     */
    class Logger {
    public:

        /**
         * A wrapper around an fmt::basic_format_string that captures the current source location.
         * This enables the log functions to receive their format arguments in a variadic template.
         * Without this, we would have to add the location as a default final parameter on each log
         * function, removing the ability to use variadic arguments.
         */
        ...
        public:
            ...
            explicit(false)
            consteval
            basic_format_string_with_source_location(const S &s, std::source_location l = std::source_location::current())
                    : fmt::basic_format_string<Char, Args...>(s),
                      location_(l) {

            }

            template<typename S> requires (!std::convertible_to<S,
                    fmt::basic_string_view < Char>> || std::same_as<S, basic_format_string_with_source_location>)

            explicit(false)
            basic_format_string_with_source_location(S &&s,
        ...
        using format_string_with_source_location = basic_format_string_with_source_location<char, fmt::type_identity_t < Args>...>;

        /**
         * Loads the log4cplus configuration from a config file, and then watches the file for
         * changes at the specified interval. A zero interval will stop watching.
         * @param filename
         * @param interval
         */
        static bool load_config(
            ...
            if (std::fstream{filename}) {
                if ( interval > std::chrono::seconds{0} ) {
                    log_.debug("Watching log config file: {}", filename);
                    config_watcher_.emplace(filename, interval.count());
                } else {
                    config_watcher_.reset();
                    log_.debug("Loading log config file: {}", filename);
                    log4cplus::PropertyConfigurator::doConfigure(filename);
                }
                return true;
            } else {
                config_watcher_.reset();
                log_.error("Error opening log config: {}", filename);
                return false;
            }
        }
        ...
        void log(Level level,
                 format_string_with_source_location<Args...> message, const Args &... args) const {
    ...
    private:
        ...
        convert_level(Level
                      level) {
            constexpr
            std::array<log4cplus::LogLevel, 6> lookup = {
    ...
    private:
        /**
         * Helper to initialize the log4cplus framework
         */
        class Init {
            friend class Logger;
        ...
        inline static Init init_{};
        inline static std::optional<log4cplus::ConfigureAndWatchThread> config_watcher_{};
        static Logger log_;

    };

    inline Logger Logger::log_{"xtuml.logging.config"};
