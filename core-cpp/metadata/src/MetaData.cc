...
namespace SWA
{
  ...
  ServiceMetaData::ServiceMetaData ( int id, ServiceType type, const std::string& name, const std::vector<int>& lines, const std::string& fileName, const std::string& fileHash )
    : id(id),
      type(type),
      name(name),
      function(false),
  ...
  ServiceMetaData::ServiceMetaData ( int id, ServiceType type, const std::string& name, const std::string& returnTypeName, const TypeMetaData& returnType, const std::vector<int>& lines, const std::string& fileName, const std::string& fileHash )
    : id(id),
      type(type),
      name(name),
      function(true),
  ...
  StateMetaData::StateMetaData ( int id, StateType type, const std::string& name, const std::vector<int>& lines, const std::string& fileName, const std::string& fileHash )
    : id(id),
      type(type),
      name(name),
      lines(lines),
  ...
  TypeMetaData::TypeMetaData (BasicType basicType, const TypeMetaData& keyType, const TypeMetaData& valueType, int collectionDepth )
    : basicType(basicType),
      typeDomain(-1),
      typeId(-1),
      collectionDepth(collectionDepth),
      dictKey(boost::shared_ptr<TypeMetaData>(new TypeMetaData(keyType))),
      dictValue(boost::shared_ptr<TypeMetaData>(new TypeMetaData(valueType)))
  {
  }
  ...
  RelationshipMetaData::RelationshipMetaData ( int id, const std::string& name, 
  ...
  namespace 
  {
    ...
    bool loadLibs()
    {
      SWA::Process::getInstance().loadDynamicLibraries ( "metadata", "_if", true );
      return true;
    }
  ...
  ProcessMetaData::DomainFunction ProcessMetaData::getDomainGetter ( int domainId ) const
  {
    ...
    {
      throw SWA::ProgramError("No Meta Data present for " + SWA::Process::getInstance().getDomain(domainId).getName() + "." );
    }
  ...
  class FullNameFormatter : public NameFormatter
  {
    private:
      ...
      {
        return ProcessMetaData::getProcess().getDomain(domainId).getTerminator(terminatorId).getService(serviceId).getName();
      }
  ...
  bool formatterInit = NameFormatter::overrideFormatter ( boost::shared_ptr<NameFormatter>(new FullNameFormatter()) );
}
