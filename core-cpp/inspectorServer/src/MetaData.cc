...
namespace Inspector
{
  ...
  void BufferedOutputStream::write<SWA::ProcessMetaData> ( const SWA::ProcessMetaData& process )
  {
    ...
    for ( SWA::Process::DomainList::const_iterator it = domains.begin(), 
          ...
          it != end; ++it )
    {
      write(process.getDomain(it->getId()));
    }
  }
  ...
  template<>
  void BufferedOutputStream::write<boost::shared_ptr<SWA::Event> > ( const boost::shared_ptr<SWA::Event>& event )
  {
    write(*event);
  }
  ...
  template<>
  void BufferedOutputStream::write<boost::shared_ptr<SWA::EventTimer> > ( const boost::shared_ptr<SWA::EventTimer>& timer )
  {
    write(*timer);
  }
