...
class ObjectMapper : public ResourceMonitorObserver
{
   public:
      ...
      ::SWA::IdType getNextArchId();
       ...
       // ********************************************************************
       std::string getObjectName() const { return sqlGenerator->getObjectName();  }

       // ********************************************************************
       //! \return the name of the SQL Table by this mapper. 
       // ********************************************************************
       std::string getTableName()  const { return sqlGenerator->getTableName(); }
       ...
       //! point just before the next database commit. Rather than using this behaviour, 
       //! this method can be used to change the behaviour so that the default 
       //! batch operation is replaced with an implementation that undertakes a 
       //! database write on each modification. This method can be called at any time, 
       //! to turn on and off the batch behaviour.
       //!
       ...
       // ********************************************************************
       //! Force all dirty objects to have their modifications applied to the database. 
       //! Required to make sure database is upto date with modifications before a 
       //! select opertaion is performed on the associated table.
       // ********************************************************************
       void forceFlush();
       ...
       void selectOne (::boost::function<bool (PsObject*)>& predicate, PsBaseObjectPtr& object);
       void selectOne (Criteria& sqlSelector, PsBaseObjectPtr& object);

       void selectAll (::boost::function<bool (PsObject*)>& predicate, PsBaseObjectPtrSwaSet& objectSet);
       void selectAll (Criteria& sqlSelector, PsBaseObjectPtrSwaSet& objectSet);

       void selectOneFromCache (const std::string& predicateDebug, ::boost::function<bool (PsObject*)>& predicate, PsBaseObjectPtr& objectSet);
       void selectAllFromCache (const std::string& predicateDebug, ::boost::function<bool (PsObject*)>& predicate, PsBaseObjectPtrSwaSet& objectSet);

   protected:
                 ObjectMapper(const boost::shared_ptr< ObjectSqlGenerator<PsBaseObject,PsObject> >& sqlGenerator);
   ...
   protected:
        ...
        boost::shared_ptr< ObjectSqlGenerator<PsBaseObject,PsObject> > sqlGenerator;

        ObjectMapperUnitOfWork<ObjectMapper>    unitOfWorkMap;
        boost::shared_ptr<CacheStrategy>        cacheStrategy;
};
...
// ***********************************************************************
template <class ObjectBase, class ObjectDerived>
ObjectMapper<ObjectBase,ObjectDerived>::ObjectMapper(const boost::shared_ptr< ObjectSqlGenerator<PsBaseObject,PsObject> >& sqlGenerator):
    writeOnModify(false),
    allLoaded(false),
    objectCount(0),
    ...
    unitOfWorkMap(*this),
    cacheStrategy(CacheStrategyFactory::singleton().getStrategy(sqlGenerator->getDomainName(),sqlGenerator->getObjectName()))
{
...
{
  // The objects cached in this object mapper might also be cached by the population
  // class as part of the find optimisation that has been applied to finds
  // on preferred attributes. If the installed strategy limits the number
  // of the object instances cached then the caching in the population class
  // has to be disabled as it will not contain the full population. 
...
void ObjectMapper<ObjectBase,ObjectDerived>::deleteInstance (const PsBaseObjectPtr object) 
{
   ...
   // by other mappers (i.e. relationshipUnitOfWorkMapper during link/unlink requests) during
   // the current transaction. Once the transaction has been committed then this
   // set of delete objects can be cleared.
...
template <class ObjectBase, class ObjectDerived> 
typename ObjectMapper<ObjectBase,ObjectDerived>::PsObjectPtr 
 ObjectMapper<ObjectBase,ObjectDerived>::findOne ()
{
    ...
    if (objectCount != 0 && cache.empty() == true){
        ...
        load((cacheStrategy->getOperationalCount(objectCount) == 0 ? 1 : cacheStrategy->getOperationalCount(objectCount)));
    }
...
bool ObjectMapper<ObjectBase,ObjectDerived>::allowInMemoryFind()
{
   ...
   if (allLoaded == true){
     ...
     if (cacheStrategy->allowFullCaching(objectCount) &&
         cacheStrategy->allowLinearFind(objectCount)){
         loadAll();
         allowFind = true;
     }
   }
...
void ObjectMapper<ObjectBase,ObjectDerived>::loadSingle (const ::SWA::IdType architectureId)
{
   if (allLoaded == false){
       if (cache.find(architectureId) == cache.end()){   
           ...
           criteria.addAllColumn();
           criteria.addFromClause(sqlGenerator->getTableName());
           std::string whereCondition = sqlGenerator->getColumnName("architecture_id") + " = " + valueToString(architectureId);
...
void ObjectMapper<ObjectBase,ObjectDerived>::loadSet (const PsObjectIdSet& identitySet)
{
   if (allLoaded == false){
       ...
       for(;idItr != idEnd; ++idItr){
           ...
           if (std::find_if(cache.begin(),cache.end(),AssociativeKeyAccessor< typename PsCachedPtrMap::value_type>(unqiueKey)) == cache.end()){
       ...
       if (!missingSet.empty()){
           ...
           if (fullCachingEnabled()){
              ...
              for(typename PsObjectIdSet::iterator missingItr = missingSet.begin(); missingItr != missingSet.end(); ++missingItr){
 ...
 ObjectMapper<ObjectBase,ObjectDerived>::locate (const ::SWA::IdType& identity)
{
   ...
   if (locatedObjItr == cache.end()){
       throw SqlException(::boost::make_tuple("ObjectMapper::locate failed for",sqlGenerator->getDomainName(),sqlGenerator->getObjectName(),"with identify",identity));
   }
...
void ObjectMapper<ObjectBase,ObjectDerived>::report (ResourceMonitorContext& context)
{
   if (context.getReportStream() != 0){
       ...
       (*reportStrm) << "   getOperationalCount : "  << cacheStrategy->getOperationalCount(objectCount) << std::endl;
   }
}
...
// ***********************************************************************
template <class ObjectBase, class ObjectDerived> 
void ObjectMapper<ObjectBase,ObjectDerived>::selectOne (::boost::function<bool (PsObject*)>& predicate, PsBaseObjectPtr& object)
{
   if (allowInMemoryFind()){
       std::find_if(cache.begin(),cache.end(),singleObjectSelector(cache,object,predicate));
   }
   else{
     throw SqlException(::boost::make_tuple("ObjectMapper::selectOne failed for",sqlGenerator->getDomainName(),sqlGenerator->getObjectName(),"in memory find not allowed"));
   }
}
...
// ***********************************************************************
template <class ObjectBase, class ObjectDerived> 
void  ObjectMapper<ObjectBase,ObjectDerived>::selectAll (::boost::function<bool (PsObject*)>& predicate, PsBaseObjectPtrSwaSet& objectSet)
{
   if (allowInMemoryFind()){
     ...
     throw SqlException(::boost::make_tuple("ObjectMapper::selectAll failed for",sqlGenerator->getDomainName(),sqlGenerator->getObjectName(),"in memory find not allowed"));
   }
}
...
// ***********************************************************************
template <class ObjectBase, class ObjectDerived> 
void  ObjectMapper<ObjectBase,ObjectDerived>::selectOneFromCache (const std::string& predicateDebug, ::boost::function<bool (PsObject*)>& predicate, PsBaseObjectPtr& object)
{
    // For Objects that contain attributes with complex types. Any finds undertaken that require access
    // to any fields of the complex data cannot be undertaken by an SQL query on the database, as complex
    // types are stored as ASN.1 encoded binary blobs. Therefore the only way the find can be satisfied is
    // to load all the data into transient memory and undertake a linear search. This might go against the
    ...
    if (allLoaded == false && !allowInMemoryFind()){
        // Cache is not supposed to undertake inmemory finds, but query contains references to
        // complex data, so have no choice but to cache all data and undertake linear find on 
        // cache, so report warning.
        std::cout << "Warning  ObjectMapper::selectOneFromCache for table " << sqlGenerator->getTableName() 
                  << " - using cache which CacheStrategy prohibits, for find expression using complex type " << predicateDebug << std::endl;
    }
    loadAll();    
    std::find_if(cache.begin(),cache.end(),singleObjectSelector(cache,object,predicate));
}
...
// ***********************************************************************
template <class ObjectBase, class ObjectDerived> 
void  ObjectMapper<ObjectBase,ObjectDerived>::selectAllFromCache (const std::string& predicateDebug, ::boost::function<bool (PsObject*)>& predicate, PsBaseObjectPtrSwaSet& objectSet)
{
    // For Objects that contain attributes with complex types. Any finds undertaken that require access
    // to any fields of the complex data cannot be undertaken by an SQL query on the database, as complex
    // types are stored as ASN.1 encoded binary blobs. Therefore the only way the find can be satisfied is
...
{
  std::for_each(objects.begin(),objects.end(),boost::bind(&ObjectSqlGenerator<PsBaseObject,PsObject>::executeUpdate,boost::ref(sqlGenerator),_1));
}
...
{
  std::for_each(objects.begin(),objects.end(),boost::bind(&ObjectSqlGenerator<PsBaseObject,PsObject>::executeInsert,boost::ref(sqlGenerator),_1));
}

// ***********************************************************************
// ***********************************************************************
template <class ObjectBase, class ObjectDerived> 
void ObjectMapper<ObjectBase,ObjectDerived>::commitDelete (const std::vector<SWA::IdType>& objects, std::string& sql)
{
  std::for_each(objects.begin(),objects.end(),boost::bind(&ObjectSqlGenerator<PsBaseObject,PsObject>::executeRemoveId,boost::ref(sqlGenerator),_1));
}
