...
{
    public:
       ObjectMapperSelector(Set& selectedSet, Pred& predicate):pred(predicate),selected(selectedSet) {}
...
{
    public:
       SingleObjectSelector(Obj& object, Pred& predicate):selectObj(object),pred(predicate) {}
...
class MapToSetKeyComparatorItr : public std::iterator <std::output_iterator_tag,void,void,void,void>
{
    public:
        ...
        explicit MapToSetKeyComparatorItr(const Map& map, Set& set):sourceMap(map),destinationSet(set){}
    ...
    public:
        ...
        LoadCacheIterator<Map,Set>& operator=(const typename Set::key_type& currentObj)
        {
             if (destination.find(currentObj->getArchitectureId()) == destination.end()){
                 destination.insert(std::make_pair(currentObj->getArchitectureId(), typename Map::mapped_type(currentObj.get())));
             }
             return *this;  
        }
