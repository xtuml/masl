...
namespace{
     ...
     {
        const std::string& name = SWA::Process::getInstance().getCommandLine().getOption("-db");
     ...
     bool registerInitRoutine()
     {
       ...
       SWA::Process::getInstance().getCommandLine().registerOption (SWA::NamedOption("-db","Database",true,"name of database",true) );
       return true;
     }

     // *****************************************************************
     // *****************************************************************
     bool registerImpl = ::SQL::DatabaseFactory::singleton().registerImpl(&Database::singleton());
}

// *****************************************************************
// *****************************************************************
Database::ScopedFinalise::ScopedFinalise(const char* const loc, sqlite3_stmt* stmt):
       location(loc),
       ppStmt(stmt)
{
...
{ 
   if (ppStmt != 0) { 
       if (Database::finaliseCompile(location,ppStmt) == true){ 
         ...
         errorMessage += sqlite3_errmsg( ::SQLITE::Database::singleton().getDatabaseImpl() );
...
// *****************************************************************
// *****************************************************************
const std::string& Database::getDbName()
{
  return dbName_;
}
...
// *****************************************************************
// *****************************************************************
const std::string& Database::getCurrentError()
{
   return error_;
}
...
void Database::initialise (const std::string& iName, const bool iIsColdStart)
{
    ...
    if (iName.empty()){
        throw SqliteException("Database::initialise - detected empty database name");
    }
    
    dbName_ = iName;
    bool opened = openMemoryBasedDb() ||
                  openDiskBasedDb(iIsColdStart);

    if (opened == false){
        throw SqliteException(::boost::make_tuple("Database::initialise - invalid database name ",dbName_));
    } 
...
bool Database::openMemoryBasedDb ()
{
   ...
   if (dbName_ == ":memory:"){
       ...
       int errorCode = sqlite3_open(dbName_.c_str(),&database_);
       if (errorCode != SQLITE_OK){
           throw SqliteException(::boost::make_tuple("Failed to open SQLITE in memory database :",sqlite3_errmsg(database_)));
       }
       schema_ = ::SQL::Schema::singleton().getSchema();
       if(executeStatement(schema_) == false){
          throw SqliteException(::boost::make_tuple("Failed to create schema for in memory database :",sqlite3_errmsg(database_)));
       }
   }
   return isMemoryDb;
}
...
// *****************************************************************
// *****************************************************************
boost::shared_ptr< ::SQL::AbstractStatementFactory> Database::getStatementFormatter()
{
   return boost::shared_ptr< ::SQL::AbstractStatementFactory>(new SqliteAbstractStatementFactory);
}  
...
void Database::abortTransaction ()
{
   ...
   if (executeStatement(ABORT_TRANSACTION + SQL_TERMINATOR) == false){
       ...
       std::cout << "Database::abortTransaction failed to abort transaction : " << getCurrentError() << std::endl;
   } 
...
void Database::commitTransaction ()
{
   ...
   if (executeStatement(COMMIT_TRANSACTION + SQL_TERMINATOR) == false){
       throw SqliteException(::boost::make_tuple("Failed to commit transaction for database",dbName_,":",getCurrentError()));
   }  
}
...
void Database::startTransaction (const std::string& iName)
{
   ...
   if (executeStatement(START_TRANSACTION + SQL_TERMINATOR) == false){
       throw SqliteException(::boost::make_tuple("Failed to start transaction for database",dbName_,":",iName));
   }  
...
bool Database::executeQuery (const std::string& iQuery, ResultSet& oResult)
{
   ...
   sqlite3_stmt *ppStmt;
   int compile_result = sqlite3_prepare(database_,iQuery.c_str(),-1,&ppStmt,NULL);
	     ...
	     for(int columnIndex = 0; columnIndex < columnCount; ++columnIndex){
	         int columnType =   sqlite3_column_type(ppStmt,columnIndex);
                 switch(columnType){
                    case SQLITE_FLOAT :
                         {
                           double doubleEntry      = sqlite3_column_double(ppStmt,columnIndex);
                           std::string doubleValue = boost::lexical_cast<std::string>(doubleEntry);
                           currentRow[columnIndex] = doubleValue;
                         }
                         break;
                    case SQLITE_BLOB :
                      {
                        int32_t column2Bytes = sqlite3_column_bytes( ppStmt, columnIndex );
                        const char* column2Blob = reinterpret_cast<const char*>( sqlite3_column_blob( ppStmt, columnIndex ) );
                        currentRow[columnIndex] = std::string(column2Blob,column2Blob+column2Bytes);
                      }
                      break;

                    default :
                      const unsigned char* entry = sqlite3_column_text(ppStmt,columnIndex);
		      if (entry != NULL){
		          currentRow[columnIndex] = reinterpret_cast<const char*>(entry);
	              }
		      else{
...
bool Database::executeQuery (const std::string& iQuery, BlobData& blobData)
{
   ...
   bool queryResult = false;
   int32_t compile_result = sqlite3_prepare( getDatabaseImpl(), iQuery.c_str(), -1, &ppStmt, 0 );
   if (compile_result == SQLITE_OK){
       ...
       if (columnCount == 1){
           while ( sqlite3_step( ppStmt ) == SQLITE_ROW )
           {
              ...
              const char* column2Blob = reinterpret_cast<const char*>( sqlite3_column_blob( ppStmt, 0 ) );
...
bool Database::executeStatement (const std::string& iStatement)
{
   ...
   bool result = false;
   char *errMsg = 0;
   if (sqlite3_exec(database_,iStatement.c_str(),NULL,NULL,&errMsg) == SQLITE_OK){
      result = true;
   }
   else{
     // exec error
...
void Database::validateSchema(const ::SQL::Schema::TableDefinitionType& tableDefinitions)
{
    ...
    if (executeQuery(schemaQuery,masterSchemaResult) == false){
        throw SqliteException(std::string("Schema::validateSchema : failed to execute query on master table - ") + getCurrentError());
    } 

    if (masterSchemaResult.getColumns() != 2) {
        throw SqliteException(::boost::make_tuple("Schema::validateSchema : failed due to unexpected master schema column count",
                                                  "expected",2,
                                                  "found", masterSchemaResult.getColumns()));
    }
     
    for(::SQL::Schema::TableDefinitionType::const_iterator tableItr = tableDefinitions.begin(); tableItr != tableDefinitions.end(); ++tableItr){
        bool tableFound = false;
        for(ResultSet::RowType rowIndex = 0; rowIndex < masterSchemaResult.getRows(); ++rowIndex){
            const ResultSet::EntryContainerType& currentRow = masterSchemaResult.getRow(rowIndex);
            ...
            if (tableItr->first == tableName){
                ...
                if (normalisedSchema != normalisedApplication){
                    ...
                    message << "Schema::validateSchema : failed because database schema definition " << tableName;
                    message << " differs from the application schema\n";
                    message << " schema      table definition : '" << normalisedSchema      << "'\n";
                    message << " application table definition : '" << normalisedApplication << "'\n";
         ...
         if (!tableFound){
             std::ostringstream message;
             message << "Schema::validateSchema : failed because table " << tableItr->second;
...
std::string Database::normalise(const std::string& sql) const
{
   ...
   std::transform(normalisedSql.begin(),normalisedSql.end(),normalisedSql.begin(), toupper);
   ...
   std::copy(std::istream_iterator<std::string>(inputTokenStream),std::istream_iterator<std::string>(), std::back_inserter(tokens)); 
   ...
   std::copy(tokens.begin(),tokens.end(),std::ostream_iterator<std::string>(outputTokenStream, " "));
...
{
   if (database_ == 0){
      throw SqliteException("Database::getDatabaseImpl - failed due to null implementation");
   }
   return database_;
}
...
{
    const std::string dropTableStatements = ::SQL::Schema::singleton().dropSchema();
...
{
    if (database_ == 0){
        throw SqliteException("Database::check - failed due to null implementation");
    }
}
...
// *****************************************************************
// *****************************************************************
bool Database::finaliseCompile (const char* const location, sqlite3_stmt* ppStmt)
{
...
// *****************************************************************
// *****************************************************************
void Database::checkColumnCount(const char* const location, const int32_t actual, const int32_t expected, const std::string& query)
{
    if(expected != actual) {
       throw ::SQLITE::SqliteException(::boost::make_tuple(location,"- column count mismatch : expected",expected,"found",actual,query));
    }
} 

// *****************************************************************
// *****************************************************************
void Database::checkCompile(const char* const location, const int32_t compile_result, const std::string& query)
{
    if( compile_result != SQLITE_OK){
      throw ::SQLITE::SqliteException(::boost::make_tuple(location,"- failed to compile statement :",
