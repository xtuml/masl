...
namespace masld_BinaryIO
{
  ...
  {
    maslp_reader.setInputStream(boost::make_shared<std::istringstream>(std::string(maslp_bytes.begin(),maslp_bytes.end())));
  }
  ...
  {
    maslp_writer.setOutputStream(boost::make_shared<std::ostringstream>());
  }
  ...
  {
    boost::shared_ptr<std::ostringstream> input = maslp_writer.getOutputStream<std::ostringstream>();
