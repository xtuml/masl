...
namespace masld_Filesystem {
    ...
    const bool localServiceRegistration_masls_open_read =
        interceptor_masls_open_read::instance().registerLocal(&masls_open_read);
    const bool localServiceRegistration_masls_open_write =
        interceptor_masls_open_write::instance().registerLocal(&masls_open_write);
    const bool localServiceRegistration_masls_open_append =
        interceptor_masls_open_append::instance().registerLocal(&masls_open_append);
    const bool localServiceRegistration_masls_open_truncate =
        interceptor_masls_open_truncate::instance().registerLocal(&masls_open_truncate);
    const bool localServiceRegistration_masls_close = interceptor_masls_close::instance().registerLocal(&masls_close);
    const bool localServiceRegistration_masls_set_position =
        interceptor_masls_set_position::instance().registerLocal(&masls_set_position);
    const bool localServiceRegistration_masls_move_position =
        interceptor_masls_move_position::instance().registerLocal(&masls_move_position);
    const bool localServiceRegistration_masls_write_file =
        interceptor_masls_write_file::instance().registerLocal(&masls_write_file);
    const bool localServiceRegistration_masls_touch_file =
        interceptor_masls_touch_file::instance().registerLocal(&masls_touch_file);
    const bool localServiceRegistration_masls_move_file =
        interceptor_masls_move_file::instance().registerLocal(&masls_move_file);
    const bool localServiceRegistration_masls_copy_file =
        interceptor_masls_copy_file::instance().registerLocal(&masls_copy_file);
    const bool localServiceRegistration_masls_copy_file_preserve =
        interceptor_masls_copy_file_preserve::instance().registerLocal(&masls_copy_file_preserve);
    const bool localServiceRegistration_masls_append_file =
        interceptor_masls_append_file::instance().registerLocal(&masls_append_file);
    const bool localServiceRegistration_masls_delete_file =
        interceptor_masls_delete_file::instance().registerLocal(&masls_delete_file);
    const bool localServiceRegistration_masls_set_current_directory =
        interceptor_masls_set_current_directory::instance().registerLocal(&masls_set_current_directory);
    const bool localServiceRegistration_masls_create_directory =
        interceptor_masls_create_directory::instance().registerLocal(&masls_create_directory);
    const bool localServiceRegistration_masls_delete_directory =
        interceptor_masls_delete_directory::instance().registerLocal(&masls_delete_directory);
    const bool localServiceRegistration_masls_set_permissions =
        interceptor_masls_set_permissions::instance().registerLocal(&masls_set_permissions);
    const bool localServiceRegistration_masls_add_permissions =
        interceptor_masls_add_permissions::instance().registerLocal(&masls_add_permissions);
    const bool localServiceRegistration_masls_remove_permissions =
        interceptor_masls_remove_permissions::instance().registerLocal(&masls_remove_permissions);
    const bool localServiceRegistration_masls_change_owner =
        interceptor_masls_change_owner::instance().registerLocal(&masls_change_owner);
    const bool localServiceRegistration_masls_change_group =
        interceptor_masls_change_group::instance().registerLocal(&masls_change_group);
    const bool localServiceRegistration_masls_create_sym_link =
        interceptor_masls_create_sym_link::instance().registerLocal(&masls_create_sym_link);
    const bool localServiceRegistration_masls_create_hard_link =
        interceptor_masls_create_hard_link::instance().registerLocal(&masls_create_hard_link);
    const bool localServiceRegistration_masls_delete_tree =
        interceptor_masls_delete_tree::instance().registerLocal(&masls_delete_tree);
    const bool localServiceRegistration_masls_copy_tree =
        interceptor_masls_copy_tree::instance().registerLocal(&masls_copy_tree);
    const bool localServiceRegistration_masls_copy_tree_preserve =
        interceptor_masls_copy_tree_preserve::instance().registerLocal(&masls_copy_tree_preserve);
    const bool localServiceRegistration_masls_set_umask =
        interceptor_masls_set_umask::instance().registerLocal(&masls_set_umask);
    const bool localServiceRegistration_masls_truncate_file =
        interceptor_masls_truncate_file::instance().registerLocal(&masls_truncate_file);
    const bool localServiceRegistration_masls_unlock_file =
        interceptor_masls_unlock_file::instance().registerLocal(&masls_unlock_file);
    ...
    void copy_file(const std::string &source, const std::string &destination, bool preserve) {
        ...
        if (!dest.good() || src.bad())
            throw SWA::IOError(::std::strerror(errno));

        if (preserve) {
            struct stat status;
            if (stat(source.c_str(), &status))
                throw SWA::IOError(::std::strerror(errno));
            if (chmod(destination.c_str(), status.st_mode))
                throw SWA::IOError(::std::strerror(errno));
            if (chown(destination.c_str(), status.st_uid, status.st_gid))
                throw SWA::IOError(::std::strerror(errno));

            utimbuf timestamps = {status.st_atime, status.st_mtime};
            if (utime(destination.c_str(), &timestamps))
                throw SWA::IOError(::std::strerror(errno));
        }
    }

    void delete_file(const std::string &file_name) {
        if (unlink(file_name.c_str()) && (errno != ENOENT))
            throw SWA::IOError(::std::strerror(errno));
    }

    void delete_directory(const std::string &directory_name) {
        if (rmdir(directory_name.c_str()) && (errno != ENOENT))
            throw SWA::IOError(::std::strerror(errno));
    }
    ...
    void create_directory(const std::string &directory_name) {
        boost::tokenizer<boost::char_separator<char>> tokens(
            directory_name, boost::char_separator<char>("/", "", boost::keep_empty_tokens)
        );

        std::string path;
        for (boost::tokenizer<boost::char_separator<char>>::const_iterator it = tokens.begin(), end = tokens.end();
             it != end;
             ++it) {
            path += *it;
            if (!path.empty()) {
    ...
    void masls_open_read(const maslt_filename &maslp_file_name, maslt_file &maslp_dev) {
        try {
    ...
    void masls_open_write(const maslt_filename &maslp_file_name, maslt_file &maslp_dev) {
        try {
            if (masls_file_exists(maslp_file_name)) {
                maslp_dev.setInOutStream(
                    std::make_shared<std::fstream>(maslp_file_name.c_str(), std::ios::in | std::ios::out)
                );
            } else {
                maslp_dev.setInOutStream(std::make_shared<std::fstream>(
                    maslp_file_name.c_str(), std::ios::in | std::ios::out | std::ios::trunc
                ));
            }
        } catch (std::ios::failure &e) {
            throw SWA::IOError(strerror(errno));
    ...
    void masls_open_append(const maslt_filename &maslp_file_name, maslt_file &maslp_dev) {
        try {
            if (masls_file_exists(maslp_file_name)) {
                maslp_dev.setInOutStream(std::make_shared<std::fstream>(
                    maslp_file_name.c_str(), std::ios::in | std::ios::out | std::ios::ate
                ));
            } else {
                maslp_dev.setInOutStream(std::make_shared<std::fstream>(
                    maslp_file_name.c_str(), std::ios::in | std::ios::out | std::ios::trunc
    ...
    void masls_open_truncate(const maslt_filename &maslp_file_name, maslt_file &maslp_dev) {
        try {
            maslp_dev.setInOutStream(
                std::make_shared<std::fstream>(maslp_file_name.c_str(), std::ios::in | std::ios::out | std::ios::trunc)
            );
        } catch (std::ios::failure &e) {
            throw SWA::IOError(strerror(errno));
        }
    ...
    void masls_truncate_file(const maslt_filename &maslp_file_name, int32_t maslp_size) {
        struct stat status;
        if (stat(maslp_file_name.c_str(), &status))
            throw SWA::IOError(::std::strerror(errno));

        if (status.st_size > maslp_size) {
            if (truncate(maslp_file_name.c_str(), maslp_size))
                throw SWA::IOError(::std::strerror(errno));
            ;
        }
    }
    ...
    void masls_set_current_directory(const maslt_filename &maslp_directory_name) {
        if (chdir(maslp_directory_name.c_str()))
            throw SWA::IOError(::std::strerror(errno));
    }
    ...
    mode_t getMode(const maslt_permissions &permission) {
        return (permission.get_masla_sticky() ? S_ISVTX : 0) | (permission.get_masla_setuid() ? S_ISUID : 0) |
    ...
    void masls_add_permissions(const maslt_filename &maslp_file_name, const maslt_permissions &maslp_permissions) {
        struct stat status;
        if (stat(maslp_file_name.c_str(), &status))
            throw SWA::IOError(::std::strerror(errno));
        ...
        if (chmod(maslp_file_name.c_str(), status.st_mode))
            throw SWA::IOError(::std::strerror(errno));
    }

    void masls_set_permissions(const maslt_filename &maslp_file_name, const maslt_permissions &maslp_permissions) {
        mode_t permissions = getMode(maslp_permissions);

        if (chmod(maslp_file_name.c_str(), permissions))
            throw SWA::IOError(::std::strerror(errno));
    }

    void masls_remove_permissions(const maslt_filename &maslp_file_name, const maslt_permissions &maslp_permissions) {
        struct stat status;
        if (stat(maslp_file_name.c_str(), &status))
            throw SWA::IOError(::std::strerror(errno));
    ...
    void masls_change_owner(const maslt_filename &maslp_file_name, ::masld_Host::maslt_uid maslp_uid) {
        if (chown(maslp_file_name.c_str(), maslp_uid, gid_t(-1))) {
            throw SWA::IOError(::std::strerror(errno));
        }
    }

    void masls_change_group(const maslt_filename &maslp_file_name, ::masld_Host::maslt_gid maslp_gid) {
    ...
    template <typename S>
    auto get_access_time(const S &status) {
        if constexpr (requires { S::st_atim; }) {
            return SWA::Timestamp(status.st_atim);
        } else {
            return SWA::Timestamp(status.st_atimespec);
        }
    }
    template <typename S>
    auto get_modification_time(const S &status) {
        if constexpr (requires { S::st_mtim; }) {
            return SWA::Timestamp(status.st_mtim);
        } else {
            return SWA::Timestamp(status.st_mtimespec);
        }
    }
    template <typename S>
    auto get_status_change_time(const S &status) {
        if constexpr (requires { S::st_ctim; }) {
            return SWA::Timestamp(status.st_ctim);
        } else {
            return SWA::Timestamp(status.st_ctimespec);
        }
    }
    ...
    maslt_file_status getStatus(const struct stat &status) {
        ...
        if (S_ISREG(status.st_mode))
            result.set_masla_file_type() = maslt_file_types::masle_File;
        else if (S_ISDIR(status.st_mode))
            result.set_masla_file_type() = maslt_file_types::masle_Directory;
        else if (S_ISCHR(status.st_mode))
            result.set_masla_file_type() = maslt_file_types::masle_Character;
        else if (S_ISBLK(status.st_mode))
            result.set_masla_file_type() = maslt_file_types::masle_Block;
        else if (S_ISFIFO(status.st_mode))
            result.set_masla_file_type() = maslt_file_types::masle_FIFO;
        else if (S_ISLNK(status.st_mode))
            result.set_masla_file_type() = maslt_file_types::masle_Symlink;
        else if (S_ISSOCK(status.st_mode))
            result.set_masla_file_type() = maslt_file_types::masle_Socket;
        ...
        result.set_masla_access_time() = get_access_time(status);
        result.set_masla_modification_time() = get_modification_time(status);
        result.set_masla_status_change_time() = get_status_change_time(status);
    ...
    maslt_file_status masls_get_file_link_status(const maslt_filename &maslp_file_name) {
        struct stat status;
        if (lstat(maslp_file_name.c_str(), &status))
            throw SWA::IOError(::std::strerror(errno));
    ...
    maslt_filesystem_status masls_get_filesystem_status(const maslt_filename &maslp_file_name) {
        ...
        if (statvfs(maslp_file_name.c_str(), &status))
            throw SWA::IOError(::std::strerror(errno));
    ...
    maslt_filename masls_read_link(const maslt_filename &maslp_link_name) {
        ...
        if (size == -1)
            ...
            return maslt_filename(buf.begin(), buf.begin() + size);
    }
    ...
    void masls_create_sym_link(const maslt_filename &maslp_existing_name, const maslt_filename &maslp_new_name) {
        if (symlink(maslp_existing_name.c_str(), maslp_new_name.c_str())) {
            throw SWA::IOError(::std::strerror(errno));
        }
    }

    void masls_create_hard_link(const maslt_filename &maslp_existing_name, const maslt_filename &maslp_new_name) {
    ...
    bool masls_lock_file(
        const maslt_filename &maslp_file_name,
        ...
        if (!masls_file_exists(maslp_file_name))
            throw SWA::IOError("File does not exist");
