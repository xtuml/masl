...
class ObjectTranslator
{
  ...
  private final Map<ObjectService, ActionTranslator>                 serviceTranslators = new HashMap<ObjectService, ActionTranslator>();
  private final Map<State, ActionTranslator>                         stateTranslators   = new HashMap<State, ActionTranslator>();
  private final Map<EventDeclaration, EventTranslator>               eventTranslators   = new HashMap<EventDeclaration, EventTranslator>();
  ...
  ObjectTranslator ( final ObjectDeclaration object )
  {
    ...
    for ( final State state : object.getStates() )
    {
      ...
      constructor.getCode()
                 .appendExpression(registerStateHandler.asFunctionCall(id,
                                                                       actionPtrType
                                                                                    .callConstructor(new NewExpression(new TypeUsage(stateHandlerClass)))));
    ...
    for ( final EventDeclaration event : object.getAllEvents() )
    {
      ...
      constructor.getCode()
                 .appendExpression(registerEventHandler.asFunctionCall(id,
                                                                       eventPtrType
                                                                                   .callConstructor(new NewExpression(new TypeUsage(eventHandlerClass)))));
  ...
  private void addObjectWriter ()
  {
    ...
    final Expression instance = write.createParameter(new TypeUsage(mainClass, TypeUsage.ConstReference), "instance")
                                     .asExpression();

    write.getCode().appendStatement(new Function("write").asFunctionCall(objTrans.getGetId().asFunctionCall(instance, false))
                                                         .asStatement());
  ...
  {
    final Function creator = handlerClass.createMemberFunction(relationshipNavigators, "createInstance", Visibility.PUBLIC);
    creator.setConst(true);
    final Expression channel = creator.createParameter(new TypeUsage(Inspector.commChannel, TypeUsage.Reference), "channel")
                                      .asExpression();
    ...
    for ( final AttributeDeclaration att : object.getAttributes() )
    {
      if ( att.isIdentifier() || !att.isReferential() )
      {
        ...
        {
          final Variable var = new Variable(Types.getInstance().getType(att.getType()), Mangler.mangleName(att));
    ...
    final Variable instance = new Variable(objTrans.getPointerType(), "instance", objTrans.getCreateInstance()
                                                                                          .asFunctionCall(createParams));
    ...
    creator.getCode().appendStatement(new BinaryExpression(channel,
                                                           ...
                                                           objTrans.getGetId()
                                                                   .asFunctionCall(instance.asExpression(), true)).asStatement());
  ...
  private void addObjectPtrReader ()
  {
    final Function read = Inspector.bufferedInputStream.specialiseMemberFunction("read",
                                                                                 TemplateSpecialisation.create(objTrans
                                                                                                                       .getPointerType()));

    final Expression instance = read.createParameter(objTrans.getPointerType().getReferenceType(), "instance").asExpression();
    ...
    doIfNotValid.appendStatement(new BinaryExpression(instance, BinaryOperator.ASSIGN, Architecture.nullPointer).asStatement());
  ...
  {
    return new Function("write").asFunctionCall(Std.static_cast(new TypeUsage(FundamentalType.INT))
                                                   .asFunctionCall(objTrans
                                                                           .getNormalFsm()
                                                                           .getGetCurrentState()
                                                                           .asFunctionCall(instance, false))).asStatement();
  }
  ...
  private Statement writeAttribute ( final AttributeDeclaration att, final Expression instance )
  {
    ...
    if ( !att.isIdentifier() && att.isReferential() )
    {
      ...
      final Function navigator = objTrans.getRelationshipTranslator(refAtt.getRelationship())
                                         .getPublicAccessors()
                                         .getNavigateFunction();
      final Variable related = new Variable(navigator.getReturnType(), "related", navigator.asFunctionCall(instance, false));
      ...
      final Function attGetter = org.xtuml.masl.translate.main.object.ObjectTranslator.getInstance(destObj)
                                                                                     .getAttributeGetter(destAtt);

      final CodeBlock existsBlock = new CodeBlock();
      existsBlock.appendStatement(write.asFunctionCall(Literal.TRUE).asStatement());
      existsBlock.appendStatement(write.asFunctionCall(attGetter.asFunctionCall(related.asExpression(), true)).asStatement());
    ...
    else
    {
      ...
      {
        result = write.asFunctionCall(objTrans.getAttributeGetter(att).asFunctionCall(instance, false)).asStatement();
      }
    }
    return result;
  }
  ...
  private Statement writeObjectRel ( final RelationshipSpecification rel, final Expression instance )
  {
    ...
    {
      final Function navigator = objTrans.getRelationshipTranslator(rel).getPublicAccessors().getNavigateFunction();
  ...
  {
    final Function write = handlerClass.createMemberFunction(relationshipNavigators, "writeRelatedInstances", Visibility.PUBLIC);
    write.setConst(true);
    final Expression channel = write.createParameter(new TypeUsage(Inspector.commChannel, TypeUsage.Reference), "channel")
                                    .asExpression();
    ...
    for ( final RelationshipSpecification rel : object.getRelationships() )
    {
      final org.xtuml.masl.translate.main.object.ObjectTranslator destTranslator = org.xtuml.masl.translate.main.object.ObjectTranslator
                                                                                                                                      .getInstance(rel.getDestinationObject());
      final Expression destObjectHandler = Inspector.getObjectHandler(domainTranslator.getMainTranslator().getDomainId(),
                                                                      destTranslator
                                                                                    .getObjectId(),
                                                                      destTranslator.getMainClass());

      final Expression navigation = objTrans.getRelationshipTranslator(rel).getPublicAccessors().getNavigateFunction()
                                            .asFunctionCall(instance, true);

      final Expression nullResult = (rel.getCardinality() == MultiplicityType.MANY) ? Architecture.set(destTranslator.getPointerType())
                                                                                                  .callConstructor()
                                                                                   : destTranslator.createPointer();

      final Expression result = new ConditionalExpression(instance, navigation, nullResult);

      final Expression writeInstances = new Function("writeInstances").asFunctionCall(destObjectHandler, false, channel, result);
  ...
  {
    final Function getter = handlerClass.createMemberFunction(relationshipNavigators, "getIdentifierText", Visibility.PUBLIC);
    ...
    for ( final AttributeDeclaration att : object.getAttributes() )
    {
      ...
      {
        final Expression attText = Boost.lexicalCast(new TypeUsage(Std.string), objTrans.getAttributeGetter(att)
                                                                                        .asFunctionCall(instance,
                                                                                                        true));
        if ( text == null )
        {
          text = attText;
        ...
        {
          text = new BinaryExpression(new BinaryExpression(text, BinaryOperator.PLUS, Literal.createStringLiteral(",")),
                                      BinaryOperator.PLUS,
                                      attText);
        }
      }
