...
public class EventTranslator
{
  ...
  private void addProcessFunction ()
  {
    ...
    for ( final ParameterDefinition param : event.getParameters() )
    {
      ...
      final Variable parameter = processFunction.createParameter(type.getOptimalParameterType(), Mangler.mangleName(param));
  ...
  void addEventClass ()
  {
    ...
    if ( event.getType() == EventDeclaration.Type.NORMAL )
    {
      ...
      final Variable instance = new Variable(objectTranslator.getPointerType(),
                                             "instance",
                                             objectTranslator.getPopulation()
                                                             .getGetInstance().asFunctionCall(consumeParam.asExpression()));
      consumeFunction.getCode().appendStatement(instance.asStatement());
      consumeFunction.getCode()
                     .appendStatement(new IfStatement(instance.asExpression(),
                                                      new ExpressionStatement(new BinaryExpression(instance.asExpression(),
                                                                                                   BinaryOperator.PTR_REF,
                                                                                                   processFunction
                                                                                                                  .asFunctionCall(processArgs))),
                                                      null));
    }
    ...
    for ( final ParameterDefinition param : event.getParameters() )
    {
      ...
      final Variable memberVar = eventClass.createMemberVariable(group,
                                                                 Mangler.mangleName(param),
                                                                 type,
                                                                 Visibility.PRIVATE);

      final Function getter = eventClass.createMemberFunction(group, "get" + Mangler.mangleName(param), Visibility.PUBLIC);
      ...
      final Variable constructorParam = eventConstructor.createParameter(type.getOptimalParameterType(), Mangler
                                                                                                                .mangleName(param));

      eventConstructor.setInitialValue(memberVar, constructorParam.asExpression());
      eventConstructor.getCode()
                      .appendExpression(Architecture.event.getAddParam()
                                                          .asFunctionCall(memberVar.asMemberReference(eventClass.getThis()
                                                                                                                .asExpression(),
                                                                                                      true)));
  ...
  Function addCreateFunction ()
  {
    ...
    {
      final TypeUsage type = domainTranslator.getTypes().getType(param.getType());
      final Variable generateVar = createFunction.createParameter(type.getOptimalParameterType(), Mangler.mangleName(param));
      eventParams.add(generateVar.asExpression());
    }

    final Expression minusOne = new UnaryExpression(UnaryOperator.MINUS, Literal.ONE);
    final Expression sourceObj = createFunction.createParameter(new TypeUsage(FundamentalType.INT), "sourceObj", minusOne)
                                               .asExpression();
    final Expression sourceInst = createFunction.createParameter(new TypeUsage(Architecture.ID_TYPE),
                                                                 "sourceInstance",
                                                                 Literal.ZERO).asExpression();
    final Variable eventVar = new Variable(new TypeUsage(Architecture.event.getEventPtr()),
                                           "event",
                                           new Expression[]
                                             { new NewExpression(new TypeUsage(eventClass), eventParams) });

    createFunction.getCode().appendStatement(eventVar.asStatement());

    if ( event.getType() == EventDeclaration.Type.NORMAL )
    {
      final Expression setDest = Architecture.event.getSetDest().asFunctionCall(eventVar.asExpression(),
                                                                                true,
                                                                                objectTranslator.getMain().getArchitectureId());
      createFunction.getCode().appendExpression(setDest);
    }

    final Expression setSource = Architecture.event.getSetSource().asFunctionCall(eventVar.asExpression(),
                                                                                  true,
                                                                                  sourceObj,
                                                                                  sourceInst);
    ...
    createFunction.getCode().appendStatement(new IfStatement(new BinaryExpression(sourceObj,
                                                                                  ...
                                                                                  minusOne), setSource.asStatement()));
  ...
  boolean wrapEventParamsInTuple ( final EventDeclaration event )
  {
    return event.getParameters().size()
           + (event.getType() == EventDeclaration.Type.NORMAL ? objectTranslator.getMain().getGetInstance().getParameters().size()
