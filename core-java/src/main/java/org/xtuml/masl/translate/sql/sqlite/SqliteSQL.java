...
public class SqliteSQL {
    ...
    static void createMaxQuery(final String className,
        ...
        final Expression
                selectExpr =
                BinaryExpression.createCompoundExpression(BinaryOperator.PLUS,
                                                          Std.string.callConstructor(Literal.createStringLiteral(
                                                                  "SELECT max(")),
    ...
    static private void createSingleCellQuery(final String className,
        ...
        final Variable
                valueSelectVar =
                new Variable(new TypeUsage(Std.string), "valueSelect", new Expression[]{selectLiteral});
        ...
        final Variable
                databaseVar =
                new Variable(new TypeUsage(SqliteDatabase.getSqlitedatabaseclass(), TypeUsage.Reference),
                             "database",
                             singletonFnCall);
        ...
        final Expression
                executeQueryFnExpr =
                executeQueryFn.asFunctionCall(databaseVar.asExpression(),
                                              false,
                                              valueSelectVar.asExpression(),
                                              valueResultVar.asExpression());
        final BinaryExpression
                executeQueryCondition =
                new BinaryExpression(executeQueryFnExpr, BinaryOperator.EQUAL, new Literal("true"));

        final CodeBlock executeQueryTrueBranch = new CodeBlock();
        final CodeBlock executeQueryFalseBranch = new CodeBlock();
        final IfStatement
                ifExecuteQueryBlock =
                new IfStatement(executeQueryCondition, executeQueryTrueBranch, executeQueryFalseBranch);
        actualFn.getCode().appendStatement(ifExecuteQueryBlock);
        executeQueryFalseBranch.appendStatement(SqliteDatabase.throwDatabaseException(className +
                                                                                      "::executeGetRowCount - query failed"));
        ...
        final BinaryExpression
                columnCountCondition =
                new BinaryExpression(getColumnsFnCall, BinaryOperator.NOT_EQUAL, new Literal("1"));
        ...
        final BinaryExpression
                getRowsCondition =
                new BinaryExpression(getRowsFnCall, BinaryOperator.NOT_EQUAL, new Literal("1"));

        // Create cpp:
        // rowCountResult.getRows() != 1 && rowCountResult.getColumns() != 1
        final BinaryExpression
                queryResultCondition =
                new BinaryExpression(getRowsCondition, BinaryOperator.AND, columnCountCondition);

        final CodeBlock queryResultFailureBranch = new CodeBlock();
        queryResultFailureBranch.appendStatement(SqliteDatabase.throwDatabaseException(className +
                                                                                       "::executeGetRowCount - incorrect result contents"));
        ...
        final Variable
                cellValueVar =
                new Variable(new TypeUsage(Std.string, TypeUsage.ConstReference), "cellValue", atFnCall);
        ...
        final BinaryExpression
                nullCondition =
                new BinaryExpression(cellValueVar.asExpression(),
    ...
    static CodeBlock addDbQueryCodeBlock(final Variable sqlite3Stmt,
        ...
        final Expression
                getImplFnCall =
                new Function("getDatabaseImpl").asFunctionCall(databaseVar.asExpression(), false);
        final Expression[]
                getImplFnArgs =
                {getImplFnCall,
                 getCStr,
        ...
        final Variable
                compileResultVar =
                new Variable(new TypeUsage(Std.int32), "compile_result", compileResultAssignment);
        ...
        final Expression
                checkCompileFnCall =
                checkCompileFn.asFunctionCall(databaseVar.asExpression(),
                                              false,
                                              methodName,
        ...
        final Expression
                sqlite3StepWhileCondition =
                new BinaryExpression(sqlite3StepfnCall, BinaryOperator.EQUAL, new Literal("SQLITE_ROW"));
        ...
        final Variable
                monitorVar =
                new Variable(new TypeUsage(SqliteDatabase.sqlMonitorClass),
                             "queryMonitor",
                             new Expression[]{queryExpr});
    ...
    static Variable addReadBinaryColumn(final Variable sqlite3Stmt,
        ...
        if (attribute.getType().getDefinedType() instanceof EnumerateType) {
            ...
            final Variable
                    enumVar =
                    new Variable(attributeType, "column" + columnIndex, new Expression[]{readValueVar.asExpression()});
            ...
            switch (columnType) {
                ...
                case DURATION:
                    readValueVar =
                            addReadBinaryDurationColumn(sqlite3Stmt, codeBlock, attribute.getName(), columnIndex);
                    break;
                case TIMESTAMP:
                    readValueVar =
                            addReadBinaryTimestampColumn(sqlite3Stmt, codeBlock, attribute.getName(), columnIndex);
                    break;
                case TEXT:
                    readValueVar = addReadBinaryTextColumn(sqlite3Stmt, codeBlock, attribute.getName(), columnIndex);
                    break;
    ...
    static Variable addReadBinaryIntColumn(final Variable sqlite3Stmt,
        ...
        final Expression
                sqlite3FnCall =
                new Function("sqlite3_column_int").asFunctionCall(sqlite3Stmt.asExpression(), new Literal(columnIndex));
    ...
    static Variable addReadBinaryInt64Column(final Variable sqlite3Stmt,
        ...
        final Expression
                sqlite3FnCall =
                new Function("sqlite3_column_int64").asFunctionCall(sqlite3Stmt.asExpression(),
    ...
    static Variable addReadBinaryTimestampColumn(final Variable sqlite3Stmt,
        ...
        final Variable
                destValueVar =
                new Variable(new TypeUsage(Architecture.Timestamp.timestampClass),
                             destVarName,
    ...
    static Variable addReadBinaryDurationColumn(final Variable sqlite3Stmt,
        ...
        final Variable
                destValueVar =
                new Variable(new TypeUsage(Architecture.Duration.durationClass),
                             destVarName,
    ...
    static Variable addReadBinaryRealColumn(final Variable sqlite3Stmt,
        ...
        final Expression
                sqlite3FnCall =
                new Function("sqlite3_column_double").asFunctionCall(sqlite3Stmt.asExpression(),
                                                                     new Literal(columnIndex));
    ...
    static Variable addReadBinaryTextColumn(final Variable sqlite3Stmt,
        ...
        final Expression
                sqlite3FnCall =
                new Function("sqlite3_column_text").asFunctionCall(sqlite3Stmt.asExpression(),
                                                                   new Literal(columnIndex));
        final Variable
                destValueVar =
                new Variable(new TypeUsage(Std.string), destVarName, reinterpretCastFn.asFunctionCall(sqlite3FnCall));
        codeBlock.appendStatement(destValueVar.asStatement());
        return destValueVar;
    }
    ...
    static Variable addReadBinaryBlobColumn(final Variable sqlite3Stmt,
        ...
        final Expression
                bytesInColumnFnCall =
                new Function("sqlite3_column_bytes").asFunctionCall(sqlite3Stmt.asExpression(),
                                                                    new Literal(columnIndex));
        final Variable
                bytesInColumnVar =
                new Variable(new TypeUsage(Std.int32), "column" + columnIndex + "Bytes", bytesInColumnFnCall);
        ...
        final Expression
                blobColumnFnCall =
                reinterpretCastFn.asFunctionCall(new Function("sqlite3_column_blob").asFunctionCall(sqlite3Stmt.asExpression(),
                                                                                                    new Literal(
                                                                                                            columnIndex)));
        final Variable
                blobColumnVar =
                new Variable(new TypeUsage(FundamentalType.UCHAR, TypeUsage.PointerToConst),
        ...
        codeBlock.appendStatement(ASN1.BERDecode.asFunctionCall(decoder.asExpression(),
                                                                destValueVar.asExpression()).asStatement());
        return destValueVar;
    }
}
