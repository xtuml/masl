...
public class Structure
{
  ...
  public void translate ()
  {
    ...
    clazz.addSuperclass(Boost.lessThanComparable(new TypeUsage(clazz), Boost.equalityComparable(new TypeUsage(clazz))),
                        Visibility.PRIVATE);
  ...
  private void addASN1Encode ()
  {
    ...
    final Variable encoder = new Variable(new TypeUsage(ASN1.DEREncoder), "encoder", Arrays.<Expression>asList(ASN1.sequenceTag));
    ...
    encode.getCode().appendStatement(new Function("reserve").asFunctionCall(encoder.asExpression(),
                                                                            false,
                                                                            new Literal(structure.getElements().size()))
                                                            .asStatement());

    for ( final StructureElement att : structure.getElements() )
    {
      encode.getCode().appendStatement(ASN1.addChild(encoder.asExpression(), getGetter(att).asFunctionCall(value, false)));
    }
  ...
  private void addASN1Decode ()
  {
    ...
    decode.getCode().appendStatement(ASN1.checkHeader(decodeValue.decoder.asExpression(),
                                                      ASN1.sequenceTag,
                                                      true));
    ...
    {
      decode.getCode()
            .appendStatement(ASN1.checkChildPresent(decodeValue.decoder.asExpression(), decodeValue.childIt.asExpression()));
      decode.getCode()
            .appendStatement(ASN1.getChild(decodeValue.childIt.asExpression(), getSetter(att).asFunctionCall(value, false)));
    }
    decode.getCode()
          .appendStatement(ASN1.checkNoMoreChildren(decodeValue.decoder.asExpression(), decodeValue.childIt.asExpression()));
  ...
  Function addDefaultConstructor ()
  {
    ...
    for ( final StructureElement att : structure.getElements() )
    {
      ...
      {
        defaultConstructor.setInitialValue(member, ExpressionTranslator.createTranslator(att.getDefault(), null)
                                                                       .getReadExpression());
      }
    }
  ...
  Function addElementConstructor ()
  {
    ...
    for ( final StructureElement att : structure.getElements() )
    {
      ...
      elementConstructor.setInitialValue(member, elementConstructor.createParameter(type, attName).asExpression());
    }
  ...
  Function addTupleConstructor ()
  {
    ...
    final Variable aggregate = tupleConstructor.createParameter(new TypeUsage(tupleTypedef.asClass()).getOptimalParameterType(),
                                                                "aggregate");
  ...
  Function addStructureConstructor ()
  {
    ...
    final Expression rhs = structureConstructor.createParameter(new TypeUsage(tt).getOptimalParameterType(), "rhs").asExpression();
    structureConstructor.createParameter(new TypeUsage(tt.asClass().referenceNestedType("TupleType"), TypeUsage.PointerToConst),
                                         "dummy",
                                         Literal.ZERO);

    structureConstructor.getCode()
                        .appendStatement(new BinaryExpression(
                                                              new UnaryExpression(UnaryOperator.DEREFERENCE,
                                                                                  clazz.getThis()
                                                                                       .asExpression()),
                                                              BinaryOperator.ASSIGN,
                                                              new Function("toTuple").asFunctionCall(rhs, false)).asStatement());
  ...
  Function addGenericTupleConstructor ()
  {
    ...
    final Variable aggregate = tupleConstructor.createParameter(tuple.getTupleType().getOptimalParameterType(), "aggregate");
  ...
  Function addGenericTupleAssignment ()
  {
    ...
    final Variable aggregate = tupleAssignment.createParameter(tuple.getTupleType().getOptimalParameterType(), "aggregate");

    int i = 0;
    for ( final StructureElement element : structure.getElements() )
    {
      final Variable member = memberVariables.get(element);
      tupleAssignment.getCode()
                     .appendStatement(new BinaryExpression(member.asExpression(),
                                                           BinaryOperator.ASSIGN,
                                                           tuple
                                                                .getTupleGetter(aggregate.asExpression(), i++)).asStatement());
    }
  ...
  Function addTupleAssignment ()
  {
    ...
    final Variable aggregate = tupleAssignment.createParameter(new TypeUsage(tupleTypedef.asClass()).getOptimalParameterType(),
        "aggregate");
  ...
  private void addAttributes ()
  {
    for ( final StructureElement att : structure.getElements() )
    {
      ...
      memberVariables.put(att, clazz.createMemberVariable(members, Mangler.mangleName(att), type, Visibility.PRIVATE));
                                             ...
                                             final boolean reverse )
  {
    ...
    final Expression reverseEltCompare = new UnaryExpression(UnaryOperator.NOT, new BinaryExpression(rhs, comparator, lhs));

    return predicate == null ? eltCompare : new BinaryExpression(eltCompare,
                                                                 BinaryOperator.OR,
                                                                 new BinaryExpression(reverseEltCompare,
                                                                                      BinaryOperator.AND,
                                                                                      predicate));
