...
public abstract class ClassAttributeTranslator
{
  ...
  private static Map<ObjectDeclaration, ClassAttributeTranslator>    translators    = new HashMap<ObjectDeclaration, ClassAttributeTranslator>();
  ...
  {
    final Variable archId = getMainClass().createMemberVariable(attributes,
                                                                "architectureId",
                                 ...
                                 final Function setter )
  {
    ...
    final Variable attribute = getMainClass()
                                             .createMemberVariable(attributes,
  ...
  {
    final Variable currentState = getMainClass().createMemberVariable(attributes, CURRENT_STATE, stateType, Visibility.PRIVATE);

    final Variable currentStateParam = concreteObj.getConstructor().createParameter(stateType.getOptimalParameterType(),
                                                                                    CURRENT_STATE);
  ...
  {
    final Variable currentState = getFactoryClass().createMemberVariable(factoryAttributes,
                                                                         ASSIGNER_STATE,
                                                                         stateType,
                                                                         Visibility.PRIVATE);
  ...
  public void addUniqueId ( final AttributeDeclaration declaration, final Function getter, final Function user )
  {
    ...
    final Variable nextUniqueId = getFactoryClass().createMemberVariable(factoryAttributes,
    ...
    final Expression newId = new BinaryExpression(Std.max(usedId, nextUniqueId.asExpression()), BinaryOperator.PLUS, Literal.ONE);

    user.getCode().appendStatement(new BinaryExpression(nextUniqueId.asExpression(), BinaryOperator.ASSIGN, newId).asStatement());
