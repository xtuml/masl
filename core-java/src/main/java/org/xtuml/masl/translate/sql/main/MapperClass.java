    ...
    implements GeneratedClass
{
  ...
  public MapperClass ( final ObjectTranslator parent, final ObjectDeclaration declaration, final Namespace topLevelNamespace )
  {
    ...
    mapperClass = new Class(className,
                            namespace,
                            parent.getFrameworkTranslator().getLibrary().createPrivateHeader(objectTranslator.getDatabase().getDatabaseTraits().getName() + Mangler.mangleFile(objectDeclaration)
                                                      + "Mapper"));
  }
    ...
    for ( final AttributeDeclaration attributeDecl : objectDeclaration.getAttributes() )
    {
      if ( attributeDecl.isIdentifier() || !attributeDecl.isReferential() )
      {
        if ( attributeDecl.isUnique() )
        {
          ...
          final Function getMaxFn = mapperClass.createMemberFunction(uniqueIdentifers,
          ...
          final Expression getMaxFnCall = executeGetMaxColumnValueFn.asFunctionCall(sqlGenVar.asExpression(),
                                                                                    true,
  ...
  private void initialise ()
  {
    ...
    bodyFile = objectTranslator.getFrameworkTranslator().getLibrary().createBodyFile(objectTranslator.getDatabase().getDatabaseTraits().getName() + Mangler.mangleFile(objectDeclaration) + "Mapper");
    headerFile = objectTranslator.getFrameworkTranslator().getLibrary().createPrivateHeader(objectTranslator.getDatabase().getDatabaseTraits().getName() + Mangler.mangleFile(objectDeclaration) + "Mapper");
  ...
  private void addAttributes ()
  {
    ...
    primaryKeyCacheVar = mapperClass.createMemberVariable(attributes,
                                                          "primarykey_cache",
  ...
  private Function addConstructor ()
  {
    ...
    final Class sqlGenPtr = Boost.getSharedPtrType(new TypeUsage(objectTranslator.getDatabase()
                                                                                 .getObjectSqlGeneratorClass(getMainObjectTranslator().getMainClass(),
                                                                                                             objectTranslator.getClass("ImplementationClass"))));
    final FunctionCall sqlGenFunCall = sqlGenPtr.callConstructor(new NewExpression(new TypeUsage(sqlGenClass),
                                                                                   new ArrayList<Expression>()));
    final List<Expression> constructorArgs = Arrays.asList(new Expression[]
      { sqlGenFunCall });
  ...
  {
    final Function creator = mapperClass.redefineFunction(instanceCreation,
    ...
    // }
    final ThrowStatement throwStatement = new ThrowStatement(Architecture.programError.callConstructor(Literal.createStringLiteral("identifier already in use")));
    final Expression valueTypeConstructCall = ((Class)primaryKeyCacheVar.getType().getType()).referenceNestedType("value_type")
                                                                                             .callConstructor(idParamList);
    final Expression insertFnCall = new Function("insert").asFunctionCall(primaryKeyCacheVar.asExpression(),
                                                                          false,
    ...
    final Expression testIdFound = new org.xtuml.masl.cppgen.BinaryExpression(cachePairSecondRef,
    ...
    final Variable uniqueIdVar = new Variable(new TypeUsage(Architecture.ID_TYPE), "uniqueId", getNextArchIdFn.asFunctionCall());
    creator.getCode().appendStatement(uniqueIdVar.asStatement());

    // Create cpp line:
    // boost::shared_ptr<maslo_Find_Test_Object_A> instance(new
    ...
    final Variable instanceVar = new Variable(new TypeUsage(sharedPtr), "instance", new Expression[]
      { newObject });
  ...
  {
    final Function deleterFn = mapperClass.redefineFunction(instanceCreation,
    ...
    final Expression deleteInstFnCall = baseClass.callStaticFunction("deleteInstance", deleterFn.getParameters()
                                                                                                .get(0)
                                                                                                .asExpression());
    ...
    final Expression eraseFnCall = new Function("erase").asFunctionCall(primaryKeyCacheVar.asExpression(),
                                                                        false,
                                                                        getPrimaryKeyFnCall);
  ...
  {
    final Function postInitFn = mapperClass.createMemberFunction(instanceCreation, "doPostInit", Visibility.PROTECTED);
    ...
    final org.xtuml.masl.cppgen.Expression allLoadedTest = new org.xtuml.masl.cppgen.BinaryExpression(new Variable("allLoaded").asExpression(),
                                                                                                    org.xtuml.masl.cppgen.BinaryOperator.EQUAL,
                                                                                                    Literal.FALSE);
    ...
    final Variable objItrVar = new Variable(new TypeUsage(mapIterator),
                                            "objItr",
                                            new Function("begin").asFunctionCall(cacheVar.asExpression(), false));
    final org.xtuml.masl.cppgen.Expression forTest = new org.xtuml.masl.cppgen.BinaryExpression(objItrVar.asExpression(),
    ...
    final Expression getPrimaryKeyFnCall = new Function("getPrimaryKey").asFunctionCall(new Variable("second").asMemberReference(objItrVar.asExpression(),
                                                                                                                                 true),
                                                                                        true);
    final Expression insertFnCall = new Function("insert").asFunctionCall(primaryKeyCacheVar.asExpression(),
                                                                          false,
    ...
    final ForStatement cacheItrFor = new ForStatement(new VariableDefinitionStatement(objItrVar),
                                                      forTest,
                                                      incrementExpr,
                                                      forStatements);
  ...
  private void createFindFn ( final Population.FindFunction func, final Function predicateFn )
  {
    ...
    // Create cpp line:
    // ::boost::function<bool (maslo_Find_Test_Object_A*) > predicate =
    // ::boost::bind(
    ...
    final Expression inMemorySelectfnCall = selectFn.asFunctionCall(predicateVar.asExpression(), selectedObjectVar.asExpression());
    ...
    if ( containsStructure(func.predicate) )
    {
      ...
      final Variable predicateDebug = new Variable(new TypeUsage(Std.string).getConstType(),
                                                   "predicatDebug",
      ...
      final Expression cacheSelectFnCall = selectFromCacheFn.asFunctionCall(predicateDebug.asExpression(),
    ...
    else
    {
      ...
      {
        final ObjectTranslator dependentObjTrans = objectTranslator.getFrameworkTranslator().getObjectTranslator(dependentObj);
        if ( dependentObjTrans != objectTranslator )
        {
          ...
          final Expression forceFlushFnCall = new Function("forceFlush").asFunctionCall(getPopulationFnCall, false);
      ...
      {
        final RelationshipTranslator dependentRelTranslator = objectTranslator.getFrameworkTranslator()
                                                                              .getRelationshipTranslator(dependentRel);
      ...
      sqlOperation.appendStatement(new BlankLine(0));
      final Expression inSqlSelectfnCall = selectFn.asFunctionCall(criteria.getVariable().asExpression(),
                                                                   selectedObjectVar.asExpression());
  ...
  private DatabaseTraits.SqlCritera createCriteria ( final String variableName, final Population.FindFunction findFn )
  {
    ...
    final DatabaseTraits.SqlCritera sqlSelector = objectTranslator.getDatabase()
                                                                  .getDatabaseTraits()
                                                                  .createSqlCriteria(objectDeclaration, variableName);
    final org.xtuml.masl.metamodel.expression.Expression predicate = findFn.predicate;

    if ( containsStructure(predicate) == false )
    {
  ...
  private boolean containsStructure ( final org.xtuml.masl.metamodel.expression.Expression predicate )
  {
    ...
    {
      final BinaryExpression binExpression = (BinaryExpression)predicate;
                            ...
                            final org.xtuml.masl.metamodel.expression.Expression currentExpression )
  {
    ...
    {
      final BinaryExpression binExpression = (BinaryExpression)currentExpression;
      final org.xtuml.masl.metamodel.expression.Expression lhs = binExpression.getLhs();
      final org.xtuml.masl.metamodel.expression.Expression rhs = binExpression.getRhs();
    ...
    else if ( currentExpression instanceof UnaryExpression )
    {
      final UnaryExpression unaryExpression = (UnaryExpression)currentExpression;
      final UnaryExpression.Operator unaryOperator = unaryExpression.getOperator();
      if ( unaryExpression.getRhs() instanceof BinaryExpression )
      {
        final BinaryExpression binaryExpr = (BinaryExpression)unaryExpression.getRhs();
        criteria.beginCondition();
      ...
      {
        throw new RuntimeException(" addCritera SQL query cannot be formed for unaryExpression : " + unaryExpression);
      }
    }
    else if ( currentExpression instanceof FindParameterExpression )
    {
      final FindParameterExpression findParam = (FindParameterExpression)currentExpression;
      criteria.addParameterOperand(findParam.getType(), findParam.getName());
    }
    else if ( currentExpression instanceof FindAttributeNameExpression )
    {
      final FindAttributeNameExpression findAtt = (FindAttributeNameExpression)currentExpression;
    ...
    else if ( currentExpression instanceof SelectedComponentExpression )
    {
      ...
      throw new RuntimeException(" addCritera SQL query cannot be formed for structure types : " + currentExpression);
    }
  }
  ...
  private Class createFindFnPredicateClass ()
  {
    ...
    functionPtrParameters.add(new TypeUsage(objectTranslator.getClass(ImplementationClass.KEY_NAME), TypeUsage.Pointer));
