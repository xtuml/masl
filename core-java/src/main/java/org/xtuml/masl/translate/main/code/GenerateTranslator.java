...
public class GenerateTranslator extends CodeTranslator
{
  ...
  protected GenerateTranslator ( final GenerateStatement generate, final Scope parentScope, final CodeTranslator parentTranslator )
  {
    super(generate, parentScope, parentTranslator);

    final EventDeclaration event = generate.getEvent();

    final ObjectDeclaration toObj = generate.getToInstance() == null ? event.getParentObject()
                                                                    : ((InstanceType)generate.getToInstance()
                                                                                             .getType()).getObjectDeclaration();
    ...
    final List<Expression> args = new ArgumentTranslator(event.getParameters(), generate.getArguments(), getScope()).getArguments();

    if ( getScope().getParentObject() != null
         && ((getScope().getState() != null && getScope().getState().getType() == State.Type.NORMAL) || (getScope()
                                                                                                                   .getObjectService() != null && getScope().getObjectService()
                                                                                                                                                            .isInstance())) )
    {
    ...
    {
      final Expression toInstance = ExpressionTranslator.createTranslator(generate.getToInstance(), getScope()).getReadExpression();
    ...
    final Expression addEvent = new Function("addEvent").asFunctionCall(Architecture.eventQueue, false, generatedEvent);
