/*
 * Filename : TenaryRelationshipMapperSqlClass.java
 * 
 ...
 */
public class TenaryRelationshipMapperSqlClass
    implements GeneratedRelationshipClass
{
                                            ...
                                            final Namespace parentNamespace )
  {
    ...
    unlinkPreparedStatement = tableTranslator.createPreparedStatement(PreparedStatement.PreparedStatementType.DELETE);
  }
  ...
  protected void addDatabaseTable ()
  {
    ...
    final Expression createTableLiteral = Literal.createStringLiteral(createTableStatement.toString());
  ...
  {
    tableName = mapperSqlClass.createMemberVariable(attributeGroup, "tableName", new TypeUsage(Std.string), Visibility.PRIVATE);
    relationshipName = mapperSqlClass.createMemberVariable(attributeGroup,
                                                           "relationshipName",
                                                           new TypeUsage(Std.string),
                                                           Visibility.PRIVATE);

    pepreparedLink = mapperSqlClass.createMemberVariable(attributeGroup,
                                                         "pepreparedLink",
                                                         new TypeUsage(linkPreparedStatement.getClassType()),
                                                         Visibility.PRIVATE);
    pepreparedUnlink = mapperSqlClass.createMemberVariable(attributeGroup,
                                                           "pepreparedUnlink",
  ...
  protected void addConstructorMethod ()
  {
    ...
    constructor.setInitialValue(pepreparedLink, Literal.createStringLiteral("INSERT INTO " + getTableName() + " VALUES(:1,:2,:3);"));
    constructor.setInitialValue(pepreparedUnlink, Literal.createStringLiteral("DELETE FROM " + getTableName()
                                                                              + " WHERE "
                                                                              + getLeftColumnName()
                                                                              + " = :1 AND "
                                                                              + getRightColumnName()
  ...
  {
    final Function getTableNameFn = mapperSqlClass.createMemberFunction(getGroup, "getTableName", Visibility.PUBLIC);
    ...
    final Function getDomainNameFn = mapperSqlClass.createMemberFunction(getGroup, "getDomainName", Visibility.PUBLIC);
    getDomainNameFn.setReturnType(new TypeUsage(Std.string, TypeUsage.Const));
    getDomainNameFn.setConst(true);
    getDomainNameFn.getCode().appendStatement(new ReturnStatement(Literal.createStringLiteral(relationshipDecl.getDomain()
                                                                                                              .getName())));
    bodyFile.addFunctionDefinition(getDomainNameFn);

    final Function getLhsColumnName = mapperSqlClass.createMemberFunction(getGroup, "getLhsColumnName", Visibility.PUBLIC);
    getLhsColumnName.setReturnType(new TypeUsage(Std.string, TypeUsage.Const));
    getLhsColumnName.setConst(true);
    getLhsColumnName.getCode()
                    .appendStatement(new ReturnStatement(Literal.createStringLiteral(tableTranslator.getLeftColumnName())));
    bodyFile.addFunctionDefinition(getLhsColumnName);

    final Function getRhsColumnName = mapperSqlClass.createMemberFunction(getGroup, "getRhsColumnName", Visibility.PUBLIC);
    getRhsColumnName.setReturnType(new TypeUsage(Std.string, TypeUsage.Const));
    getRhsColumnName.setConst(true);
    getRhsColumnName.getCode()
                    .appendStatement(new ReturnStatement(Literal.createStringLiteral(tableTranslator.getRightColumnName())));
    bodyFile.addFunctionDefinition(getRhsColumnName);

    final Function getAssocColumnName = mapperSqlClass.createMemberFunction(getGroup, "getAssocColumnName", Visibility.PUBLIC);
    getAssocColumnName.setReturnType(new TypeUsage(Std.string, TypeUsage.Const));
    getAssocColumnName.setConst(true);
    getAssocColumnName.getCode()
                      .appendStatement(new ReturnStatement(Literal.createStringLiteral(tableTranslator.getAssocColumnName())));
    bodyFile.addFunctionDefinition(getAssocColumnName);

    final Function getRelationshipNameFn = mapperSqlClass.createMemberFunction(getGroup, "getRelationshipName", Visibility.PUBLIC);
  ...
  {
    final Function executeGetRowCountFn = mapperSqlClass.createMemberFunction(executeGroup, "executeGetRowCount", Visibility.PUBLIC);
  ...
  protected void addCommitMethods ()
  {
    ...
    final Variable linkObjectsVar = commitLinkFn.createParameter(new TypeUsage(mapperSqlClass.referenceNestedType("LinkedTenaryType"),
    ...
    final Expression getLinkFirstArhIdFnCall = getArhIdFn.asFunctionCall(getCheckedFn.asFunctionCall(new Function("get<0>").asFunctionCall(linkObjectsVar.asExpression(),
                                                                                                                                           false),
                                                                                                     false),
                                                                         true);
    final Expression getLinkSecondArhIdFnCall = getArhIdFn.asFunctionCall(getCheckedFn.asFunctionCall(new Function("get<1>").asFunctionCall(linkObjectsVar.asExpression(),
                                                                                                                                            false),
                                                                                                      false),
                                                                          true);
    final Expression getLinkThirdArhIdFnCall = getArhIdFn.asFunctionCall(getCheckedFn.asFunctionCall(new Function("get<2>").asFunctionCall(linkObjectsVar.asExpression(),
                                                                                                                                           false),
                                                                                                     false),
                                                                         true);
    final List<Expression> commitLinkTuple = BigTuple.getTupleList((Arrays.asList(new Expression[]
      { getLinkFirstArhIdFnCall, getLinkSecondArhIdFnCall, getLinkThirdArhIdFnCall })));
    final Expression LinkExecuteFnCall = executeFn.asFunctionCall(pepreparedLink.asExpression(), false, commitLinkTuple);
    commitLinkFn.getCode().appendExpression(LinkExecuteFnCall);
    bodyFile.addFunctionDefinition(commitLinkFn);

    final Function commitUnlinkFn = mapperSqlClass.createMemberFunction(commitGroup, "commitUnlink", Visibility.PUBLIC);
    final Variable unlinkObjectsVar = commitUnlinkFn.createParameter(new TypeUsage(mapperSqlClass.referenceNestedType("LinkedTenaryType"),
                                                                                   TypeUsage.ConstReference),
                                                                     "unlinkObjects");
    commitUnlinkFn.setConst(true);
    final Expression getunLinkFirstArhIdFnCall = getArhIdFn.asFunctionCall(getCheckedFn.asFunctionCall(new Function("get<0>").asFunctionCall(unlinkObjectsVar.asExpression(),
                                                                                                                                             false),
                                                                                                       false),
                                                                           true);
    final Expression getunLinkSecondArhIdFnCall = getArhIdFn.asFunctionCall(getCheckedFn.asFunctionCall(new Function("get<1>").asFunctionCall(unlinkObjectsVar.asExpression(),
                                                                                                                                              false),
                                                                                                        false),
                                                                            true);
    final Expression getunLinkThirdArhIdFnCall = getArhIdFn.asFunctionCall(getCheckedFn.asFunctionCall(new Function("get<2>").asFunctionCall(unlinkObjectsVar.asExpression(),
                                                                                                                                             false),
                                                                                                       false),
                                                                           true);
    final List<Expression> commitUnlinkTuple = BigTuple.getTupleList((Arrays.asList(new Expression[]
      { getunLinkFirstArhIdFnCall, getunLinkSecondArhIdFnCall, getunLinkThirdArhIdFnCall })));
    final Expression unLinkExecuteFnCall = executeFn.asFunctionCall(pepreparedUnlink.asExpression(), false, commitUnlinkTuple);
  ...
  private void addLoadAll ()
  {
    ...
    final Variable cachedTenaryContVar = loadAllFn.createParameter(new TypeUsage(mapperSqlClass.referenceNestedType("CachedTenaryContainerSet"),
                                                                                 TypeUsage.Reference),
                                                                   "cachedTenaryContainers");
  ...
  private void addLoadLhs ()
  {
    ...
    final Variable rhsIdentityVar = loadLhsFn.createParameter(new TypeUsage(Architecture.ID_TYPE, TypeUsage.ConstReference),
                                                              "rhsIdentity");
    final Variable cachedTenaryContVar = loadLhsFn.createParameter(new TypeUsage(mapperSqlClass.referenceNestedType("CachedTenaryContainerSet"),
                                                                                 TypeUsage.Reference),
                                                                   "cachedTenaryContainers");
  ...
  private void addLoadRhs ()
  {
    ...
    final Variable lhsIdentityVar = loadRhsFn.createParameter(new TypeUsage(Architecture.ID_TYPE, TypeUsage.ConstReference),
                                                              "lhsIdentity");
    final Variable cachedTenaryContVar = loadRhsFn.createParameter(new TypeUsage(mapperSqlClass.referenceNestedType("CachedTenaryContainerSet"),
                                                                                 TypeUsage.Reference),
                                                                   "cachedTenaryContainers");
  ...
  private void addLoadAss ()
  {
    ...
    final Variable assIdentityVar = loadAssFn.createParameter(new TypeUsage(Architecture.ID_TYPE, TypeUsage.ConstReference),
                                                              "assIdentity");
    final Variable cachedTenaryContVar = loadAssFn.createParameter(new TypeUsage(mapperSqlClass.referenceNestedType("CachedTenaryContainerSet"),
                                                                                 TypeUsage.Reference),
                                                                   "cachedTenaryContainers");
