...
public class ActionTranslator
{
  ...
  ActionTranslator ( final DomainServiceTranslator translator )
  {
    ...
    enteringAction = Stack.getEnteringDomainService(translator.getDomainTranslator().getDomainId(), translator.getServiceId());
  ...
  ActionTranslator ( final ObjectServiceTranslator translator )
  {
    codeTranslator = translator.getCodeTranslator();
    parameters = translator.getParameters();
    enteringAction = Stack.getEnteringObjectService(translator.getObjectTranslator().getDomainTranslator().getDomainId(),
                                                    translator.getObjectTranslator()
                                                              .getObjectId(),
  ...
  ActionTranslator ( final TerminatorServiceTranslator translator )
  {
    codeTranslator = translator.getCodeTranslator();
    parameters = translator.getParameters();
    enteringAction = Stack.getEnteringTerminatorService(translator.getTerminatorTranslator().getDomainTranslator().getDomainId(),
                                                        translator.getTerminatorTranslator()
                                                                  .getTerminatorId(),
                                                        translator.getServiceId());
    mainObjectTranslator = null;
    isInstance = false;
  ...
  ActionTranslator ( final StateActionTranslator translator )
  {
    codeTranslator = translator.getCodeTranslator();
    parameters = translator.getParameters();
    enteringAction = Stack.getEnteringState(translator.getObjectTranslator().getDomainTranslator().getDomainId(),
                                            translator.getObjectTranslator()
                                                      .getObjectId(),
                                            translator.getStateId());
    mainObjectTranslator = translator.getObjectTranslator();
    isInstance = translator.getState().getType() == State.Type.NORMAL || translator.getState().getType() == State.Type.TERMINAL;
  }
  ...
  ActionTranslator ( final ProjectTerminatorServiceTranslator translator )
  {
    ...
    final TerminatorServiceTranslator domainTSTranslator = TerminatorServiceTranslator.getInstance(translator.getService()
                                                                                                             .getDomainTerminatorService());
    enteringAction = Stack.getEnteringTerminatorService(domainTSTranslator.getTerminatorTranslator()
                                                                          .getDomainTranslator()
                                                                          .getDomainId(),
                                                        domainTSTranslator.getTerminatorTranslator()
                                                                          .getTerminatorId(),
  ...
  void addAtLines ( final CodeTranslator code )
  {
    ...
    if ( code instanceof CodeBlockTranslator )
    {
      final CodeBlockTranslator codeBlock = (CodeBlockTranslator)code;
    ...
    else if ( code instanceof ForTranslator )
    {
      final ForTranslator forStatement = (ForTranslator)code;
      forStatement.getLoopVarDef().appendStatement(Stack.defineVariable(new Literal(varId++), forStatement.getLoopVariable()));
    }
