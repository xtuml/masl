//
// File: ConcreteObjectTranslator.java
//
...
public abstract class ConcreteObjectTranslator
{
  ...
  void addFindFunctions ()
  {
    for ( final Population.FindFunction func : mainObjectTranslator.getPopulation().getFindFunctions() )
    {
      ...
      {
        final Function findFn = populationClass.redefineFunction(populationFinders, func.function, Visibility.PUBLIC);
  ...
  void addPopulation ()
  {
    ...
    final Function creator = populationClass.redefineFunction(populationCreators,
                                                              mainObjectTranslator.getPopulation()
                                                                                  .getCreateInstance(),
                                                              Visibility.PUBLIC);
    ...
    for ( final IdentifierDeclaration identifier : objectDeclaration.getIdentifiers() )
    {
      ...
      if ( needsCheck )
      {
        ...
        identifierTaken = identifierTaken == null ? thisCheck : new BinaryExpression(identifierTaken, BinaryOperator.OR, thisCheck);
      }
    }
    ...
    {
      creator.getCode()
             .appendStatement(new IfStatement(identifierTaken,
                                              new ThrowStatement(Architecture.programError.callConstructor(Literal
                                                                                                                  .createStringLiteral("identifier already in use")))));
    }

    final Variable instance = new Variable(creator.getReturnType(), "instance", new Expression[]
    ...
    getPopulation = populationClass.createStaticFunction(populationRegistration, "getPopulation", Visibility.PUBLIC);
    ...
    final Variable registered = populationClass.createStaticVariable(populationRegistration,
                                                                     "registered",
  ...
  private void addAttributes ()
  {
    for ( final AttributeDeclaration att : objectDeclaration.getAttributes() )
    {
      ...
      if ( att.isIdentifier() || !att.isReferential() )
      {
        ...
        final Function getter = mainClass.redefineFunction(getters, mainObjectTranslator.getAttributeGetter(att), Visibility.PUBLIC);
        final Function setter = att.isIdentifier() ? null
                                                  : mainClass.redefineFunction(setters,
                                                                               mainObjectTranslator
                                                                                                   .getAttributeSetter(att),
                                                                               Visibility.PUBLIC);

        final Variable constructorParam = constructor.createParameter(type.getOptimalParameterType(), Mangler.mangleName(att));

        addAttribute(att, getter, constructorParam, setter);

        if ( att.isUnique() )
        {
          final Function uniqueIdGetter = populationClass.redefineFunction(populationCreators,
                                                                           mainObjectTranslator.getPopulation()
                                                                                               .getGetUniqueId(att),
                                                                           Visibility.PUBLIC);

          final Function uniqueIdUser = populationClass.redefineFunction(populationCreators,
                                                                         mainObjectTranslator.getPopulation()
                                                                                             .getUseUniqueId(att),
                                                                         Visibility.PUBLIC);
  ...
  private void addCurrentState ()
  {
    if ( objectDeclaration.hasAssignerState() )
    {
      ...
      final Function getter = populationClass.redefineFunction(populationGetters,
                                                               mainObjectTranslator.getPopulation()
                                                                                   .getGetCurrentState(),
                                                               Visibility.PUBLIC);

      final Function setter = populationClass.redefineFunction(populationGetters,
                                                               mainObjectTranslator.getPopulation()
                                                                                   .getSetCurrentState(),
                                                               Visibility.PUBLIC);
    ...
    if ( objectDeclaration.hasCurrentState() )
    {
      ...
      final Function getter = mainClass.redefineFunction(getters,
                                                         mainObjectTranslator.getNormalFsm().getGetCurrentState(),
                                                         Visibility.PUBLIC);

      final Function setter = mainClass.redefineFunction(setters,
  ...
  private void addRelationships ()
  {
    ...
    {
      final RelationshipTranslator.SubclassOverrides relTrans = mainObjectTranslator.getRelationshipTranslator(rel)
                                                                                    .getSubclassOverrides();

      final Function navigator = mainClass.redefineFunction(relationshipNavs,
                                                            relTrans.getNavigateFunction(),
                                                            Visibility.PUBLIC);
      ...
      {
        final Function condNavigator = mainClass.redefineFunction(relationshipNavs,
                                                                  relTrans.getNavigateFunction(predicate),
                                                                  Visibility.PUBLIC);
        addRelationshipNav(rel, condNavigator, predicate);
      }

      final Function counter = mainClass.redefineFunction(relationshipCounts,
                                                          relTrans.getCountFunction(),
                                                          Visibility.PUBLIC);
      ...
      if ( mainLink != null && mainUnlink != null )
      {
        ...
        final Function unlinker = mainClass.redefineFunction(relationshipLinkers, mainUnlink, Visibility.PUBLIC);
