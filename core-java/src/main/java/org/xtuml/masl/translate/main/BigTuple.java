...
public final class BigTuple
{
   ...
   */
  public final static Expression getMakeTuple ( final List<Expression> params )
  {
   ...
   */
  public final Expression callConstructor ( final List<Expression> params )
  {
    final List<Expression> tupleparams = new ArrayList<Expression>();
    ...
    if ( params.size() > Boost.MAX_TUPLE_SIZE )
    {
      for ( int i = 0; i < params.size(); i += Boost.MAX_TUPLE_SIZE )
      {
        tupleparams.add(childTuples.get(i / Boost.MAX_TUPLE_SIZE).callConstructor(params.subList(i,
                                                                                                 Math.min(i + Boost.MAX_TUPLE_SIZE,
                                                                                                          params.size()))));
      }
      return getMakeTuple(tupleparams);
    }
  ...
  public final static List<Expression> getTupleList ( final List<Expression> params )
  {
   ...
   */
  public final Expression getTupleGetter ( final Expression lhs, final int index )
  {
    ...
    else
    {
      ...
      return childTuples.get((index / childTupleSize) % childTuples.size()).getTupleGetter(newLhs, index % childTupleSize);
    }
  }
