...
public class EnumerationTranslator
{
  ...
  {
    return new ThrowStatement(Architecture.programError.callConstructor(Literal.createStringLiteral("Enumerate out of RangeExpression")));
  }
  ...
  private void addASN1Encode ()
  {
    ...
    final Expression value = encode.createParameter(new TypeUsage(clazz, TypeUsage.ConstReference), "value").asExpression();

    encode.getCode()
          .appendStatement(new ReturnStatement(ASN1.DEREncode.asFunctionCall(getGetIndex().asFunctionCall(value, false))));
  ...
  private void addASN1Decode ()
  {
    ...
    final Expression value = decode.createParameter(new TypeUsage(clazz, TypeUsage.Reference), "value").asExpression();

    decode.getCode().appendStatement(ASN1.BERDecode.asFunctionCall(decodeValue.decoder.asExpression(),
                                                                   getSetIndex().asFunctionCall(value, false)).asStatement());
  ...
  private void addHashFunction ()
  {
    ...
    final Expression value = function.createParameter(new TypeUsage(clazz, TypeUsage.ConstReference), "value").asExpression();
  ...
  private void addDecrement ()
  {
    ...
    final Statement boundCheck = new IfStatement(new BinaryExpression(index.asExpression(), BinaryOperator.EQUAL, minIndex),
                                                 new ThrowStatement(Architecture.programError.callConstructor(Literal.createStringLiteral("Enumerate out of RangeExpression"))));

    function.getCode().appendStatement(boundCheck);
    function.getCode().appendExpression(new BinaryExpression(index.asExpression(),
                                                             BinaryOperator.ASSIGN,
                                                             indexEnum
                                                                      .callConstructor(new BinaryExpression(index.asExpression(),
                                                                                                            BinaryOperator.MINUS,
                                                                                                            Literal.ONE))));
    function.getCode().appendStatement(new ReturnStatement(new UnaryExpression(UnaryOperator.DEREFERENCE, clazz.getThis()
                                                                                                               .asExpression())));
  ...
  private void addDifference ()
  {
    ...
    function.declareInClass(true);
    function.getCode()
            .appendStatement(new ReturnStatement(new BinaryExpression(index.asExpression(),
                                                                      BinaryOperator.MINUS,
                                                                      index
                                                                           .asMemberReference(rhs.asExpression(), false))));
  ...
  private void addIncrement ()
  {
    ...
    function.setReturnType(new TypeUsage(clazz, TypeUsage.Reference));
    final Statement boundCheck = new IfStatement(new BinaryExpression(index.asExpression(), BinaryOperator.EQUAL, maxIndex),
                                                 getRangeError());
  ...
  private void addIndexLookup ()
  {
    ...
    {
      final EnumerationType.Enumerator indexEnumItem = indexEnum.addEnumerator("index_" + Mangler.mangleName(maslEnumItem), null);
      indexEnumerators.add(indexEnumItem);

      final Variable enumeratorVar = clazz.createStaticVariable(enumConstants,
  ...
  private void addBeginEnd ()
  {
    ...
    final TypedefType constIteratorType = new TypedefType("const_iterator",
                                                          new TypeUsage(Architecture.rangeIterator(new TypeUsage(clazz))));
    ...
    beginEnumFn.getCode().appendStatement(new ReturnStatement(constIteratorType.callConstructor(beginEnum, endEnum)));
  ...
  private void addLessThan ()
  {
    ...
    function.getCode()
            .appendStatement(new ReturnStatement(new BinaryExpression(index.asExpression(),
                                                                      ...
                                                                      index.asMemberReference(rhs.asExpression(), false))));
  ...
  private void addOperators ()
  {
    ...
    clazz.addSuperclass(Boost.lessThanComparable(new TypeUsage(clazz),
                                                 Boost.equalityComparable(new TypeUsage(clazz),
                                                                          Boost
                                                                               .incrementable(new TypeUsage(clazz),
                                                                                              Boost.decrementable(new TypeUsage(clazz))))),
                        Visibility.PRIVATE);
  ...
  private Function addOStreamOperator ( final Namespace namespace )
  {
    ...
    final Expression stream = streamOperator.createParameter(new TypeUsage(Std.ostream, TypeUsage.Reference), "stream")
                                            .asExpression();
    final Expression obj = streamOperator.createParameter(new TypeUsage(clazz, TypeUsage.ConstReference), "obj").asExpression();

    final Expression result = new BinaryExpression(stream, BinaryOperator.LEFT_SHIFT, getText.asFunctionCall(obj, false));
  ...
  private Function addIStreamOperator ( final Namespace namespace )
  {
    ...
    final Expression stream = streamOperator.createParameter(new TypeUsage(Std.istream, TypeUsage.Reference), "stream")
                                            .asExpression();
    final Expression obj = streamOperator.createParameter(new TypeUsage(clazz, TypeUsage.Reference), "obj").asExpression();
    ...
    final Expression assignment = new BinaryExpression(obj, BinaryOperator.ASSIGN, clazz.callConstructor(text.asExpression()));
  ...
  private void addTextLookup ()
  {
    ...
    textLookupVar = clazz.createStaticVariable(textConversion,
                                               "textLookup",
                                               new TypeUsage(FundamentalType.CHAR,
                                                             TypeUsage.ConstPointerToConst),
                                               new AggregateInitialiser(literals),
                                               Visibility.PRIVATE);
    textLookupVar.setArray();
    bodyFile.addVariableDefinition(textLookupVar);
    getText.getCode().appendStatement(new ReturnStatement(new ArrayAccess(textLookupVar.asExpression(), index.asExpression())));
  ...
  private void addValueLookup ()
  {
    ...
    for ( final EnumerateItem maslEnum : maslEnumerates )
    {
      ...
      final EnumerationType.Enumerator valueEnumItem = valueEnum.addEnumerator("value_" + Mangler.mangleName(maslEnum),
                                                                               specifiedValue);
    ...
    valueLookupVar = clazz.createStaticVariable(valueConversion,
                                                "valueLookup",
    ...
    getValue.getCode().appendStatement(new ReturnStatement(new ArrayAccess(valueLookupVar.asExpression(), index.asExpression())));
    ...
    intConstructor.setInitialValue(index, fromValue.asFunctionCall(valueEnum.callConstructor(intValue.asExpression())));
    ...
    cast.setConst(true);
    cast.getCode().appendStatement(new ReturnStatement(Std.static_cast(new TypeUsage(Std.int32))
                                                          .asFunctionCall(getValue.asFunctionCall())));

  }

  private Function createFromText ()
  {
    final TypedefType lookupTableType = new TypedefType("TextLookupTable", new TypeUsage(Std.map(new TypeUsage(Std.string),
                                                                                                 new TypeUsage(indexEnum))));
    clazz.addTypedef(textConversion, lookupTableType, Visibility.PRIVATE);

    final Function getLookupTable = clazz.createStaticFunction(textConversion, "getLookupTable", Visibility.PRIVATE);
    ...
    {
      final Expression index = new ArrayAccess(textLookupVar.asExpression(), indexEnumerators.get(i).asExpression());
      final Expression mapEntry = lookupTableType.referenceNestedType("value_type")
                                                 .callConstructor(index,
                                                                  indexEnumerators.get(i).asExpression());

      getLookupTable.getCode().appendExpression(new Function("insert").asFunctionCall(lookup.asExpression(), false, mapEntry));
    }
    ...
    final Variable lookupStatic = new Variable(new TypeUsage(lookupTableType), "lookup", getLookupTable.asFunctionCall());
    lookupStatic.setStatic(true);

    fromText.getCode().appendStatement(new VariableDefinitionStatement(lookupStatic));

    final Variable pos = new Variable(new TypeUsage(lookupTableType.referenceNestedType("const_iterator")),
                                      "pos",
                                      new Function("find")
                                                          .asFunctionCall(lookupStatic.asExpression(), false, text.asExpression()));
    fromText.getCode().appendStatement(new VariableDefinitionStatement(pos));

    fromText.getCode()
            .appendStatement(new IfStatement(new BinaryExpression(pos.asExpression(),
                                                                  BinaryOperator.EQUAL,
                                                                  new Function("end").asFunctionCall(lookupStatic.asExpression(),
                                                                                                     false)), getRangeError()));

    fromText.getCode().appendStatement(new ReturnStatement(new Variable("second").asMemberReference(pos.asExpression(), true)));
