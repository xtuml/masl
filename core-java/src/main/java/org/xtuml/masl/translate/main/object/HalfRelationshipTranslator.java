//
// File: HalfRelationshipObject.java
//
...
public abstract class HalfRelationshipTranslator
{
  ...
  private static Map<ObjectDeclaration, HalfRelationshipTranslator> translators = new HashMap<ObjectDeclaration, HalfRelationshipTranslator>();
  ...
  public void addRelationship ( final RelationshipSpecification spec, final RelationshipSpecification assocSpec )
  {
    ...
    final String relName = spec.getRelationship().getName()
                           + "_"
                           + (spec.getRole() == null ? "" : spec.getRole() + "_")
                           + spec.getDestinationObject().getName();

    final Variable relAtt = getMainClass().createMemberVariable(relationshipVars,
                                                                relName,
  ...
  Map<RelationshipSpecification, Variable> relationshipAttributes = new HashMap<RelationshipSpecification, Variable>();
  ...
  public void addRelationshipCorr ( final RelationshipSpecification spec, final Function correlator )
  {
    ...
    {
      final AssociativeRelationshipDeclaration assocRel = (AssociativeRelationshipDeclaration)relDec;
      ...
      if ( assocRel.getAssocObject() != mainObjectTranslator.getObjectDeclaration() )
      {
        ...
        final Statement code = new ReturnStatement(navFunction.asFunctionCall(relGetter.asFunctionCall(),
                                                                              false,
                                                                              downcast
                                                                                      .asFunctionCall(rhs.asExpression(), false)));
  ...
  public void addRelationshipLink ( final RelationshipSpecification spec, final Function linker, final boolean unlink )
  {
    ...
    final Variable rhs2 = new Variable(new TypeUsage(Architecture.objectPtr(new TypeUsage(rhsTrans.getMainClass()))),
                                       "rhs2",
                                       downcast
                                               .asFunctionCall(rhs.asExpression(), false));
    ...
    final FunctionCall lhs = Architecture.objectPtr(new TypeUsage(getMainClass())).callConstructor(getMainClass().getThis()
                                                                                                                 .asExpression());
    ...
    if ( !(relDec instanceof AssociativeRelationshipDeclaration) )
    {
      ...
      if ( !unlink && relDec instanceof SubtypeRelationshipDeclaration )
      {
        final SubtypeRelationshipDeclaration supRelDec = (SubtypeRelationshipDeclaration)relDec;

        final Expression supObj = supRelDec.getSupertype() == spec.getDestinationObject() ? rhs2.asExpression()
                                                                                         : getMainClass()
                                                                                                         .getThis().asExpression();
        ...
        for ( final ObjectDeclaration subtype : supRelDec.getSubtypes() )
        {
          ...
          final Expression linkCount = new Function("count").asFunctionCall(supTrans.getRelationshipConstGetter(subSpec)
                                                                                    .asFunctionCall(supObj, true), false);
          isLinked = isLinked == null ? linkCount : new BinaryExpression(isLinked, BinaryOperator.OR, linkCount);
        }
        final ThrowStatement throwStatement = new ThrowStatement(Architecture.programError.callConstructor(Literal
                                                                                                                  .createStringLiteral("Cannot link instance - relationship " + relDec.getName()
                                                                                                                                       + " already linked")));
      ...
      linker.getCode().appendStatement(new TryCatchBlock(tryBlock, new TryCatchBlock.CatchBlock(null, undoBlock)));

    }
    else
    {
      final HalfRelationshipTranslator assocTrans = getTranslator(((AssociativeRelationshipDeclaration)relDec).getAssocObject());

      final Function assocForwardGetter = assocTrans.getRelationshipGetter(reverseSpec.getAssocSpec().getReverseSpec());
      ...
      final Variable assoc2 = new Variable(new TypeUsage(Architecture.objectPtr(new TypeUsage(assocTrans.getMainClass()))),
                                           "assoc2",
      ...
      final Expression forwardLink = linkFn.asFunctionCall(forwardRel, false, rhs2.asExpression(), assoc2.asExpression());
      ...
      final Expression forwardUndo = undoFn.asFunctionCall(forwardRel, false, rhs2.asExpression(), assoc2.asExpression());
                                   ...
                                   final org.xtuml.masl.metamodel.expression.Expression predicate )
  {
    ...
    final Statement code = new ReturnStatement(navFunction.asFunctionCall(relGetter.asFunctionCall(), false, predicateArg));
    navigator.getCode().appendStatement(code);
  }
  ...
  private final Map<RelationshipSpecification, Function>             relationshipConstGetters = new HashMap<RelationshipSpecification, Function>();

  private final Map<RelationshipSpecification, Function>             relationshipGetters      = new HashMap<RelationshipSpecification, Function>();
