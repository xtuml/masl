...
public class ForTranslator extends CodeTranslator
{
                            ...
                            final CodeTranslator parentTranslator )
  {
    ...
    if ( loopSpec instanceof FromToRange )
    {
      final FromToRange fromTo = (FromToRange)loopSpec;

      final Expression from = ExpressionTranslator.createTranslator(fromTo.getFrom(), getScope()).getReadExpression();
    ...
    else if ( loopSpec instanceof TypeRange )
    {
      ...
      final Expression from = ExpressionTranslator.createTranslator(rangeType.getMinValue(), getScope()).getReadExpression();
      final Expression to = ExpressionTranslator.createTranslator(rangeType.getMaxValue(), getScope()).getReadExpression();
      ...
      loopVar = new Variable(type.getConstReferenceType(),
    ...
    else if ( loopSpec instanceof VariableElements )
    {
      final VariableElements elements = (VariableElements)loopSpec;
      ...
      final Variable collectionVar = new Variable(new TypeUsage(varType),
                                                  "collection",
                                                  ExpressionTranslator.createTranslator(maslVariable, getScope())
                                                                      .getReadExpression());
      ...
      endCondition = new BinaryExpression(it.asExpression(), BinaryOperator.NOT_EQUAL, new Function(end).asFunctionCall(collection,
                                                                                                                        false));
    ...
    else if ( loopSpec instanceof VariableRange )
    {
      final VariableRange range = (VariableRange)loopSpec;
      ...
      final Class varType = (Class)Types.getInstance().getType(maslVariable.getType().getBasicType()).getType();
      final Variable collectionVar = new Variable(new TypeUsage(varType, TypeUsage.ConstReference),
                                                  "collection",
                                                  ExpressionTranslator
                                                                      .createTranslator(maslVariable, getScope())
                                                                      .getReadExpression());
