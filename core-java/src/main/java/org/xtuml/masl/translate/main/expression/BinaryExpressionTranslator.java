//
// File: BinaryExpressionTranslator.java
//
...
public class BinaryExpressionTranslator extends ExpressionTranslator
{
                               ...
                               final org.xtuml.masl.metamodel.expression.Expression assignedTo )
  {
    ...
    switch ( maslExpression.getOperator() )
    {
      ...
      case CONCATENATE:
      {
        ...
        if ( maslExpression.getLhs().getType().getBasicType() instanceof CollectionType )
        {
          final CollectionType collection = (CollectionType)maslExpression.getLhs().getType().getBasicType();
          rhsExp = ExpressionTranslator.createTranslator(maslExpression.getRhs(), scope, collection.getContainedType())
                                       .getReadExpression();
        }
        ...
        if ( maslExpression.getLhs() instanceof org.xtuml.masl.metamodel.expression.BinaryExpression
               && ((org.xtuml.masl.metamodel.expression.BinaryExpression)maslExpression.getLhs()).getOperator() == org.xtuml.masl.metamodel.expression.BinaryExpression.Operator.CONCATENATE
      ...
      case UNION:
        setReadExpression(new Function("set_union").asFunctionCall(promoteToUnorderedCollection(maslExpression.getLhs(), lhsExp),
                                                                   false,
                                                                   promoteToUnorderedCollection(maslExpression.getRhs(), rhsExp)));
        break;
      case DISUNION:
        setReadExpression(new Function("set_disunion")
                                                      .asFunctionCall(promoteToUnorderedCollection(maslExpression.getLhs(), lhsExp),
                                                                      false,
                                                                      promoteToUnorderedCollection(maslExpression.getRhs(), rhsExp)));
        break;
      case INTERSECTION:
        setReadExpression(new Function("set_intersection").asFunctionCall(promoteToUnorderedCollection(maslExpression.getLhs(),
                                                                                                       lhsExp),
                                                                          false,
        ...
        break;
      case NOT_IN:
        setReadExpression(new Function("set_not_in").asFunctionCall(promoteToUnorderedCollection(maslExpression.getLhs(), lhsExp),
                                                                    false,
                                                                    promoteToUnorderedCollection(maslExpression.getRhs(), rhsExp)));
        break;
      default:
                                                    ...
                                                    final Expression exp )
  {
    ...
    {
      final TypeUsage lhsType = Types.getInstance().getType(((SequenceType)maslExp.getType().getBasicType()).getContainedType());
