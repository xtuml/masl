...
public class DomainServiceTranslator
{
  ...
  public DomainServiceTranslator ( final DomainService service, final Expression serviceId )
  {
    ...
    if ( !service.isFunction() && !service.isExternal() && !service.isScenario() )
    {
      ...
      interceptorType = new TypedefType(interceptorName, getNamespace(), new TypeUsage(interceptorClass), headerFile);
                                        ...
                                        final Namespace registrationNamespace )
  {
    ...
    if ( interceptorType != null )
    {
      ...
      final Expression initialValue = registerFunction.asFunctionCall(interceptorFnCall,
                                                                                   false,
                                                                                   forwardFunction.asFunctionPointer());
      registrationVar = new Variable(new TypeUsage(FundamentalType.BOOL, TypeUsage.Const),
  ...
  private final Map<ParameterDefinition, ParameterTranslator> parameters = new LinkedHashMap<ParameterDefinition, ParameterTranslator>();
