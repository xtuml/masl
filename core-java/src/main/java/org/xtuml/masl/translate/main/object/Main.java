...
public class Main
{
  ...
  public Main ( final ObjectTranslator translator )
  {
    ...
    // null);
    headerFile = domainTranslator.getLibrary().createPrivateHeader(Mangler.mangleFile(translator.getObjectDeclaration()));
  ...
  {
    final String name = "Event_"
                        + Mangler.mangleName(translator.getObjectDeclaration())
                        + "_"
                        + (event.getParentObject() != translator.getObjectDeclaration() ? Mangler.mangleName(event.getParentObject())
                                                                                          + "_"
                                                                                       : "")
    ...
    final Function getDomainId = eventClass.redefineFunction(group, Architecture.event.getGetDomainId(), Visibility.PUBLIC);
    ...
    final Function getObjectId = eventClass.redefineFunction(group, Architecture.event.getGetObjectId(), Visibility.PUBLIC);
    ...
    final Function getEventId = eventClass.redefineFunction(group, Architecture.event.getGetEventId(), Visibility.PUBLIC);
  ...
  {
    final Function consumeFunction = mainClass.createStaticFunction(consumeEvent, "consume_"
                                                                                  + Mangler.mangleName(event.getParentObject())
                                                                                  + "_"
                                                                                  + Mangler.mangleName(event), Visibility.PUBLIC);
  ...
  public Function addCreateEventFunction ( final EventDeclaration event )
  {
    ...
    {
      generateFunction = mainClass.createMemberFunction(generateEvent, "create_"
                                                                       + Mangler.mangleName(event.getParentObject())
                                                                       + "_"
                                                                       + Mangler.mangleName(event), Visibility.PUBLIC);
    }
  ...
  public Function addGenerateDelayedFunction ( final EventDeclaration event )
  {
    ...
    {
      generateDelayedFunction = mainClass
                                         .createMemberFunction(delayedGenerateEvent, "generate_delayed_"
                                                                                     + Mangler.mangleName(event.getParentObject())
                                                                                     + "_"
                                                                                     + Mangler.mangleName(event), Visibility.PUBLIC);
  ...
  public Function addCreateDelayedFunction ( final EventDeclaration event )
  {
    ...
    createDelayedFunction = mainClass.createMemberFunction(delayedCreateEvent, "create_delayed_"
                                                                               + Mangler.mangleName(event.getParentObject())
                                                                               + "_"
                                                                               + Mangler.mangleName(event), Visibility.PUBLIC);
  ...
  public Function createProcessFunction ( final EventDeclaration event )
  {
    ...
    if ( event.getType() == EventDeclaration.Type.NORMAL )
    {
      processFunction = mainClass.createMemberFunction(processEvent, "process_"
                                                                     + Mangler.mangleName(event.getParentObject())
                                                                     + "_"
                                                                     + Mangler.mangleName(event), Visibility.PUBLIC);
    }
    ...
    {
      processFunction = mainClass.createStaticFunction(processEvent, "process_"
                                                                     + Mangler.mangleName(event.getParentObject())
                                                                     + "_"
                                                                     + Mangler.mangleName(event), Visibility.PRIVATE);
  ...
  {
    final Function getArchitectureId = mainClass.createMemberFunction(getters, "getArchitectureId", Visibility.PUBLIC);
  ...
  Function addAttributeGetter ( final AttributeDeclaration att )
  {
    ...
    final Function getter = mainClass.createMemberFunction(getters, "get_" + Mangler.mangleName(att), Visibility.PUBLIC);
    getter.setPure(true);
  ...
  void addReferentialAttributeGetter ( final AttributeDeclaration att, final Function getter )
  {
    ...
    {
      final Expression relatedObj = translator.getRelationshipTranslator(refAtt.getRelationship()).getPublicAccessors()
                                              .getNavigateFunction().asFunctionCall();
  ...
  Function addAttributeSetter ( final AttributeDeclaration att )
  {
    ...
    final Function setter = mainClass.createMemberFunction(setters, "set_" + Mangler.mangleName(att), Visibility.PUBLIC);
    setter.setPure(true);
  ...
  {
    final Variable isDeletedFlag = mainClass.createMemberVariable(creators,
                                                                  "isDeletedFlag",
    ...
    if ( domainTranslator.addRefIntegChecks() || CommandLine.INSTANCE.isForTest() )
    {
      ...
      {
        final Expression count = translator.getRelationshipTranslator(rel).getPublicAccessors().getCountFunction().asFunctionCall();
        final ThrowStatement throwStatement = new ThrowStatement(Architecture.programError.callConstructor(Literal
                                                                                                                  .createStringLiteral("Cannot delete instance - relationship " + rel.getRelationship()
                                                                                                                                                                                     .getName()
                                                                                                                                       + " still linked")));
    ...
    deleteInstance.getCode().appendStatement(popDelete.asFunctionCall(population.getSingleton(),
                                                                      false,
                                                                      translator.createPointer(mainClass.getThis().asExpression()))
                                                      .asStatement());
    for ( final AttributeDeclaration att : translator.getObjectDeclaration().getAttributes() )
    {
      if ( att.getType().getBasicType().getActualType() == ActualType.TIMER )
      {
        deleteInstance.getCode()
                      .appendStatement(Architecture.Timer.deleteTimer(translator.getAttributeGetter(att).asFunctionCall())
                                                         .asStatement());
      }
    }

    deleteInstance.getCode()
                  .appendStatement(new BinaryExpression(isDeletedFlag.asExpression(), BinaryOperator.ASSIGN, Literal.TRUE).asStatement());
  ...
  Function addFindFunction ( final org.xtuml.masl.metamodel.expression.Expression predicate, final FindExpression.Type type )
  {
    ...
    if ( predicate != null )
    {
      fn.setComment("MASL find: " + predicate.toString());
    }
    return fn;
  }
  ...
  {
    final Function predicateFn = mainClass.createMemberFunction(findPredicates,
                                                                "findPredicate_" + PredicateNameMangler.createMangler(predicate)
                                                                                                       .getName(),
                                                                Visibility.PUBLIC);
    ...
    if ( predicate.getFindParameters().size() + 1 > Boost.MAX_BIND_PARAMS )
    {
      ...
      final Variable params = predicateFn.createParameter(tuple.getTupleType().getOptimalParameterType(), "params");
    ...
    else
    {
      for ( final FindParameterExpression maslParam : predicate.getFindParameters() )
      {
        final Variable param = predicateFn.createParameter(domainTranslator.getTypes().getType(maslParam.getType())
    ...
    predicateExpression = new BinaryExpression(new UnaryExpression(UnaryOperator.NOT, getIsDeleted.asFunctionCall()),
                                               BinaryOperator.AND,
                                               predicateExpression);
    ...
    predicateFn.setComment("MASL find: " + predicate.toString());
  ...
  Function addOrderByPredicate ( final List<? extends InstanceOrderingExpression.Component> attributeOrder )
  {
    ...
    {
      fnName = fnName + (orderAttribute.isReverse() ? "r_" : "") + Mangler.mangleName(orderAttribute.getAttribute());
    }
    ...
    final Expression rhsObj = predicateFn.createParameter(new TypeUsage(mainClass, TypeUsage.ConstReference), "rhs").asExpression();

    Expression predicateExpression = null;

    if ( attributeOrder.size() == 0 )
    {
      final List<AttributeDeclaration> reversedAttributes = new ArrayList<AttributeDeclaration>(translator.getObjectDeclaration()
    ...
    {
      final List<InstanceOrderingExpression.Component> reversedAttributes = new ArrayList<InstanceOrderingExpression.Component>(attributeOrder);
      Collections.reverse(reversedAttributes);
      for ( final InstanceOrderingExpression.Component orderAttribute : reversedAttributes )
      {
        ...
        final Expression lhs = getter.asFunctionCall();
        final Expression rhs = getter.asFunctionCall(rhsObj, false);
        predicateExpression = Structure.buildComparator(predicateExpression, lhs, rhs, orderAttribute.isReverse());
      }
    }
    predicateFn.getCode().appendStatement(new ReturnStatement(predicateExpression));
    predicateFn.declareInClass(true);
  ...
  Function addPopulationForwarder ( final String newName, final Function populationFunction, final DeclarationGroup group )
  {
    ...
    {
      forwarder.getCode()
               .appendStatement(populationFunction.asFunctionCall(population.getSingleton(), false, params).asStatement());
    }
    else
    {
  ...
  Function addRelationshipCorr ( final RelationshipSpecification spec, final TypeUsage rhsType, final TypeUsage assocType )
  {
    final String navName = "correlate_"
  ...
  Function addRelationshipLink ( final boolean unlink, final RelationshipSpecification forwardSpec, final TypeUsage related )
  {
  ...
  {
    final String linkName = (unlink ? "un" : "")
                            + "link_"
  ...
  {
    final String navName = "navigate_"
                           + spec.getRelationship().getName()
                           + "_"
                           + spec.getRole()
                           + "_"
  ...
  Function addStateFunction ( final State state )
  {
    ...
    {
      return mainClass.createStaticFunction(stateActions, "state_" + Mangler.mangleName(state), Visibility.PRIVATE);
    }
    else
    {
      return mainClass.createMemberFunction(stateActions, "state_" + Mangler.mangleName(state), Visibility.PRIVATE);
    }
  }
  ...
  private CodeFile getEventHeaderFile ()
  {
    ...
    {
      eventHeaderFile = domainTranslator.getLibrary()
                                        .createPrivateHeader(Mangler.mangleFile(translator.getObjectDeclaration()) + "Events");
    }
