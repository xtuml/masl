...
public class CharacteristicTranslator extends ExpressionTranslator
{
  ...
  void translateFirst ( final CharacteristicExpression characteristicReference, final Scope scope )
  {
    ...
    {
      setReadExpression(createTranslator(((ArrayType)defined).getMinValue(), scope).getReadExpression());
    }
    else if ( defined instanceof UnconstrainedArraySubtype )
    {
      setReadExpression(createTranslator(((UnconstrainedArraySubtype)defined).getMinValue(), scope).getReadExpression());
    }
    else
    {
  ...
  {
    final org.xtuml.masl.cppgen.Expression base = createTranslator(characteristicReference.getLhs(), scope).getReadExpression();
    final org.xtuml.masl.cppgen.Expression ch = createTranslator(characteristicReference.getArguments().get(0), scope).getReadExpression();
  ...
  void translateGetUnique ( final CharacteristicExpression characteristicReference, final Scope scope )
  {
    ...
    setWriteableExpression(type.getType()
                               .callConstructor(prefixTrans.getWriteableExpression()));
  ...
  void translateImage ( final CharacteristicExpression characteristicReference, final Scope scope )
  {
    ...
    {
      setReadExpression(Architecture.stringClass.callConstructor(Boost.lexicalCast(new TypeUsage(Std.string), base)));
    }
    else
    {
  ...
  void translateLast ( final CharacteristicExpression characteristicReference, final Scope scope )
  {
    ...
    {
      setReadExpression(createTranslator(((ArrayType)defined).getMaxValue(), scope).getReadExpression());
    }
    else if ( defined instanceof UnconstrainedArraySubtype )
    {
      setReadExpression(createTranslator(((UnconstrainedArraySubtype)defined).getMaxValue(), scope).getReadExpression());
    }
    else
    {
  ...
  {
    setReadExpression(new Function("lower").asFunctionCall(createTranslator(characteristicReference.getLhs(), scope)
                                                                                                                    .getReadExpression(),
                                                           false));
  }
  ...
  void translatePred ( final CharacteristicExpression characteristicReference, final Scope scope )
  {
    setReadExpression(new UnaryExpression(UnaryOperator.PREDECREMENT,
                                          createTranslator(characteristicReference.getLhs(), scope)
                                                                                                   .getReadExpression()));
  }
  ...
  void translateRange ( final CharacteristicExpression characteristicReference, final Scope scope )
  {
    ...
    {
      minExp = createTranslator(((ArrayType)defined).getMinValue(), scope).getReadExpression();
      maxExp = createTranslator(((ArrayType)defined).getMaxValue(), scope).getReadExpression();
    }
    else if ( defined instanceof UnconstrainedArraySubtype )
    {
      minExp = createTranslator(((UnconstrainedArraySubtype)defined).getMinValue(), scope).getReadExpression();
      maxExp = createTranslator(((UnconstrainedArraySubtype)defined).getMaxValue(), scope).getReadExpression();
    }
    else
    {
  ...
  {
    setReadExpression(new Function("upper").asFunctionCall(createTranslator(characteristicReference.getLhs(), scope)
                                                                                                                    .getReadExpression(),
                                                           false));
  }
  ...
  void translateValue ( final CharacteristicExpression characteristicReference, final Scope scope )
  {
    ...
    {
      final TypeUsage type = Types.getInstance().getType(characteristicReference.getType());
      setReadExpression(type.getType()
                            .callConstructor(createTranslator(characteristicReference.getArguments().get(0), scope)
                                                                                                                   .getReadExpression()));
    }
    else
    {
      setReadExpression(new Function("getValue").asFunctionCall(createTranslator(characteristicReference.getLhs(), scope)
                                                                                                                         .getReadExpression(),
                                                                false));
    }
  }
