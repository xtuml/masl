    ...
    implements org.xtuml.masl.metamodel.expression.LinkUnlinkExpression
{
  ...
  public static Type                      LINK   = new Type("link",
                                                            org.xtuml.masl.metamodel.expression.LinkUnlinkExpression.Type.LINK);
  public static Type                      UNLINK = new Type("unlink",
                                                            org.xtuml.masl.metamodel.expression.LinkUnlinkExpression.Type.UNLINK);
        ...
        if ( att.isIdentifier() && !att.isUnique() )
        {
          ...
          for ( final ReferentialAttributeDefinition refAttDef : att.getRefAttDefs() )
          {
            if ( refAttDef.getDestinationAttribute().getParentObject() == lhsObj
                 || refAttDef.getDestinationAttribute().getParentObject() == relationship.getDestinationObject() )
            {
              ok = true;
              break;
            }
          ...
          if ( !ok )
          {
            throw new SemanticError(SemanticErrorCode.CannotDeduceAssocIdentifier,
                                    relRef.getPosition(),
                                    relationship
                                                .getAssocSpec().getDestinationObject().getName(),
                                    att.getName());
          }
        }
      }
  ...
  public boolean equals ( final Object obj )
  {
    ...
    if ( !(obj instanceof LinkUnlinkExpression) )
    {
      return false;
    }
    else
    {
      final LinkUnlinkExpression obj2 = (LinkUnlinkExpression)obj;

      return type.equals(obj2.type)
             && lhs.equals(obj2.lhs)
             && relationship.equals(obj2.relationship)
             && (rhs == null ? obj2.rhs == null : rhs.equals(obj2.rhs));
    }
  }
  ...
  {
    final InstanceType instance = InstanceType.createAnonymous(((AssociativeRelationshipDeclaration)relationship.getRelationship())
                                                                                                                                   .getAssocObject());
