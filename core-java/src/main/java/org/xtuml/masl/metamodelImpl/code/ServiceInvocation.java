                                                              ...
                                                              final List<Expression> args )
  {
    ...
    try
    {
      if ( lhs instanceof Domain.ServiceExpression )
      {
        ...
        final DomainService matchingService = service.getCallable(position, ServiceOverload.ServiceType.Service, args);
      ...
      else if ( lhs instanceof DomainTerminator.ServiceExpression )
      {
        ...
        final DomainTerminatorService matchingService = service.getCallable(position, ServiceOverload.ServiceType.Service, args);
        final List<Expression> newArgs = new ArrayList<Expression>();
        for ( int i = 0; i < args.size(); ++i )
        {
          if ( args.get(i) != null )
      ...
      else if ( lhs instanceof ObjectDeclaration.ServiceExpression )
      {
        ...
        final ObjectService matchingService = service.getOverload()
                                                     .getCallable(position, ServiceOverload.ServiceType.Service, args);
