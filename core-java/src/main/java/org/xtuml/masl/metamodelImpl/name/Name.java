...
public abstract class Name extends Positioned
    implements Named
{
                                    ...
                                    final State currentState )
  {
    try
    {
      ...
      {
        final boolean hasThis = currentState != null
                                && currentState.isInstance()
                                || currentService != null
        ...
        if ( found instanceof AttributeDeclaration )
        {
          ...
          {
            throw new SemanticError(SemanticErrorCode.OnlyForInstance, position, name, currentObject.getName());
          }
        }
        else if ( found instanceof ObjectDeclaration.ServiceOverload )
        {
          if ( ((ObjectDeclaration.ServiceOverload)found).isInstance() )
          {
            if ( hasThis )
            {
              return ((ObjectDeclaration.ServiceOverload)found).getReference(position, new ThisLiteral(position, currentObject));
            }
            else
            {
              throw new SemanticError(SemanticErrorCode.OnlyForInstance, position, name, currentObject.getName());
