...
public class DomainTerminator extends Name
    implements org.xtuml.masl.metamodel.domain.DomainTerminator
{
  ...
  public static class ServiceExpression extends org.xtuml.masl.metamodelImpl.expression.ServiceExpression<DomainTerminatorService>
  ...
  public static DomainTerminator create ( final Position position, final Domain domain, final String name, final PragmaList pragmas )
  {
  ...
  public static Expression createServiceExpression ( final Position position, final Expression lhs, final String name )
  {
    ...
    try
    {
      ...
      if ( lhs instanceof TerminatorNameExpression )
      {
        final TerminatorNameExpression termName = (TerminatorNameExpression)lhs;
  ...
  private DomainTerminator ( final Position position, final Domain domain, final String name, final PragmaList pragmas )
  {
    super(position, name);
    this.domain = domain;
  ...
  {
    final DomainTerminatorService found = services.get(service.getName()).get(service.getParameters(), service.isFunction());
    if ( found == null || !(service.getReturnType() == null ? found.getReturnType() == null : service.getReturnType()
                                                                                                     .equals(found.getReturnType())) )
    {
