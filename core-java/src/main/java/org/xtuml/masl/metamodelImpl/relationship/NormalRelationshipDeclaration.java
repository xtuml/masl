    ...
    implements org.xtuml.masl.metamodel.relationship.NormalRelationshipDeclaration
{
                              ...
                              final PragmaList pragmas )
  {
    ...
    try
    {
      domain.addRelationship(new NormalRelationshipDeclaration(position, domain, name, leftToRight, rightToLeft, pragmas));
    }
  ...
  {
    if ( getRightRole().equals(spec.getRole()) && getRightObject() == spec.getDestinationObject() && getLeftObject() == source )
    {
      return true;
    }
    else if ( getLeftRole().equals(spec.getRole()) && getLeftObject() == spec.getDestinationObject() && getRightObject() == source )
    {
      return false;
    }
    else
