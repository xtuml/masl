...
public class SplitExpression extends Expression implements org.xtuml.masl.metamodel.expression.SplitExpression {
    ...
    static {
        ...
        tsTimeLookup.put("hmsfm",
                         EnumSet.of(Field.HourOfDay, Field.MinuteOfHour, Field.SecondOfMinute, Field.MilliOfSecond));
        tsTimeLookup.put("hmsfmu",
                         EnumSet.of(Field.HourOfDay,
                                    Field.MinuteOfHour,
                                    Field.SecondOfMinute,
                                    Field.MilliOfSecond,
                                    Field.MicroOfMilli));
        tsTimeLookup.put("hmsfmun",
                         EnumSet.of(Field.HourOfDay,
                                    Field.MinuteOfHour,
                                    Field.SecondOfMinute,
                                    Field.MilliOfSecond,
                                    Field.MicroOfMilli,
                                    Field.NanoOfMicro));
        tsTimeLookup.put("hmsfmn",
                         EnumSet.of(Field.HourOfDay,
                                    Field.MinuteOfHour,
                                    Field.SecondOfMinute,
                                    Field.MilliOfSecond,
                                    Field.NanoOfMilli));
        tsTimeLookup.put("hmsfu",
                         EnumSet.of(Field.HourOfDay, Field.MinuteOfHour, Field.SecondOfMinute, Field.MicroOfSecond));
        tsTimeLookup.put("hmsfun",
                         EnumSet.of(Field.HourOfDay,
                                    Field.MinuteOfHour,
                                    Field.SecondOfMinute,
                                    Field.MicroOfSecond,
                                    Field.NanoOfMicro));
        tsTimeLookup.put("hmsfn",
                         EnumSet.of(Field.HourOfDay, Field.MinuteOfHour, Field.SecondOfMinute, Field.NanoOfSecond));
    ...
    SplitExpression(final Position position,
                    final Expression lhs,
                    final String characteristic,
                    final List<Expression> arguments) throws SemanticError {
        super(position);
    ...
    public Expression getFindSkeletonInner() {
        ...
        return new SplitExpression(getPosition(),
                                   lhs.getFindSkeleton(),
                                   splitType,
                                   characteristic,
                                   fields,
        ...
        for (final char ch : suffix.toCharArray()) {
            if (!inTime) {
                if (ch == 't') {
                    ...
                    if (f == null) {
                        throw new SemanticError(SemanticErrorCode.CharacteristicSuffixInvalid,
                                                getPosition(),
                                                suffix,
                                                lhs.getType());
                    } else {
    ...
    private void decodeTimestampSuffix(final String suffix) throws SemanticError {
        ...
        if (dateFields == null ||
            ...
            (timeFields.size() > 0 &&
             !(dateFields.contains(Field.DayOfMonth) ||
               dateFields.contains(Field.DayOfWeek) ||
               dateFields.contains(Field.DayOfYear)))) {
            throw new SemanticError(SemanticErrorCode.CharacteristicSuffixInvalid,
                                    getPosition(),
                                    suffix,
                                    lhs.getType());
