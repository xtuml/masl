...
class TypeBodyImpl extends ASTNodeImpl implements TypeBody, Scoped {

    protected class MemberGroupImpl implements TypeMemberGroup, TypeMemberGroupMember {
        ...
        public Property addProperty(final Type type, final String name, final Constructor initBy) {
            ...
            final MethodImpl
                    getter =
                    propertyGetters.addMethod("get" + TextUtils.upperFirst(name), ((TypeImpl) type).deepCopy());
            getter.setCodeBlock().addStatement(getAST().createReturn(field.asExpression()));
            getter.setVisibility(Visibility.PUBLIC);

            final MethodImpl
                    setter =
                    propertySetters.addMethod("set" + TextUtils.upperFirst(name), getAST().createType(void.class));
            ...
            if (initBy != null) {
                final ParameterImpl
                        constructorParam =
                        (ParameterImpl) initBy.addParameter(((TypeImpl) type).deepCopy(), name);
    ...
    private final class TBScope extends Scope {
    ...
    boolean containsNonPrivateTypeNamed(final String name) {
        for (final TypeDeclaration type : typeDeclarations) {
            if (type.getName().equals(name) && !type.getModifiers().isPrivate()) {
                return true;
            }
        }
        return false;
    ...
    boolean containsPublicTypeNamed(final String name) {
        for (final TypeDeclaration type : typeDeclarations) {
            if (type.getName().equals(name) && type.getModifiers().isPublic()) {
                return true;
            }
        }
        return false;
    ...
    boolean containsStaticPublicFieldNamed(final String name) {
        for (final Field field : fieldDeclarations) {
            if (field.getName().equals(name) && field.getModifiers().isStatic() && field.getModifiers().isPublic()) {
                return true;
            }
        }
    ...
    boolean containsStaticPublicMethodNamed(final String name) {
        for (final Method method : methodDeclarations) {
            if (method.getName().equals(name) && method.getModifiers().isStatic() && method.getModifiers().isPublic()) {
                return true;
            }
        }
        return false;
    ...
    boolean containsStaticPublicTypeNamed(final String name) {
        for (final TypeDeclaration type : typeDeclarations) {
            if (type.getName().equals(name) && type.getModifiers().isStatic() && type.getModifiers().isPublic()) {
                return true;
            }
        }
        return false;
