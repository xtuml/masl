...
public class Variable
{
  ...
  private class VariableDeclaration extends Declaration
  {
    ...
    @Override
    public void writeDeclaration ( final Writer writer, final String indent, final Namespace currentNamespace ) throws IOException
    {
      if ( comment != null )
      {
      ...
      writer.write(type.getQualifiedName(currentNamespace) + " " + getQualifiedName(currentNamespace) + getArraySize());
      writer.write(";\n");
    }

    @Override
    public void writeForwardDeclaration ( final Writer writer, final String indent, final Namespace currentNamespace ) throws IOException
    {
      writer.write(indent);
      writer.write("extern ");
      writer.write(type.getQualifiedName(currentNamespace) + " " + getQualifiedName(currentNamespace) + getArraySize());
  ...
  class VariableDefinition extends Definition
  {
    ...
    public void writeCodeDefinition ( final Writer writer, final String indent, final Namespace currentNamespace ) throws IOException
    {
      writer.write(indent
                   + (isStatic ? "static " : "")
    ...
    @Override
    public void writeDefinition ( final Writer writer, final String indent, final Namespace currentNamespace ) throws IOException
    {
      writer.write(indent + type.getQualifiedName(currentNamespace) + " " + getQualifiedName(currentNamespace) + getArraySize());
      writeInitialisation(writer, currentNamespace);
      writer.write(";");
    }
                                                 ...
                                                 final Namespace currentNamespace ) throws IOException
  {
    TextUtils.formatList(writer, variables, "", new Formatter<Variable>()
    {
      ...
      {
        String varDef = "\t" + var.type.getQualifiedName(currentNamespace) + "\t" + var.getQualifiedName(currentNamespace);
                                                ...
                                                final Namespace currentNamespace ) throws IOException
  {
    TextUtils.formatList(writer, variables, "", new Formatter<Variable>()
    {
      ...
      public String format ( final Variable var )
      {
        return "\t" + var.type.getQualifiedName(currentNamespace) + "\t" + var.getQualifiedName(currentNamespace);
      }
    }, ",\n", "");
  }
  ...
  public Variable ( final TypeUsage type, final String name, final Namespace parentNamespace, final Expression initialValue )
  {
    this(type, name, initialValue);
    setParentNamespace(parentNamespace);
  }

  public Variable ( final TypeUsage type, final String name, final Namespace parentNamespace, final Expression... constructorParams )
  {
  ...
  {
    return new BinaryExpression(obj, pointer ? BinaryOperator.PTR_REF : BinaryOperator.OBJ_REF, this.asExpression());
  }
