...
public class Function
{
        ...
        if ( isDeclaredInClass )
        {
          if ( isConstructor() )
          {
            ...
            for ( final Variable var : memberVariables )
            {
              ...
              final String initialVal = initialExp == null ? "" : initialExp.getCode(declaration.getParentNamespace());
              inits.add(var.getQualifiedName(declaration.getParentNamespace()) + "(" + initialVal + ")");
            }

            writer.write("\n");
            TextUtils.formatList(writer, inits, indent + TextUtils.getIndent() + ": ", ",\n"
                                                                                       + indent
                                                                                       + TextUtils.getIndent()
                                                                                       + "  ", "\n");
    ...
    public void writeDefinition ( final Writer writer, final String indent, final Namespace currentNamespace ) throws IOException
    {
      if ( isDestructor || !isPure )
      {
        ...
        while ( parentClass != null )
        {
          ...
          {
            definition.write(TextUtils.formatList(parentTemplateParamNames, indent + "template<", "", "", ", ", ">\n"));
          }
        ...
        if ( isCast )
        {
          definition.write(getQualifiedName(currentNamespace) + " " + returnType.getQualifiedName(currentNamespace) + " (");
        }
        else
        {
          if ( returnType != null )
  ...
  public Expression asFunctionCall ( final Expression obj, final boolean pointer, final List<Expression> args )
  {
    ...
    {
      return new BinaryExpression(obj, pointer ? BinaryOperator.PTR_REF : BinaryOperator.OBJ_REF, asFunctionCall(args));
    }
  ...
  String getQualifiedName ( final Namespace currentNamespace )
  {
    ...
    if ( getDeclaration().getParentNamespace() == null || getDeclaration().getParentNamespace().contains(currentNamespace) )
    {
      qualName = getFullName(currentNamespace);
    }
    else
    {
      qualName = getDeclaration().getParentNamespace().getQualifiedName(currentNamespace) + "::" + getFullName(currentNamespace);
    }
