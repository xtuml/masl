...
class ObjectMapper : public ResourceMonitorObserver {
  public:
    ...
    typedef typename PsObject_Traits<ObjectBase>::PsObjectPtrSwaSet
        PsBaseObjectPtrSwaSet;
    ...
    typedef
        typename PsObject_Traits<ObjectDerived>::PsObjectIdSet PsObjectIdSet;
    typedef
        typename PsObject_Traits<ObjectDerived>::PsObjectPtrSet PsObjectPtrSet;

    typedef typename PsObject_Traits<ObjectDerived>::PsSharedPtr PsSharedPtr;
    typedef
        typename PsObject_Traits<ObjectDerived>::PsCachedPtrSet PsCachedPtrSet;
    typedef
        typename PsObject_Traits<ObjectDerived>::PsCachedPtrMap PsCachedPtrMap;
    ...
    void commitDelete(const std::vector<SWA::IdType> &objects,
                      std::string &sql);
    ...
    void selectOne(::std::function<bool(PsObject *)> &predicate,
                   PsBaseObjectPtr &object);
    void selectOne(Criteria &sqlSelector, PsBaseObjectPtr &object);

    void selectAll(::std::function<bool(PsObject *)> &predicate,
                   PsBaseObjectPtrSwaSet &objectSet);
    void selectAll(Criteria &sqlSelector, PsBaseObjectPtrSwaSet &objectSet);

    void selectOneFromCache(const std::string &predicateDebug,
                            ::std::function<bool(PsObject *)> &predicate,
                            PsBaseObjectPtr &objectSet);
    void selectAllFromCache(const std::string &predicateDebug,
                            ::std::function<bool(PsObject *)> &predicate,
                            PsBaseObjectPtrSwaSet &objectSet);

  protected:
    ObjectMapper(
        const std::shared_ptr<ObjectSqlGenerator<PsBaseObject, PsObject>>
            &sqlGenerator);
...
ObjectMapper<ObjectBase, ObjectDerived>::ObjectMapper(
    const std::shared_ptr<ObjectSqlGenerator<PsBaseObject, PsObject>>
        &sqlGenerator)
    : writeOnModify(false), allLoaded(false), objectCount(0),
      currentArchIdentifier(0), sqlGenerator(sqlGenerator),
      unitOfWorkMap(*this),
...
bool ObjectMapper<ObjectBase, ObjectDerived>::initialise() {
    ...
    writeOnModify =
        WriteOnChangeEnabler(sqlGenerator->getTableName()).isEnabled();
...
// ***********************************************************************
template <class ObjectBase, class ObjectDerived>
void ObjectMapper<ObjectBase, ObjectDerived>::markAsDirty(
...
ObjectMapper<ObjectBase, ObjectDerived>::getInstance(
...
// ***********************************************************************
template <class ObjectBase, class ObjectDerived>
void ObjectMapper<ObjectBase, ObjectDerived>::deleteInstance(
    const PsBaseObjectPtr object) {
...
ObjectMapper<ObjectBase, ObjectDerived>::findOne() {
    ...
    if (objectCount != 0 && cache.empty() == true) {
        ...
        load((cacheStrategy->getOperationalCount(objectCount) == 0
                  ? 1
                  : cacheStrategy->getOperationalCount(objectCount)));
    }
    return cache.begin() == cache.end()
               ? PsObjectPtr()
               : PsObjectPtr((*cache.begin()).second.get());
}

// ***********************************************************************
// ***********************************************************************
template <class ObjectBase, class ObjectDerived>
void ObjectMapper<ObjectBase, ObjectDerived>::findAll(
    ::SWA::Set<PsBaseObjectPtr> &objectSet) {
    loadAll();
...
template <class ObjectBase, class ObjectDerived>
typename ObjectMapper<ObjectBase, ObjectDerived>::PsObjectPtr
ObjectMapper<ObjectBase, ObjectDerived>::find(
    const ::SWA::IdType architectureId) {
...
ObjectMapper<ObjectBase, ObjectDerived>::find(
    const PsObjectIdSet &identitySet) {
    loadSet(identitySet);
    PsObjectPtrSet objectSet(identitySet.size());
    std::copy(identitySet.begin(), identitySet.end(),
              MapToSetKeyComparator(cache, objectSet));
    return objectSet;
}
...
// ***********************************************************************
template <class ObjectBase, class ObjectDerived>
void ObjectMapper<ObjectBase, ObjectDerived>::load(
    const ::SWA::IdType instanceCount) {
...
// ***********************************************************************
template <class ObjectBase, class ObjectDerived>
void ObjectMapper<ObjectBase, ObjectDerived>::loadSingle(
    const ::SWA::IdType architectureId) {
    if (allLoaded == false) {
        if (cache.find(architectureId) == cache.end()) {
            ...
            std::string whereCondition =
                sqlGenerator->getColumnName("architecture_id") + " = " +
                valueToString(architectureId);
...
// ***********************************************************************
template <class ObjectBase, class ObjectDerived>
void ObjectMapper<ObjectBase, ObjectDerived>::loadSet(
    const PsObjectIdSet &identitySet) {
    if (allLoaded == false) {
        ...
        for (; idItr != idEnd; ++idItr) {
            ...
            if (std::find_if(
                    ...
                    AssociativeKeyAccessor<typename PsCachedPtrMap::value_type>(
                        unqiueKey)) == cache.end()) {
        ...
        if (!missingSet.empty()) {
            ...
            if (fullCachingEnabled()) {
                ...
                for (typename PsObjectIdSet::iterator missingItr =
                         missingSet.begin();
                     missingItr != missingSet.end(); ++missingItr) {
                ...
                missingObjectCritera.addFromClause(
                    sqlGenerator->getTableName());
...
ObjectMapper<ObjectBase, ObjectDerived>::locate(const ::SWA::IdType &identity) {
    ...
    if (locatedObjItr == cache.end()) {
        throw SqlException(::boost::make_tuple(
            "ObjectMapper::locate failed for", sqlGenerator->getDomainName(),
            sqlGenerator->getObjectName(), "with identify", identity));
    }
...
// ***********************************************************************
template <class ObjectBase, class ObjectDerived>
void ObjectMapper<ObjectBase, ObjectDerived>::report(
    ResourceMonitorContext &context) {
    if (context.getReportStream() != 0) {
        std::ostream *reportStrm = context.getReportStream();
        (*reportStrm) << "ObjectMapper[" << sqlGenerator->getTableName() << "]"
                      << std::endl;
        (*reportStrm) << "  cache  size           : " << cache.size()
                      << std::endl;
        (*reportStrm) << "  deleted cache size    : " << deletedCache.size()
                      << std::endl;
        (*reportStrm) << "  object size           : " << objectCount
                      << std::endl;
        (*reportStrm) << "  allLoaded             : " << allLoaded << std::endl;
        (*reportStrm) << "  currentArchIdentifier : " << currentArchIdentifier
                      << std::endl;
        (*reportStrm) << "ObjectStrategy [" << cacheStrategy->getName() << "]"
                      << std::endl;
        (*reportStrm) << "   allowFullCaching    : "
                      << cacheStrategy->allowFullCaching(objectCount)
                      << std::endl;
        (*reportStrm) << "   allowLinearFind     : "
                      << cacheStrategy->allowLinearFind(objectCount)
                      << std::endl;
        (*reportStrm) << "   getOperationalCount : "
                      << cacheStrategy->getOperationalCount(objectCount)
                      << std::endl;
    }
}
...
// ***********************************************************************
template <class ObjectBase, class ObjectDerived>
void ObjectMapper<ObjectBase, ObjectDerived>::compact(
    ResourceMonitorContext &context) {
...
// ***********************************************************************
template <class ObjectBase, class ObjectDerived>
void ObjectMapper<ObjectBase, ObjectDerived>::release(
    ResourceMonitorContext &context) {
    cache.clear();
}
...
void ObjectMapper<ObjectBase, ObjectDerived>::selectOne(
    ...
    if (allowInMemoryFind()) {
        std::find_if(cache.begin(), cache.end(),
                     singleObjectSelector(cache, object, predicate));
    } else {
        throw SqlException(::boost::make_tuple(
            "ObjectMapper::selectOne failed for", sqlGenerator->getDomainName(),
            sqlGenerator->getObjectName(), "in memory find not allowed"));
    }
}
...
template <class ObjectBase, class ObjectDerived>
void ObjectMapper<ObjectBase, ObjectDerived>::selectOne(
    Criteria &sqlSelector, PsBaseObjectPtr &object) {
...
void ObjectMapper<ObjectBase, ObjectDerived>::selectAll(
    ...
    if (allowInMemoryFind()) {
        std::for_each(cache.begin(), cache.end(),
                      objectMapperSelector(cache, objectSet, predicate));
        objectSet.forceUnique();
    } else {
        throw SqlException(::boost::make_tuple(
            "ObjectMapper::selectAll failed for", sqlGenerator->getDomainName(),
            sqlGenerator->getObjectName(), "in memory find not allowed"));
    }
}
...
template <class ObjectBase, class ObjectDerived>
void ObjectMapper<ObjectBase, ObjectDerived>::selectAll(
    Criteria &sqlSelector, PsBaseObjectPtrSwaSet &objectSet) {
    // As the find is not being done using the current transient population
    // need to write any pending changes into the database, so the select
    // can act upon the correct data set.
    unitOfWorkMap.flush();
...
void ObjectMapper<ObjectBase, ObjectDerived>::selectOneFromCache(
    ...
    if (allLoaded == false && !allowInMemoryFind()) {
        ...
        std::cout << "Warning  ObjectMapper::selectOneFromCache for table "
                  << sqlGenerator->getTableName()
...
void ObjectMapper<ObjectBase, ObjectDerived>::selectAllFromCache(
    ...
    if (allLoaded == false && !allowInMemoryFind()) {
        ...
        // references to complex data, so have no choice but to cache all data
        // and undertake linear find on cache, so report warning.
        std::cout << "Warning  ObjectMapper::selectAllFromCache for table "
                  << sqlGenerator->getTableName()
                  << " - using cache which CacheStrategy prohibits, for find "
                     "expression using complex type"
                  << predicateDebug << std::endl;
...
// ***********************************************************************
template <class ObjectBase, class ObjectDerived>
void ObjectMapper<ObjectBase, ObjectDerived>::commitUpdate(
    const PsObjectPtrSet &objects, std::string &sql) {
    std::for_each(objects.begin(), objects.end(),
                  [&](const auto &obj) { sqlGenerator->executeUpdate(obj); });
}

// ***********************************************************************
// ***********************************************************************
template <class ObjectBase, class ObjectDerived>
void ObjectMapper<ObjectBase, ObjectDerived>::commitInsert(
    const PsObjectPtrSet &objects, std::string &sql) {
    std::for_each(objects.begin(), objects.end(),
                  [&](const auto &obj) { sqlGenerator->executeInsert(obj); });
}

// ***********************************************************************
// ***********************************************************************
template <class ObjectBase, class ObjectDerived>
void ObjectMapper<ObjectBase, ObjectDerived>::commitDelete(
    const std::vector<SWA::IdType> &objects, std::string &sql) {
    std::for_each(objects.begin(), objects.end(),
                  [&](const auto &obj) { sqlGenerator->executeRemoveId(obj); });
}
