    ...
    class RelationshipAtomicity {
      public:
        RelationshipAtomicity(
            Container &cont, const ::SWA::IdType &key, const ObjectFirst &first, const ObjectSecond &second
        )
            : modified(false), complete(false), container(cont), key(key), first(first), second(second) {
