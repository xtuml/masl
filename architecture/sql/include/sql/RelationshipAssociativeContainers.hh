...
namespace SQL {
    ...
    class OneAssociativeRelationshipContainer {
      ...
      public:
        ...
        // ***********************************************************************
        void link(
            const ::SWA::IdType &relatedObjId,
            const ::SWA::IdType &assocObjId
        ); // used to form links from database
    ...
    class ManyAssociativeRelationshipContainer {
      ...
      public:
        ...
        // ***********************************************************************
        void display(std::ostream &ostr) {
            std::copy(linked.begin(), linked.end(), std::ostream_iterator<Container::value_type>(ostr, ","));
        }
      ...
      private:
        ...
        RelatedIterator relatedEnd() const {
            return RelatedIterator(linked.end());
        }

        AssocIterator assocBegin() const {
            return AssocIterator(linked.begin());
        }
        AssocIterator assocEnd() const {
            return AssocIterator(linked.end());
        }
    ...
    void ManyAssociativeRelationshipContainer<relNo, T, A>::unlink(
        ...
        typename Container::iterator relatedObjItr =
            linked.find(std::make_pair(relatedObj->getArchitectureId(), assocObj->getArchitectureId()));
