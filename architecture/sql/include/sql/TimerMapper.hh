...
class TimerMapper : public ::SWA::EventTimers {
  ...
  public:
    struct EventTimerData {
        EventTimerData()
            : id(), scheduled(), expired(), missed(), expiryTime(), period(),
              event() {}

        EventTimerData(const SWA::EventTimer &eventTimer)
            : id(eventTimer.getId()), scheduled(eventTimer.isScheduled()),
              expired(eventTimer.isExpired()), missed(eventTimer.getMissed()),
              expiryTime(eventTimer.getExpiryTime()),
              period(eventTimer.getPeriod()), event(eventTimer.getEvent()) {}
