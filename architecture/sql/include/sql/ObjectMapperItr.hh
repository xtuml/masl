...
// *****************************************************************
template <class Map, class Set, class Pred>
ObjectMapperSelector<Map, Set, Pred>
objectMapperSelector(Map &cachedMap, Set &selectedSet, Pred &predicate) {
...
class SingleObjectSelector
    : public std::unary_function<typename Map::value_type, bool> {
  public:
...
// *****************************************************************
template <class Map, class Obj, class Pred>
SingleObjectSelector<Map, Obj, Pred>
singleObjectSelector(Map &cachedMap, Obj &selectedObj, Pred &predicate) {
...
class MapToSetLoadIterator
    : public std::iterator<std::output_iterator_tag, void, void, void, void> {
  public:
    explicit MapToSetLoadIterator(Set &cont) : container(cont) {}

    MapToSetLoadIterator<Map, Set> &
    operator=(const typename Map::value_type &value) {
        container += typename Set::value_type(value.second.get());
        return *this;
    }

    // dereferencing is a no-op that returns the iterator itself
    MapToSetLoadIterator<Map, Set> &operator*() { return *this; }

    // increment operation is a no-op that returns the iterator itself
    MapToSetLoadIterator<Map, Set> &operator++() { return *this; }
    MapToSetLoadIterator<Map, Set> &operator++(int) { return *this; }
...
// *****************************************************************
template <class Map, class Set>
class MapToSetKeyComparatorItr
    : public std::iterator<std::output_iterator_tag, void, void, void, void> {
  public:
    ...
    MapToSetKeyComparatorItr<Map, Set> &
    operator=(const typename SourceSet::key_type &key) {
        typename Map::const_iterator matchingKeyItr = sourceMap.find(key);
        if (matchingKeyItr != sourceMap.end()) {
            destinationSet.insert(
                typename Set::key_type((*matchingKeyItr).second.get()));
        }
        return *this;
    }

    // dereferencing is a no-op that returns the iterator itself
    MapToSetKeyComparatorItr<Map, Set> &operator*() { return *this; }

    // increment operation is a no-op that returns the iterator itself
    MapToSetKeyComparatorItr<Map, Set> &operator++() { return *this; }
    MapToSetKeyComparatorItr<Map, Set> &operator++(int) { return *this; }
...
// *****************************************************************
template <class Map, class Set>
MapToSetKeyComparatorItr<Map, Set> MapToSetKeyComparator(const Map &map,
                                                         Set &set) {
...
// *****************************************************************
template <class Map, class Set>
class LoadCacheIterator
    : public std::iterator<std::output_iterator_tag, void, void, void, void> {
  public:
    explicit LoadCacheIterator(Map &dest) : destination(dest) {}

    LoadCacheIterator<Map, Set> &
    operator=(const typename Set::key_type &currentObj) {
        if (destination.find(currentObj->getArchitectureId()) ==
            destination.end()) {
            destination.insert(
                std::make_pair(currentObj->getArchitectureId(),
                               typename Map::mapped_type(currentObj.get())));
        }
        return *this;
    }

    // dereferencing is a no-op that returns the iterator itself
    LoadCacheIterator<Map, Set> &operator*() { return *this; }

    // increment operation is a no-op that returns the iterator itself
    LoadCacheIterator<Map, Set> &operator++() { return *this; }
    LoadCacheIterator<Map, Set> &operator++(int) { return *this; }
