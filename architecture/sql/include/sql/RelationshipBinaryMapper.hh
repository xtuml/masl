...
// ***********************************************************************
//! @Brief Class to handle binary relationships and their associated link
//! tables.
//!
...
//!
//!     relationship R11 is Aerial_Switch unconditionally feed_signals_to many
//!     Receiver_Unit,
//!                         Receiver_Unit unconditionally is_connected_to one
//!                         Aerial_Switch;
//!
//!     relationship R16 is Receiver_Unit unconditionally belongs_to one
//!     Receiver_Group,
//!                         Receiver_Group unconditionally groups_together one
//!                         Receiver_Unit;
//!
//! The placement of the one or many multipicity does not follow a set pattern,
//! i.e. the many side always comes first. As a result the following class is
//! used to handle the relationship definitions that define any combination of
//! multipicity for a Binary Relationship.
//!
// ***********************************************************************
template <int rel, class LhsRelContainer, class RhsRelContainer, bool lhsC,
          bool rhsC>
class RelationshipBinaryMapper : public ResourceMonitorObserver {
  public:
    ...
    typedef RelationshipBinarySqlGenerator<LhsRelContainer, RhsRelContainer>
        RelSqlGeneratorType;
    typedef RelationshipMapperUnitOfWork<RelationshipBinaryMapper>
        UnitOfWorkType;
  ...
  public:
    ...
    // *****************************************************************
    //! Method used to unassociate an object on the one side of the
    //! relationship, defined by the mapper class, with another object on the
    //! many side.
    //!
    ...
    // *****************************************************************
    //! Method used to unassociate an object on the many side of the
    //! relationship, defined by the mapper class, with another object on the
    //! one side.
    //!
    //! On unlink error will throw SqlException.
  ...
  private:
    typedef std::unordered_map<::SWA::IdType, RhsRelContainer>
        LhsToRhsLinksType;
    typedef std::unordered_map<::SWA::IdType, LhsRelContainer>
        RhsToLhsLinksType;
    typedef std::pair<typename LhsToRhsLinksType::iterator,
                      typename RhsToLhsLinksType::iterator>
        IteratorPairType;
  ...
  private:
    ...
    IteratorPairType loadLinks(const ::SWA::IdType &lhsObjId,
                               const ::SWA::IdType &rhsObjId);

    typename LhsToRhsLinksType::iterator
    loadLhsToRhsLinks(const ::SWA::IdType &lhsObjId);
    typename RhsToLhsLinksType::iterator
    loadRhsToLhsLinks(const ::SWA::IdType &rhsObjId);
...
template <int rel, class LhsRelContainer, class RhsRelContainer, bool lhsC,
          bool rhsC>
RelationshipBinaryMapper<rel, LhsRelContainer, RhsRelContainer, lhsC,
                         rhsC>::RelationshipBinaryMapper()
    : allLoaded(false), loadOnDemand(false), writeOnChange(false),
      unitOfWork(*this), lhsToRhsLinks(rel), rhsToLhsLinks(rel) {}
...
template <int rel, class LhsRelContainer, class RhsRelContainer, bool lhsC,
          bool rhsC>
RelationshipBinaryMapper<rel, LhsRelContainer, RhsRelContainer, lhsC, rhsC> &
RelationshipBinaryMapper<rel, LhsRelContainer, RhsRelContainer, lhsC,
                         rhsC>::singleton() {
...
template <int rel, class LhsRelContainer, class RhsRelContainer, bool lhsC,
          bool rhsC>
void RelationshipBinaryMapper<rel, LhsRelContainer, RhsRelContainer, lhsC,
                              rhsC>::
...
template <int rel, class LhsRelContainer, class RhsRelContainer, bool lhsC,
          bool rhsC>
bool RelationshipBinaryMapper<rel, LhsRelContainer, RhsRelContainer, lhsC,
                              rhsC>::isInitialised() const {
...
template <int rel, class LhsRelContainer, class RhsRelContainer, bool lhsC,
          bool rhsC>
void RelationshipBinaryMapper<rel, LhsRelContainer, RhsRelContainer, lhsC,
                              rhsC>::setToLoadOnDemand(bool isEnabled) {
...
template <int rel, class LhsRelContainer, class RhsRelContainer, bool lhsC,
          bool rhsC>
void RelationshipBinaryMapper<
    rel, LhsRelContainer, RhsRelContainer, lhsC,
    rhsC>::linkFromLhsToRhs(const LhsPsObjectPtr &lhsObj,
                            const RhsPsObjectPtr &rhsObj) {
    ...
    if (itrPair.first == lhsToRhsLinks.end()) {
        itrPair.first =
            lhsToRhsLinks.insert(std::make_pair(lhsObjId, RhsRelContainer()))
                .first;
    }

    if (itrPair.second == rhsToLhsLinks.end()) {
        itrPair.second =
            rhsToLhsLinks.insert(std::make_pair(rhsObjId, LhsRelContainer()))
                .first;
    }
...
void RelationshipBinaryMapper<
    rel, LhsRelContainer, RhsRelContainer, lhsC,
    rhsC>::linkFromRhsToLhs(const RhsPsObjectPtr &rhsObj,
                            const LhsPsObjectPtr &lhsObj) {
...
void RelationshipBinaryMapper<
    rel, LhsRelContainer, RhsRelContainer, lhsC,
    rhsC>::unlinkFromLhsToRhs(const LhsPsObjectPtr &lhsObj,
                              const RhsPsObjectPtr &rhsObj) {
    const ::SWA::IdType &lhsObjId = lhsObj.getChecked()->getArchitectureId();
    const ::SWA::IdType &rhsObjId = rhsObj.getChecked()->getArchitectureId();
    ...
    if (itrPair.first == lhsToRhsLinks.end() ||
        itrPair.second == rhsToLhsLinks.end()) {
        throw SqlException("RelationshipBinaryMapper::unlinkFromLhsToRhs - no "
                           "links to unlink");
    }
...
void RelationshipBinaryMapper<
    rel, LhsRelContainer, RhsRelContainer, lhsC,
    rhsC>::unlinkFromRhsToLhs(const RhsPsObjectPtr &rhsObj,
                              const LhsPsObjectPtr &lhsObj) {
...
::std::size_t
RelationshipBinaryMapper<rel, LhsRelContainer, RhsRelContainer, lhsC, rhsC>::
    countFromLhsToRhs(const LhsPsObjectPtr &lhsObj) {
    typename LhsToRhsLinksType::iterator lhsToRhsLinksItr =
        lhsToRhsLinks.find(lhsObj.getChecked()->getArchitectureId());
    return lhsToRhsLinksItr != lhsToRhsLinks.end()
               ? lhsToRhsLinksItr->second.linkCount()
               : 0;
}
...
::std::size_t
RelationshipBinaryMapper<rel, LhsRelContainer, RhsRelContainer, lhsC, rhsC>::
    countFromRhsToLhs(const RhsPsObjectPtr &rhsObj) {
    typename RhsToLhsLinksType::iterator rhsToLhsLinksItr =
        rhsToLhsLinks.find(rhsObj.getChecked()->getArchitectureId());
    return rhsToLhsLinksItr != rhsToLhsLinks.end()
               ? rhsToLhsLinksItr->second.linkCount()
               : 0;
}
...
template <int rel, class LhsRelContainer, class RhsRelContainer, bool lhsC,
          bool rhsC>
void RelationshipBinaryMapper<rel, LhsRelContainer, RhsRelContainer, lhsC,
                              rhsC>::objectDeletedLhs(const LhsPsObjectPtr
                                                          &lhsObj) {
    ...
    if (hasLinksLhs(lhsObj) == true) {
        ...
        errorMsgStrm << " Found dangling relationship(s) in relationship R"
                     << identity;
...
template <int rel, class LhsRelContainer, class RhsRelContainer, bool lhsC,
          bool rhsC>
void RelationshipBinaryMapper<rel, LhsRelContainer, RhsRelContainer, lhsC,
                              rhsC>::objectDeletedRhs(const RhsPsObjectPtr
                                                          &rhsObj) {
    const ::SWA::IdType architectureId =
    ...
    if (hasLinksRhs(rhsObj) == true) {
        std::ostringstream errorMsgStrm;
        errorMsgStrm
            << " Found dangling relationship(s) in one-to-many relationship R"
            << identity;
...
template <int rel, class LhsRelContainer, class RhsRelContainer, bool lhsC,
          bool rhsC>
bool RelationshipBinaryMapper<rel, LhsRelContainer, RhsRelContainer, lhsC,
                              rhsC>::hasLinksLhs(const LhsPsObjectPtr &lhsObj) {
    typename LhsToRhsLinksType::iterator lhsToRhsLinkItr =
        loadLhsToRhsLinks(lhsObj.getChecked()->getArchitectureId());
    return lhsToRhsLinkItr != lhsToRhsLinks.end()
               ? lhsToRhsLinkItr->second.isLinked()
               : false;
}
...
template <int rel, class LhsRelContainer, class RhsRelContainer, bool lhsC,
          bool rhsC>
bool RelationshipBinaryMapper<rel, LhsRelContainer, RhsRelContainer, lhsC,
                              rhsC>::hasLinksRhs(const RhsPsObjectPtr &rhsObj) {
    typename RhsToLhsLinksType::iterator rhsToLhsLinkItr =
        loadRhsToLhsLinks(rhsObj.getChecked()->getArchitectureId());
    return rhsToLhsLinkItr != rhsToLhsLinks.end()
               ? rhsToLhsLinkItr->second.isLinked()
               : false;
}
...
template <int rel, class LhsRelContainer, class RhsRelContainer, bool lhsC,
          bool rhsC>
const typename RelationshipBinaryMapper<rel, LhsRelContainer, RhsRelContainer,
                                        lhsC, rhsC>::NavigatedRhsType
RelationshipBinaryMapper<rel, LhsRelContainer, RhsRelContainer, lhsC, rhsC>::
...
template <int rel, class LhsRelContainer, class RhsRelContainer, bool lhsC,
          bool rhsC>
void RelationshipBinaryMapper<rel, LhsRelContainer, RhsRelContainer, lhsC,
                              rhsC>::report(ResourceMonitorContext &context) {}
...
template <int rel, class LhsRelContainer, class RhsRelContainer, bool lhsC,
          bool rhsC>
void RelationshipBinaryMapper<rel, LhsRelContainer, RhsRelContainer, lhsC,
                              rhsC>::compact(ResourceMonitorContext &context) {}
...
template <int rel, class LhsRelContainer, class RhsRelContainer, bool lhsC,
          bool rhsC>
void RelationshipBinaryMapper<rel, LhsRelContainer, RhsRelContainer, lhsC,
                              rhsC>::release(ResourceMonitorContext &context) {}
...
template <int rel, class LhsRelContainer, class RhsRelContainer, bool lhsC,
          bool rhsC>
typename RelationshipBinaryMapper<rel, LhsRelContainer, RhsRelContainer, lhsC,
                                  rhsC>::IteratorPairType
RelationshipBinaryMapper<rel, LhsRelContainer, RhsRelContainer, lhsC,
                         rhsC>::loadLinks(const ::SWA::IdType &lhsObjId,
                                          const ::SWA::IdType &rhsObjId) {
    typename LhsToRhsLinksType::iterator lhsToRhsLinksItr =
...
template <int rel, class LhsRelContainer, class RhsRelContainer, bool lhsC,
          bool rhsC>
typename RelationshipBinaryMapper<rel, LhsRelContainer, RhsRelContainer, lhsC,
                                  rhsC>::LhsToRhsLinksType::iterator
RelationshipBinaryMapper<rel, LhsRelContainer, RhsRelContainer, lhsC, rhsC>::
    loadLhsToRhsLinks(const ::SWA::IdType &lhsObjId) {
    typename LhsToRhsLinksType::iterator lhsToRhsLinksItr =
        lhsToRhsLinks.find(lhsObjId);
...
template <int rel, class LhsRelContainer, class RhsRelContainer, bool lhsC,
          bool rhsC>
typename RelationshipBinaryMapper<rel, LhsRelContainer, RhsRelContainer, lhsC,
                                  rhsC>::RhsToLhsLinksType::iterator
RelationshipBinaryMapper<rel, LhsRelContainer, RhsRelContainer, lhsC, rhsC>::
    loadRhsToLhsLinks(const ::SWA::IdType &rhsObjId) {
    typename RhsToLhsLinksType::iterator rhsToLhsLinksItr =
        rhsToLhsLinks.find(rhsObjId);
