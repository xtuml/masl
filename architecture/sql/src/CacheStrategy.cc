...
bool registerCommandLine() {
    SWA::CommandLine::getInstance().registerOption(SWA::NamedOption(
        SQL_CACHE_DISABLE, "disable sql object caching", false));
    return true;
}
...
// *************************************************
// *************************************************
CacheStrategyFactory::CacheStrategyFactory() : disabled_(false) {}
...
bool CacheStrategyFactory::registerStrategy(
    const std::string &domain, const std::string &object,
    const std::shared_ptr<CacheStrategy> loadStratergy) {
...
CacheStrategyFactory::getStrategy(const std::string &domain,
                                  const std::string &object) {
    std::shared_ptr<CacheStrategy> requiredStrategy(new DefaultCacheStrategy);
    if (disabled_ == true ||
        SWA::CommandLine::getInstance().optionPresent(SQL_CACHE_DISABLE)) {
        disabled_ = true;
        requiredStrategy =
            std::shared_ptr<CacheStrategy>(new DisableCacheStrategy);
    } else {
        // check and see if a specific stratergy has
        // been associated with the specified  object.
        std::string key(domain + "::" + object);
        DefinedStratergyDbType::iterator definedItr =
            definedStratergyDb_.find(key);
        if (definedItr != definedStratergyDb_.end()) {
            requiredStrategy =
                std::shared_ptr<CacheStrategy>(definedItr->second->clone());
        }
    }
