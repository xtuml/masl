...
bool ObjectSqlRepository::registerObjectSql(const ObjectSql *const sql) {
    const std::string objectKey(sql->getDomainName() +
                                "::" + sql->getObjectName());
...
// ***********************************************************************
const ObjectSql &
ObjectSqlRepository::getObjectSql(const std::string &domainName,
                                  const std::string &objectName) const {
    const std::string objectKey(domainName + "::" + objectName);
    std::map<std::string, const ObjectSql *const>::const_iterator objSqlItr =
        repository.find(objectKey);
