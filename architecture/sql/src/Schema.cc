...
namespace SQL {
    ...
    namespace {
        ...
        bool registerCommandLine() {
            ...
            SWA::Process::getInstance().getCommandLine().registerOption(
                SWA::NamedOption(SCHEMA_PREINCLUDE, "sql schema file", false, "include sql before ooa schema", true)
            );
            SWA::Process::getInstance().getCommandLine().registerOption(
                SWA::NamedOption(SCHEMA_POSTINCLUDE, "sql schema file", false, "include sql after ooa schema", true)
            );
            return true;
        }
        ...
        void loadSchemaFile(const std::string &fileName, std::string &schema) {
            ...
            if (iFileStream) {
                ...
                std::cout << "Schema formation failed to include file " << fileName << " : " << strerror(errno)
                          << std::endl;
            }
        }
    } // namespace
    ...
    class SchemaFormation {
      public:
        SchemaFormation(std::string &schema)
            : schema_(schema) {}
        SchemaFormation() {}
    ...
    class DropFormation {
      public:
        DropFormation(const std::shared_ptr<DropStatementFormatter> &dropFormatter, std::string &drop)
    ...
    template <class C>
    DropFormation<C> DropSchema(
        const std::shared_ptr<DropStatementFormatter> &dropFormatter, const C &container, std::string &dropSchema
    ) {
    ...
    bool Schema::registerTable(const std::string &iTableName, const std::string &iTableDefinition) {
        if (std::find_if(tableDefinitions_.begin(), tableDefinitions_.end(), matchKey(tableDefinitions_, iTableName)) !=
    ...
    std::string Schema::dropSchema() {
        ...
        std::shared_ptr<DropStatementFormatter> dropStmtFormatter = statementFactory->createDropStatementFormatter();

        std::string dropSchema;
        std::for_each(
            tableDefinitions_.begin(),
            tableDefinitions_.end(),
