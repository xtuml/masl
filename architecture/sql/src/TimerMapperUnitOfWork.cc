...
// ***********************************************************************
// ***********************************************************************
void TimerMapperUnitOfWork::updateTimer(
    const TimerMapper::EventTimerData &data) {
...
// ***********************************************************************
// ***********************************************************************
void TimerMapperUnitOfWork::committed(UnitOfWorkContext &context) { clear(); }
...
void TimerMapperUnitOfWork::startTransaction(UnitOfWorkContext &context) {
    if (isDirty()) {
        throw SqlException(
            "TimerMapperUnitOfWork::start_transaction : mapper is not clean");
    }
}
...
void TimerMapperUnitOfWork::commitTransaction(UnitOfWorkContext &context) {
    for (IdSet::const_iterator it = createSet.begin(), end = createSet.end();
         it != end; ++it)
        parent.commitCreate(*it);
    for (UpdateSet::const_iterator it = updateSet.begin(),
                                   end = updateSet.end();
         it != end; ++it)
        parent.commitUpdate(it->second);
    for (IdSet::const_iterator it = deleteSet.begin(), end = deleteSet.end();
         it != end; ++it)
