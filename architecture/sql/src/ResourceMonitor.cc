...
bool registerCommandLine() {
    SWA::CommandLine::getInstance().registerOption(SWA::NamedOption(
        CACHE_REPORTING, "report on the cache usage for the sql implementation",
        false));
    return true;
}
...
ResourceMonitor::ResourceMonitor()
    : reportingEnabled(
          SWA::CommandLine::getInstance().optionPresent(CACHE_REPORTING)) {}

// *************************************************
// *************************************************
ResourceMonitor::~ResourceMonitor() { observerList_.clear(); }
...
// *************************************************
// *************************************************
void ResourceMonitor::registerActiveResource(
    ResourceMonitorObserver *observer) {
    observerList_.insert(observer);
}

// *************************************************
// *************************************************
void ResourceMonitor::deregisterActiveResource(
    ResourceMonitorObserver *observer) {
...
void ResourceMonitor::reportOnResources() {
    ...
    std::for_each(observerList_.begin(), observerList_.end(),
                  [&](ResourceMonitorObserver *obs) { obs->report(context); });
}
...
void ResourceMonitor::compactResources() {
    ResourceMonitorContext context;
    std::for_each(observerList_.begin(), observerList_.end(),
                  [&](ResourceMonitorObserver *obs) { obs->compact(context); });
}
...
void ResourceMonitor::releaseResources() {
    ResourceMonitorContext context;
    std::for_each(observerList_.begin(), observerList_.end(),
                  [&](ResourceMonitorObserver *obs) { obs->release(context); });
}
} // namespace SQL
