...
namespace SQL {
    ...
    const RelationshipSql &RelationshipSqlRepository::getRelationshipSql(
        const std::string &domainName, const std::string &relationshipName
    ) const {
        const std::string relationshipKey(domainName + "_" + relationshipName);
        std::map<std::string, const RelationshipSql *const>::const_iterator relSqlItr =
            repository.find(relationshipKey);
