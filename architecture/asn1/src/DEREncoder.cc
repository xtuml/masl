...
namespace ASN1 {
    namespace DER {
        Encoder::Encoder()
            : tag(), tagClass(UNIVERSAL), constructed(false), value(), children(), stale(true), length(), header() {}
        ...
        void Encoder::cacheTag() const {
            ...
            if (tag < tagOverflow) {
                ...
                // bit   [ 7 | 6 |      5     | 4 | 3 | 2 | 1 | 0 ]
                header.push_back(
                    static_cast<Octet>(tagClass) | static_cast<Octet>(constructed ? CONSTRUCTED : PRIMITIVE) |
                    static_cast<Octet>(tag)
                );
            } else {
