...
namespace ASN1 {
    namespace DER {
        namespace {
            ...
            Buffer encode2sComplementInt(I value) {
                ...
                while (octetNo >= 0) {
                    ...
                    if (!valBuf.empty() || !((curOctet == 0x00 && (nextOctet & 0x80) == 0x00) ||
            ...
            Buffer encodeDouble(double value) {
                // Assume IEEE 754-2008 binary64 encoding... 1 bit sign, 11 bit exponent, 52 bit fraction (53 bit
                // normalized mantissa)
                ...
                if (exponentBits == specialExponent) {
                    if (fractionBits)
                        ...
                        return negative ? DER_negInfinity : DER_posInfinity;
                ...
                switch (exponentBuffer.size()) {
                    case 1:
                        buffer.push_back(firstOctet | ExponentLength1);
                        break;
                    case 2:
                        buffer.push_back(firstOctet | ExponentLength2);
                        break;
                    case 3:
                        buffer.push_back(firstOctet | ExponentLength3);
                        break;
                    default:
        ...
        Buffer encodeBoolean(bool value) {
            return Buffer(1, value ? 0xFF : 0x00);
        }
        ...
        Buffer encodeInteger(uint64_t value) {
            return encode2sComplementInt(value);
        }
