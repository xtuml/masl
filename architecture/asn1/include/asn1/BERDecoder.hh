    ...
    namespace BER {
        ...
        class Decoder {
          public:
            ...
            class ChildIterator : public boost::forward_iterator_helper<
                                      ChildIterator,
                                      Decoder<ForwardIterator>,
                                      std::ptrdiff_t,
                                      Decoder<ForwardIterator> &,
                                      Decoder<ForwardIterator> *> {
              public:
                ChildIterator(ForwardIterator curPos, ForwardIterator stopPos)
                    : curPos(curPos), stopPos(stopPos) {
                    if (curPos != stopPos) {
                ...
                ChildIterator(ForwardIterator stopPos)
                    : curPos(stopPos), stopPos(stopPos) {}

                const Decoder<ForwardIterator> &operator*() const {
                    return curChild;
                }
          ...
          public:
            ...
            TagClass getTagClass() const {
                return static_cast<TagClass>(*beginPos & tagClassMask);
            }
            bool isConstructed() const {
                return *beginPos & constructedMask;
            }

            bool isEndOfContent() const {
                return getTag() == 0 && getTagClass() == UNIVERSAL && !isConstructed();
            }

            ForwardIterator getValueBegin() const {
                return valuePos;
            }
            ForwardIterator getValueEnd() const {
                return endPos;
            }

            ChildIterator getChildrenBegin() const {
                return ChildIterator(valuePos, endPos);
            }
            ChildIterator getChildrenEnd() const {
                return ChildIterator(endPos);
            }
        ...
        Decoder<ForwardIterator>::Decoder()
            : beginPos(), valuePos(), endPos(), tag(), bufSize() {}

        template <class ForwardIterator>
        Decoder<ForwardIterator>::Decoder(ForwardIterator begin)
            : beginPos(begin), valuePos(begin), endPos(begin), tag(), bufSize() {
        ...
        void Decoder<ForwardIterator>::checkHeader(UniversalTag tag, bool constructed) const {
            ...
            if (getTag() != static_cast<uint64_t>(tag)) {
                throw std::runtime_error(std::format(
                    "ASN1 BER Decode Error: Expected tag {}, found {}", static_cast<uint64_t>(tag), getTag()
                ));
            }

            if (isConstructed() != constructed) {
                if (constructed)
                    throw std::runtime_error("ASN1 BER Decode Error: Expected constructed value");
                else
                    throw std::runtime_error("ASN1 BER Decode Error: Expected primitive value");
            }
        }
