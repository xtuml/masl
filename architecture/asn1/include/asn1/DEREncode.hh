...
namespace ASN1 {
    namespace DER {
        ...
        // Masl Types
        template <>
        inline Encoder encodeValue(const SWA::Duration &value) {
            return encode(value.nanos());
        }

        template <>
        inline Encoder encodeValue(const SWA::Timestamp &value) {
            return encode(value.nanosSinceEpoch());
        }

        template <>
        inline Encoder encodeValue(const SWA::Device &value) {
            return encode();
        }
