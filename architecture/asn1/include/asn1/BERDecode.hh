...
namespace ASN1 {
    namespace BER {
        ...
        namespace {
            template <class It>
            Octet octet(It it) {
                return static_cast<Octet>(*it);
            }

            template <class I, class It>
            I decode2sComplementInt(It curPos, It end) {
                I result = (octet(curPos) & 0x80) && std::numeric_limits<I>::is_signed ? -1 : 0;
        ...
        template <class It>
        bool decodeBoolean(It begin, It end) {
            return *begin;
        }

        template <class It>
        uint64_t decodeInteger(It begin, It end, uint64_t dummy) {
            return decode2sComplementInt<uint64_t>(begin, end);
        }
        template <class It>
        int64_t decodeInteger(It begin, It end, int64_t dummy) {
            return decode2sComplementInt<int64_t>(begin, end);
        }
        template <class It>
        uint32_t decodeInteger(It begin, It end, uint32_t dummy) {
            return decode2sComplementInt<uint32_t>(begin, end);
        }
        template <class It>
        int32_t decodeInteger(It begin, It end, int32_t dummy) {
            return decode2sComplementInt<int32_t>(begin, end);
        }
        template <class It>
        uint16_t decodeInteger(It begin, It end, uint16_t dummy) {
            return decode2sComplementInt<uint16_t>(begin, end);
        }
        template <class It>
        int16_t decodeInteger(It begin, It end, int16_t dummy) {
            return decode2sComplementInt<int16_t>(begin, end);
        }
        template <class It>
        uint8_t decodeInteger(It begin, It end, uint8_t dummy) {
            return decode2sComplementInt<uint8_t>(begin, end);
        }
        template <class It>
        int8_t decodeInteger(It begin, It end, int8_t dummy) {
            return decode2sComplementInt<int8_t>(begin, end);
        }
        template <class It>
        char decodeInteger(It begin, It end, char dummy) {
            return decode2sComplementInt<char>(begin, end);
        }

        template <class It>
        double decodeReal(It begin, It end, double dummy) {
            return decodeDouble(begin, end);
        }
        template <class It>
        float decodeReal(It begin, It end, float dummy) {
            return decodeDouble(begin, end);
        }
        ...
        std::string decodeOid(It curPos, It end) {
            ...
            std::string result = std::format("{}.{}", firstArc, secondArc);
        ...
        template <class MFT, MFT mf>
        struct mem_fun_test;

        template <class C, class I>
        void try_reserve(
            C &container, const Decoder<I> &decoder, mem_fun_test<void (C::*)(typename C::size_type), &C::reserve> * = 0
        ) {
            container.reserve(std::distance(decoder.getChildrenBegin(), decoder.getChildrenEnd()));
        }

        template <class C, class D>
        void try_reserve(C &container, const D &decoder) {}
        ...
        void decodeBasic(const Decoder<I> &decoder, T &value, Encoding::SequenceOf) {
            ...
            std::insert_iterator<T> ins(value, value.end());
            for (typename Decoder<I>::ChildIterator childIt = decoder.getChildrenBegin(),
                                                    end = decoder.getChildrenEnd();
                 childIt != end;
                 ++childIt) {
                if constexpr (requires {
                    ...
                    std::pair<
                        std::remove_const_t<typename T::value_type::first_type>,
                        typename T::value_type::second_type>
                        element;
