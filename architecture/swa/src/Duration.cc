...
namespace SWA {
    ...
    Duration &Duration::operator-=(const Duration &rhs) {
        if (rhs.ticks < 0 && std::numeric_limits<Tick>::max() + rhs.ticks < ticks)
            throw ProgramError("Duration Overflow");
        if (rhs.ticks > 0 && std::numeric_limits<Tick>::min() + rhs.ticks > ticks)
            throw ProgramError("Duration Underflow");
        ticks -= rhs.ticks;
        return *this;
    }
    ...
    Duration &Duration::operator*=(int rhs) {
        if (std::numeric_limits<Tick>::max() / rhs < ticks)
            throw ProgramError("Duration Overflow ");
        if (std::numeric_limits<Tick>::min() / rhs > ticks)
            throw ProgramError("Duration Underflow");
        ticks *= rhs;
        return *this;
    }
    ...
    Duration &Duration::operator*=(unsigned long rhs) {
        if (static_cast<Tick>(std::numeric_limits<Tick>::max() / rhs) < ticks)
            throw ProgramError("Duration Overflow ");
        if (static_cast<Tick>(std::numeric_limits<Tick>::min() / rhs) > ticks)
            throw ProgramError("Duration Underflow");
        ticks *= rhs;
        return *this;
    }
    ...
    Duration Duration::operator-() const {
        if (-std::numeric_limits<Tick>::max() > ticks)
            throw ProgramError("Duration Overflow");
        Duration res;
        res.ticks = -ticks;
        return res;
        ...
        while (it != end && !finished && (std::isdigit(*it) || (!inTimePart && *it == 'T'))) {
            ...
            switch (*it++) {
                ...
                case 'Y':
                    ...
                    break;
                case 'D':
                    if (inTimePart || lastEncountered >= FDays)
                        throw format_error();
                    lastEncountered = FDays;
                    result += Duration(DAYS, value);
                    if (finished)
                        result += Duration(DAYS, decimal);
                    break;
                case 'H':
                    if (!inTimePart || lastEncountered >= FHours)
                        throw format_error();
                    lastEncountered = FHours;
                    result += Duration(HOURS, value);
                    if (finished)
                        result += Duration(HOURS, decimal);
                    break;
                case 'M':
                    if (inTimePart) {
                        if (lastEncountered >= FMinutes)
                            throw format_error();
                        lastEncountered = FMinutes;
                        result += Duration(MINUTES, value);
                        if (finished)
                            result += Duration(MINUTES, decimal);
                    } else {
                        if (lastEncountered >= FMonths)
                            throw format_error();
                    ...
                    break;
                case 'S':
                    if (!inTimePart || lastEncountered >= FSeconds)
                        throw format_error();
                    lastEncountered = FSeconds;
                    result += Duration(SECONDS, value);
                    if (finished)
                        result += Duration(SECONDS, decimal);
    ...
    std::ostream &operator<<(std::ostream &stream, const Duration &time) {
        return stream << time.format_iso(
                   Duration::Day, Duration::Second, true, stream.precision(), !(stream.flags() & std::ios::showpoint)
               );
    }

    String Duration::format_iso(
        FormatField largestField, FormatField smallestField, bool hideZeros, int32_t decimalPlaces, bool truncateDecimal
    ) const {
    ...
    String
    Duration::format_hms(FormatField smallestField, int decimalPlaces, bool truncateDecimal, bool compactForm) const {
    ...
    String Duration::format(
        ...
        const Sequence<String> &suffixes
    ) const {
        static const Units divisors[] = {WEEKS, DAYS, HOURS, MINUTES, SECONDS};

        if (largestField > smallestField)
            throw ProgramError("Invalid field specification");
        ...
        if (value > 0.0 || !truncateDecimal || !hideZeros || result == prefix.s_str()) {
            ...
            std::ostringstream str;
            str << std::setfill('0') << std::setw(fieldWidth + decimalPlaces + (decimalPlaces > 0 ? 1 : 0))
                << std::fixed << std::setprecision(decimalPlaces) << value;
    ...
    Sequence<int64_t> Duration::getSplit(DurationFields fields) const {
        ...
        if (fields & Weeks) {
            result.push_back(remaining / WEEKS);
            remaining %= WEEKS;
        }
        if (fields & Days) {
            result.push_back(remaining / DAYS);
            remaining %= DAYS;
        }
        if (fields & Hours) {
            result.push_back(remaining / HOURS);
            remaining %= HOURS;
        }
        if (fields & Minutes) {
            result.push_back(remaining / MINUTES);
            remaining %= MINUTES;
        }
        if (fields & Seconds) {
            result.push_back(remaining / SECONDS);
            remaining %= SECONDS;
        }
        if (fields & Millis) {
            result.push_back(remaining / MILLIS);
            remaining %= MILLIS;
        }
        if (fields & Micros) {
            result.push_back(remaining / MICROS);
            remaining %= MICROS;
        }
        if (fields & Nanos) {
            result.push_back(remaining / NANOS);
            remaining %= NANOS;
        }
    ...
    Duration::Duration(
        ...
        if (fields & Weeks) {
            result += fromWeeks(*value++);
        }
        if (fields & Days) {
            result += fromDays(*value++);
        }
        if (fields & Hours) {
            result += fromHours(*value++);
        }
        if (fields & Minutes) {
            result += fromMinutes(*value++);
        }
        if (fields & Seconds) {
            result += fromSeconds(*value++);
        }
        if (fields & Millis) {
            result += fromMillis(*value++);
        }
        if (fields & Micros) {
            result += fromMicros(*value++);
        }
        if (fields & Nanos) {
            result += fromNanos(*value++);
        }
