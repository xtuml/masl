...
int main(int argc, const char *const *argv) {
    Process::getInstance().getCommandLine().registerOption(NamedOption(
        UtilOption, "Utility to include", false, "utility", true, true));
    Process::getInstance().getCommandLine().registerOption(NamedOption(
        LoopDisableOption, "terminate after schedules", false, "", false));

    try {
        try {
            Process::getInstance().getCommandLine().parse(argc, argv, true);
        } catch (ProgramError &e) {
            std::cerr << Process::getInstance().getCommandLine().getUsage()
                      << std::endl;
            return 1;
        }
        Process::getInstance().runStartup();

        std::cout << "Starting Process " << Process::getInstance().getName()
                  << std::endl;
        Process::getInstance().initialise();

        if (Process::getInstance().getCommandLine().optionPresent(UtilOption)) {
            const std::vector<std::string> &libs =
                Process::getInstance().getCommandLine().getMultiOption(
                    UtilOption);
        ...
        if (!Process::getInstance().getCommandLine().optionPresent(
                LoopDisableOption)) {
