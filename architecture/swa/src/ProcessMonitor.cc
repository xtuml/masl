...
namespace SWA {
    ...
    void ProcessMonitor::registerConnection(MonitorConnection *connection) {
        if (std::find(pendingConnections.begin(), pendingConnections.end(), connection) == pendingConnections.end()) {
            for (std::vector<MonitorConnection *>::const_iterator it = connection->getPrerequisites().begin();
                 it != connection->getPrerequisites().end();
                 ++it) {
    ...
    void ProcessMonitor::startMainLoop() {
        erase_safe_for_each(activeConnections.begin(), activeConnections.end(), [](MonitorConnection *monitor) {
            monitor->startMainLoop();
        });
    }
    ...
    void ProcessMonitor::endProcessingEventQueue() {
        erase_safe_for_each(activeConnections.begin(), activeConnections.end(), [](MonitorConnection *monitor) {
            monitor->endProcessingEventQueue();
        });
    }

    void ProcessMonitor::processRunning() {
        erase_safe_for_each(activeConnections.begin(), activeConnections.end(), [](MonitorConnection *monitor) {
            monitor->processRunning();
        });
    }
    ...
    void ProcessMonitor::threadAborted() {
        erase_safe_for_each(activeConnections.begin(), activeConnections.end(), [](MonitorConnection *monitor) {
            monitor->threadAborted();
        });
    }

    void ProcessMonitor::threadCompleting() {
        erase_safe_for_each(activeConnections.begin(), activeConnections.end(), [](MonitorConnection *monitor) {
            monitor->threadCompleting();
        });
    }
    ...
    void ProcessMonitor::creatingTimer(int timerId) {
        erase_safe_for_each(activeConnections.begin(), activeConnections.end(), [&](MonitorConnection *monitor) {
            monitor->creatingTimer(timerId);
        });
    }

    void ProcessMonitor::deletingTimer(int timerId) {
        erase_safe_for_each(activeConnections.begin(), activeConnections.end(), [&](MonitorConnection *monitor) {
            monitor->deletingTimer(timerId);
        });
    }

    void ProcessMonitor::cancellingTimer(int timerId) {
        erase_safe_for_each(activeConnections.begin(), activeConnections.end(), [&](MonitorConnection *monitor) {
            monitor->cancellingTimer(timerId);
        });
    }

    void ProcessMonitor::settingTimer(
        int timerId, const Timestamp &timeout, const Duration &period, const std::shared_ptr<Event> &event
    ) {
        erase_safe_for_each(activeConnections.begin(), activeConnections.end(), [&](MonitorConnection *monitor) {
            monitor->settingTimer(timerId, timeout, period, event);
        });
    }

    void ProcessMonitor::pauseRequested() {
        erase_safe_for_each(pendingConnections.begin(), pendingConnections.end(), [](MonitorConnection *monitor) {
            monitor->pauseRequested();
        });
    }
