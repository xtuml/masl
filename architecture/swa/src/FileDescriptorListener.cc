...
FileDescriptorListener::FileDescriptorListener(int fd, const Callback &callback,
                                               ActivityMonitor &monitor)
    : fd(fd), callback(callback), priority(ListenerPriority::getNormal()),
      active(false), forceCallback(false), monitor(monitor) {
    initFd();
    updateFdStatus(active);

    if (valid()) {
        monitor.addFdCallback(fd, [this](int fd) { callCallback(fd); });
    }
}

FileDescriptorListener::FileDescriptorListener(const Callback &callback,
                                               ActivityMonitor &monitor)
    : fd(-1), callback(callback), priority(ListenerPriority::getNormal()),
      active(false), forceCallback(false), monitor(monitor) {
    updateFdStatus(active);
}

FileDescriptorListener::~FileDescriptorListener() { clearFd(); }
...
bool FileDescriptorListener::valid() const { return !(fd < 0); }
