...
namespace SWA {
    ...
    int64_t Timestamp::toTicks(tm &split, int64_t nanos) {
        ...
        if (utctime == -1) {
            ...
            err << "Unable to convert time " << split.tm_year + 1900 << "-" << split.tm_mon + 1 << "-" << split.tm_mday
                << "T" << split.tm_hour << ":" << split.tm_min << ":" << split.tm_sec << "..." << nanos << "ns\n";
    ...
    Timestamp &Timestamp::operator-=(const Duration &rhs) {
        ...
        if (rhsTicks < 0 && std::numeric_limits<Tick>::max() + rhsTicks < ticks)
            throw ProgramError("Timestamp Overflow");
        if (rhsTicks > 0 && std::numeric_limits<Tick>::min() + rhsTicks > ticks)
            throw ProgramError("Timestamp Underflow");
        ticks -= rhsTicks;
        return *this;
    }
    ...
    int64_t Timestamp::readFixedSizeInt(Iterator &it, const Iterator &end, size_t length) {
        ...
        if (number.length() < length)
            ...
            return strtoll(number.c_str(), 0, 10);
    }
    ...
    bool Timestamp::readDatePart(
        ...
        if (it == end)
            ...
            rangeValid =
                rangeValid &&
                isInRange(week, 1, (jan1stDay == Thursday || (isLeap(year) && jan1stDay == Wednesday)) ? 53 : 52);
            ...
            if (useSeparators) {
                if (it != end && std::isdigit(*it)) {
                    ...
                    if (it != end && *it == dateSep) {
                        ...
                        rangeValid = rangeValid &&
                                     isInRange(day, 1, isLeap(year) ? leapMonthDays[month - 1] : monthDays[month - 1]);
                    }
                }
            } else {
                ...
                if (it != end && std::isdigit(*it)) {
                    ...
                    fields |= DayOfMonth;
                    rangeValid =
                        rangeValid && isInRange(day, 1, isLeap(year) ? leapMonthDays[month - 1] : monthDays[month - 1]);
                } else {
                    // yyyyddd format
    ...
    Timestamp::Tick Timestamp::readTicksFromDecimal(Iterator &it, const Iterator &end, size_t length, Units scale) {
        ...
        return static_cast<int64_t>(roundl(strtold(decimal.c_str(), 0) * static_cast<int64_t>(scale)));
    }

    template <class Iterator>
    bool Timestamp::readTimePart(
        Iterator &it,
        const Iterator &end,
        Timestamp::TimestampFields &fields,
        std::vector<int64_t> &values,
        ...
        bool inRange =
            isInRange(hours, 0, (minutes || seconds || nanos) ? 23 : 24) && isInRange(minutes, 0, 59) &&
    ...
    std::ostream &operator<<(std::ostream &stream, const Timestamp &time) {
        return stream << time.format_iso_ymdhms(
                   Timestamp::Second, stream.precision(), stream.flags() & std::ios::showpoint
               );
    }

    String Timestamp::format_iso_ymdhms(
        FormatField smallestField, int32_t decimalPlaces, bool truncateDecimal, bool compactFormat
    ) const {
        ...
        if (smallestField >= Month)
            result << (compactFormat ? "" : "-") << std::setw(2) << split[1];
        if (smallestField >= Day)
            result << (compactFormat ? "" : "-") << std::setw(2) << split[2];
        if (smallestField >= Hour)
            result << "T" << format_time(smallestField, decimalPlaces, truncateDecimal, compactFormat) << "Z";
        return result.str();
    }

    String Timestamp::format_iso_ydhms(
        FormatField smallestField, int32_t decimalPlaces, bool truncateDecimal, bool compactFormat
    ) const {
        ...
        result << std::setw(4) << split[0];
        if (smallestField >= Day)
            result << (compactFormat ? "" : "-") << std::setw(3) << split[1];
        if (smallestField >= Hour)
            result << "T" << format_time(smallestField, decimalPlaces, truncateDecimal, compactFormat) << "Z";
        return result.str();
    }

    String Timestamp::format_iso_ywdhms(
        FormatField smallestField, int32_t decimalPlaces, bool truncateDecimal, bool compactFormat
    ) const {
        ...
        result << std::setw(4) << split[0];
        if (smallestField >= Month)
            result << (compactFormat ? "W" : "-W") << std::setw(2) << split[1];
        if (smallestField >= Day)
            result << (compactFormat ? "" : "-") << std::setw(1) << split[2];
        if (smallestField >= Hour)
            result << "T" << format_time(smallestField, decimalPlaces, truncateDecimal, compactFormat) << "Z";
        return result.str();
    }
    ...
    String Timestamp::format_dmy() const {
        Sequence<int64_t> split =
            getSplit(::SWA::Timestamp::CalendarYear | ::SWA::Timestamp::MonthOfYear | ::SWA::Timestamp::DayOfMonth);
    ...
    String Timestamp::format_dtg() const {
        ...
        std::ostringstream result;
        result << std::setfill('0');
        result << std::setw(2) << day << std::setw(2) << hour << std::setw(2) << minute << "Z " << monthStr << " "
               << std::setw(2) << year % 100;
        return result.str();
    }
    ...
    String Timestamp::format_time(
        ...
        if (smallestField < Hour)
            return "";
        else if (smallestField == Hour)
            minDurField = Duration::Hour;
        else if (smallestField == Minute)
            minDurField = Duration::Minute;
        else
            minDurField = Duration::Second;
    ...
    Sequence<int64_t> Timestamp::getSplit(TimestampFields fields) const {
        ...
        if (fields & (CalendarYear | MonthOfYear | DayOfMonth | DayOfYear | WeekYear | WeekOfYear | DayOfWeek)) {
            ...
            // YMD fields
            if (fields & CalendarYear)
                result.push_back(split.tm_year + 1900);
            if (fields & MonthOfYear)
                result.push_back(split.tm_mon + 1);
            if (fields & DayOfMonth)
                result.push_back(split.tm_mday);

            if (fields & DayOfYear)
                result.push_back(split.tm_yday + 1);
            ...
            if (fields & (WeekYear | WeekOfYear)) {
                ...
                if (fields & WeekYear)
                    result.push_back(weekYearandWeek.first);
                if (fields & WeekOfYear)
                    result.push_back(weekYearandWeek.second);
            }

            if (fields & DayOfWeek)
                result.push_back((split.tm_wday + 6) % 7 + 1);
            if (fields & HourOfDay)
                result.push_back(split.tm_hour);
            if (fields & MinuteOfHour)
                result.push_back(split.tm_min);
            if (fields & SecondOfMinute)
                result.push_back(split.tm_sec);
        } else {
            if (fields & HourOfDay)
                result.push_back(hourOfDay());
            if (fields & MinuteOfHour)
                result.push_back(minuteOfHour());
            if (fields & SecondOfMinute)
                result.push_back(secondOfMinute());
        }

        if (fields & MilliOfSecond)
            result.push_back(milliOfSecond());
        if (fields & MicroOfSecond)
            result.push_back(microOfSecond());
        if (fields & NanoOfSecond)
            result.push_back(nanoOfSecond());

        if (fields & MicroOfMilli)
            result.push_back(microOfMilli());
        if (fields & NanoOfMilli)
            result.push_back(nanoOfMilli());

        if (fields & NanoOfMicro)
            result.push_back(nanoOfMicro());
    ...
    Timestamp::Timestamp(TimestampFields fields, const std::vector<int64_t> &values)
