...
Process::Process() : name(), shutdownRequested(false) {
    CommandLine::getInstance().registerOption(NamedOption(
        NAME_OPTION, "Name of the Process", false, "processName", true));
    CommandLine::getInstance().registerOption(
        NamedOption(COLD_START_OPTION, "Cold Start", false));
    CommandLine::getInstance().registerOption(
        NamedOption(PreInitSchedule, "Schedules to run before initialisation",
                    false, "scheduleFile", true, true));
    CommandLine::getInstance().registerOption(
        NamedOption(PostInitSchedule, "Schedules to run after initialisation",
                    false, "scheduleFile", true, true));
}
...
const Domain &Process::getDomain(int id) const { return domains[id]; }

Domain &Process::getDomain(int id) { return domains[id]; }
...
void Process::runScheduleFile(const std::string &fileName) {
    if (!shutdownRequested) {
        ...
        if (file >> fileStr.rdbuf()) {
            ...
            std::cerr << "Schedule file '" + fileName +
                             "' not found. Ignored.\n";
        }
    }
}
...
void Process::runSchedule(const Schedule &schedule) {
    ...
    for (Schedule::Actions::const_iterator
             action = schedule.getActions().begin(),
             end = schedule.getActions().end();
         action != end && !shutdownRequested; ++action) {
        (*action)();
    }
    postSchedule();
}

void Process::runService(std::function<void()> service,
                         const std::string &input) {
    ProcessingThread thread("Run Service");
    SWA::Device saveConsole(SWA::console());

    if (input.size() > 0) {
        SWA::console().setInputStream(
...
void Process::idle(int timeout_secs) {
    Timestamp finishTime =
        Timestamp::now() + Duration::fromSeconds(timeout_secs);
...
CommandLine &Process::getCommandLine() { return CommandLine::getInstance(); }

bool Process::coldStart() const {
    return CommandLine::getInstance().optionPresent(COLD_START_OPTION);
}

void Process::runStartup() const { starting(); }
...
void Process::runSchedules() {
    ...
    std::vector<std::string> preInit =
        getCommandLine().getMultiOption(PreInitSchedule);
    std::for_each(preInit.begin(), preInit.end(),
                  [this](const std::string &sched) { runScheduleFile(sched); });

    startNextSchedulePhase();

    std::vector<std::string> postInit =
        getCommandLine().getMultiOption(PostInitSchedule);
    std::for_each(postInit.begin(), postInit.end(),
                  [this](const std::string &sched) { runScheduleFile(sched); });

    postSchedules();
}

void Process::endInitialisation() const { initialised(); }

boost::signals2::connection
Process::registerStartupListener(const std::function<void()> &function) {
    return starting.connect(function);
}

boost::signals2::connection
Process::registerInitialisingListener(const std::function<void()> &function) {
    return initialising.connect(function);
}

boost::signals2::connection
Process::registerInitialisedListener(const std::function<void()> &function) {
    return initialised.connect(function);
}

boost::signals2::connection
Process::registerStartedListener(const std::function<void()> &function) {
    return started.connect(function);
}

boost::signals2::connection
Process::registerPreSchedulesListener(const std::function<void()> &function) {
    return preSchedules.connect(function);
}

boost::signals2::connection Process::registerPostSchedulesListener(
    const std::function<void()> &function) {
    return postSchedules.connect(function);
}

boost::signals2::connection
Process::registerPreScheduleListener(const std::function<void()> &function) {
    return preSchedule.connect(function);
}

boost::signals2::connection
Process::registerPostScheduleListener(const std::function<void()> &function) {
    return postSchedule.connect(function);
}

boost::signals2::connection Process::registerNextSchedulePhaseListener(
    const std::function<void()> &function) {
    return nextSchedulePhase.connect(function);
}

boost::signals2::connection Process::registerThreadStartedListener(
    const std::function<void(const std::string &)> &function) {
    return threadStarted.connect(function);
}

boost::signals2::connection Process::registerThreadCompletingListener(
    const std::function<void()> &function) {
    return threadCompleting.connect(function);
}

boost::signals2::connection Process::registerThreadCompletedListener(
    const std::function<void()> &function) {
    return threadCompleted.connect(function);
}

boost::signals2::connection Process::registerThreadAbortedListener(
    const std::function<void()> &function) {
    return threadAborted.connect(function);
}

boost::signals2::connection
Process::registerShutdownListener(const std::function<void()> &function) {
...
void Process::loadDynamicLibraries(const std::string &libName,
    ...
    for (SWA::Process::DomainList::const_iterator it = domains.begin(),
         ...
         it != end; ++it) {
        requiredDomainLibs.insert("lib" + it->getName() +
                                  (it->isInterface() ? interfaceSuffix : "") +
                                  "_" + libName + ".so");
    }
    ...
    if (requiredDomainLibs.size()) {
        throw std::runtime_error(
            std::string("failed to load domain metadata library(s) : ") +
            errorText);
    }
    ...
    if (loadProjectLib && projectName.size()) {
        const std::string processLib =
            "lib" + projectName + "_" + libName + ".so";
        if (!dlopen(processLib.c_str(), RTLD_NOW | RTLD_GLOBAL)) {
            throw std::runtime_error(
                std::string("failed to load process metadata library ") +
                processLib + " : " + std::string(dlerror()) + "\n");
        }
    }
}
