...
namespace SWA
{
  ...
  {
    EventTimer& result = *timers.insert(TimerStore::value_type(id, std::shared_ptr<EventTimer>(new EventTimer(id)))).first->second;
    return result;
  }
  ...
  void EventTimers::scheduleTimer (const TimerIdType id, const Timestamp& expiryTime, const Duration& period, const std::shared_ptr<Event>& event)
  {
    EventTimer& timer = getTimer(id);
  ...
  void EventTimers::scheduleTimer (const TimerIdType id, const Timestamp& expiryTime, const std::shared_ptr<Event>& event)
  {
