...
namespace SWA {
    ...
    void EventQueue::addEvent(const std::shared_ptr<Event> event) {
        ...
        if (event->getHasDest() && event->getHasSource() && event->getObjectId() == event->getSourceObjectId() &&
    ...
    std::vector<std::shared_ptr<Event>> EventQueue::getEvents() const {
        ...
        if (!empty()) {
            ...
            while (it != queue.end()) {
                if (it->second.empty()) {
                    ...
                    std::shared_ptr<Event> event = it->second.front();
                    it->second.pop_front();
