...
class Set
  ...
  public:
    ...
    template <class T2> Set &operator=(const Set<T2> &rhs) {
    ...
    template <class T2> Set &operator=(const Sequence<T2> &rhs) {
        data.assign(rhs.begin(), rhs.end());
        unique = rhs.isUnique();
        sorted = rhs.isSorted();
        return *this;
    ...
    template <class T2> Set &operator=(const Bag<T2> &rhs) {
        data.assign(rhs.begin(), rhs.end());
        unique = rhs.isUnique();
        sorted = rhs.isSorted();
        return *this;
    ...
    template <class T2> Set &operator+=(const ObjectPtr<T2> &rhs) {
        if (rhs) {
            unique = false;
            sorted = false;
            data.push_back(rhs);
    ...
    // Find functions
    const Set<T> &find() const { return *this; }
    ...
    template <class Predicate> Set<T> find(Predicate predicate) const {
        Set<T> result;
    ...
    Set<T> any(size_type count) const {
        if (count >= data.size()) {
            ...
            for (typename Container::const_iterator it = data.begin();
                 it != data.end() && result.size() < count; ++it) {
                result.insert(*it);
            }
            return Set<T>(result);
        }
  ...
  private:
    ...
    template <class T2> friend class Set;
};
...
template <class T, class T2> Set<T> operator+(Set<T> lhs, const T2 &rhs) {
    return lhs += rhs;
}

template <class T> inline std::size_t hash_value(const Set<T> &value) {
