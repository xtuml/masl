...
template <class Key, class Value> class Dictionary {
  ...
  public:
    ...
    Set<Key> getKeys() const {
        return Set<Key>(key_iterator(data.begin()), key_iterator(data.end()),
                        true, false);
    }

    Bag<Value> getValues() const {
        return Bag<Value>(value_iterator(data.begin()),
    ...
    bool operator==(const Dictionary &rhs) const {
        return std::map<Key, Value>(begin(), end()) ==
               std::map<Key, Value>(rhs.begin(), rhs.end());
    }
    bool operator<(const Dictionary &rhs) const {
        return std::map<Key, Value>(begin(), end()) <
               std::map<Key, Value>(rhs.begin(), rhs.end());
    ...
    friend void to_json(nlohmann::json &json,
                        const SWA::Dictionary<Key, Value> &v) {
        json = v.data;
    }

    friend void from_json(const nlohmann::json &json,
                          SWA::Dictionary<Key, Value> &v) {
        json.get_to(v.data);
    }
  ...
  private:
    ...
    class value_iterator
        : public boost::input_iteratable<value_iterator, Value *> {
      public:
        using iterator_category = std::input_iterator_tag;
