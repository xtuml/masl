...
class Timestamp
    : private boost::totally_ordered<Timestamp,
                                     boost::additive<Timestamp, Duration>> {
  private:
    enum Units {
        TICKS = 1LL,
        NANOS = TICKS * 1LL,
    ...
    template <class T> Timestamp(Units units, T time);
  ...
  public:
    ...
    Timestamp(TimestampFields fields, int64_t value1, int64_t value2 = 0,
              int64_t value3 = 0, int64_t value4 = 0, int64_t value5 = 0,
              int64_t value6 = 0, int64_t value7 = 0, int64_t value8 = 0,
              int64_t value9 = 0, int64_t value10 = 0);

    int64_t calendarYear() const { return getTm().tm_year + 1900; }
    int64_t monthOfYear() const { return getTm().tm_mon + 1; }
    int64_t dayOfMonth() const { return getTm().tm_mday; }
    int64_t weekYear() const;
    int64_t weekOfYear() const;
    int64_t dayOfWeek() const { return (getTm().tm_wday + 6) % 7 + 1; }
    int64_t dayOfYear() const { return getTm().tm_yday + 1; }
    int64_t hourOfDay() const { return mod(ticks, DAYS) / HOURS; }
    int64_t minuteOfHour() const { return mod(ticks, HOURS) / MINUTES; }
    int64_t secondOfMinute() const { return mod(ticks, MINUTES) / SECONDS; }
    int64_t milliOfSecond() const { return mod(ticks, SECONDS) / MILLIS; }
    int64_t microOfMilli() const { return mod(ticks, MILLIS) / MICROS; }
    int64_t nanoOfMicro() const { return mod(ticks, MICROS) / NANOS; }
    int64_t microOfSecond() const { return mod(ticks, SECONDS) / MICROS; }
    int64_t nanoOfMilli() const { return mod(ticks, MILLIS) / NANOS; }
    int64_t nanoOfSecond() const { return mod(ticks, SECONDS) / NANOS; }
    int64_t nanoOfDay() const { return mod(ticks, DAYS) / NANOS; }

    int64_t daysSinceEpoch() const { return ticks / DAYS; }
    int64_t secondsSinceEpoch() const { return ticks / SECONDS; }
    int64_t nanosSinceEpoch() const { return ticks / NANOS; }

    template <class T> static Timestamp fromDaysSinceEpoch(T value) {
        return Timestamp(DAYS, value);
    }

    template <class T> static Timestamp fromNanosSinceEpoch(T value) {
        return Timestamp(NANOS, value);
    }

    template <class T> static Timestamp fromSecondsSinceEpoch(T value) {
    ...
    String format_iso_ymdhms(FormatField smallestField = Second,
                             int decimalPlaces = 3, bool truncateDecimal = true,
                             bool compactForm = false) const;

    String format_iso_ywdhms(FormatField smallestField = Second,
                             int decimalPlaces = 3, bool truncateDecimal = true,
                             bool compactForm = false) const;

    String format_iso_ydhms(FormatField smallestField = Second,
                            int decimalPlaces = 3, bool truncateDecimal = true,
                            bool compactForm = false) const;
    ...
    String format_time(FormatField smallestField = Second,
                       int decimalPlaces = 3, bool truncateDecimal = true,
                       bool compactForm = false) const;
  ...
  private:
    ...
    template <class Iterator>
    static int64_t readFixedSizeInt(Iterator &it, const Iterator &end,
                                    size_t length);
    ...
    template <class Iterator>
    static bool readDatePart(Iterator &it, const Iterator &end,
                             Timestamp::TimestampFields &fields,
                             std::vector<int64_t> &values, bool &useSeparators);

    template <class Iterator>
    static Tick readTicksFromDecimal(Iterator &it, const Iterator &end,
                                     size_t length, Units scale);

    template <class Iterator>
    static bool readTimePart(Iterator &it, const Iterator &end,
                             Timestamp::TimestampFields &fields,
                             std::vector<int64_t> &values, bool useSeparators);
  ...
  private:
    explicit Timestamp(int64_t ticks) : ticks(ticks) {}
...
template <class T>
Timestamp::Timestamp(Units scale, T time) : ticks(scaledToTicks(time, scale)) {}

inline Timestamp::TimestampFields operator|(Timestamp::TimestampFields field1,
                                            Timestamp::TimestampFields field2) {
    return Timestamp::TimestampFields(static_cast<uint32_t>(field1) |
                                      static_cast<uint32_t>(field2));
}

inline Timestamp::TimestampFields &
operator|=(Timestamp::TimestampFields &field1,
           Timestamp::TimestampFields field2) {
...
struct std::formatter<SWA::Timestamp> : std::formatter<std::string> {
    ...
    auto format(const SWA::Timestamp &time, std::format_context &ctx) const {
        return std::formatter<std::string>::format(
            time.format_iso_ymdhms(SWA::Timestamp::Second, 9).s_str(), ctx);
    }
};
