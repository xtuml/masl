...
namespace SWA {
    ...
    class String : private boost::less_than_comparable<String, boost::equality_comparable<String>> {
      public:
        ...
        template <class It>
        void insert(iterator p, It i, It j) {
            data.insert(p, i, j);
        }
        ...
        String(const Container &rhs)
            : data(rhs) {}

        // Construct sequence type
        template <class V>
        String(const Sequence<V> &rhs)
            : data(rhs.begin(), rhs.end()) {}

        // Construct sequence type
        template <class V>
        String(const Set<V> &rhs)
            : data(rhs.begin(), rhs.end()) {}

        // Construct sequence type
        template <class V>
        String(const Bag<V> &rhs)
            : data(rhs.begin(), rhs.end()) {}
      ...
      private:
        ...
        void checkBounds(size_type index) const {
            if (index < 1)
                throw SWA::ProgramError("Attempt to read String element before start");
            if (index > size())
