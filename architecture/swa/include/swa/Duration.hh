...
namespace SWA {
    ...
    class Duration : private boost::totally_ordered<
                         Duration,
                         boost::additive<
                             Duration,
      ...
      private:
        ...
        template <class T>
        Duration(Units units, T time)
            : ticks(scaledToTicks(time, units)) {}
      ...
      public:
        ...
        template <class T>
        static Duration fromWeeks(T value) {
            return Duration(WEEKS, value);
        }
        template <class T>
        static Duration fromDays(T value) {
            return Duration(DAYS, value);
        }
        template <class T>
        static Duration fromHours(T value) {
            return Duration(HOURS, value);
        }
        template <class T>
        static Duration fromMinutes(T value) {
            return Duration(MINUTES, value);
        }
        template <class T>
        static Duration fromSeconds(T value) {
            return Duration(SECONDS, value);
        }
        template <class T>
        static Duration fromMillis(T value) {
            return Duration(MILLIS, value);
        }
        template <class T>
        static Duration fromMicros(T value) {
            return Duration(MICROS, value);
        }
        template <class T>
        static Duration fromNanos(T value) {
        ...
        int64_t dayOfWeek() const {
            return ticks % WEEKS / DAYS;
        }
        int64_t hourOfDay() const {
            return ticks % DAYS / HOURS;
        }
        int64_t minuteOfHour() const {
            return ticks % HOURS / MINUTES;
        }
        int64_t secondOfMinute() const {
            return ticks % MINUTES / SECONDS;
        }
        int64_t milliOfSecond() const {
            return ticks % SECONDS / MILLIS;
        }
        int64_t microOfMilli() const {
            return ticks % MILLIS / MICROS;
        }
        int64_t nanoOfMicro() const {
            return ticks % MICROS / NANOS;
        }
        int64_t microOfSecond() const {
            return ticks % SECONDS / MICROS;
        }
        int64_t nanoOfMilli() const {
            return ticks % MILLIS / NANOS;
        }
        int64_t nanoOfSecond() const {
            return ticks % SECONDS / NANOS;
        }
...
template <>
struct std::formatter<SWA::Duration> : std::formatter<std::string> {

    auto format(const SWA::Duration &dur, std::format_context &ctx) const {
        return std::formatter<std::string>::format(
            dur.format_iso(SWA::Duration::Day, SWA::Duration::Second, true, 9).s_str(), ctx
        );
    }
};
