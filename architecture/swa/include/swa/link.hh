...
void unlink(const LhsColl<ObjectPtr<LhsObj>> &lhs, void (LhsObj::*unlinker)()) {
    for (typename LhsColl<ObjectPtr<LhsObj>>::const_iterator it = lhs.begin(),
                                                             end = lhs.end();
         it != end; ++it) {
        (it->get()->*unlinker)();
    }
...
Set<ObjectPtr<AssocObj>> link(const LhsColl<ObjectPtr<LhsObj>> &lhs,
                              ...
                              Set<ObjectPtr<AssocObj>> (LhsObj::*linker)(
                                  const Bag<ObjectPtr<RhsObj>> &)) {
    Set<ObjectPtr<AssocObj>> result =
        combine_collection<Set, ObjectPtr<AssocObj>>(
            lhs,
            [&](const ObjectPtr<LhsObj> &l) { return (l.get().*linker)(rhs); });
    result.forceUnique();
    return result;
}
...
unlink(const LhsColl<ObjectPtr<LhsObj>> &lhs,
       Set<ObjectPtr<AssocObj>> (LhsObj::*unlinker)()) {
    Set<ObjectPtr<AssocObj>> result =
        combine_collection<Set, ObjectPtr<AssocObj>>(
            lhs, [&](const ObjectPtr<LhsObj> &l) {
