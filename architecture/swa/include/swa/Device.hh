...
namespace SWA {
    ...
    class basic_Device {
      public:
        ...
        basic_Device(
            const std::shared_ptr<InputStream> &input_stream, const std::shared_ptr<OutputStream> &output_stream
        );
        ...
        friend void to_json(nlohmann::json &json, const SWA::basic_Device<C, T> &v) {
            json = nullptr;
        }

        friend void from_json(const nlohmann::json &json, SWA::basic_Device<C, T> &v) {}
      ...
      private:
        ...
        template <class V>
        void write_raw(V buffer, std::true_type) const;

        template <class V>
        std::vector<V> read_raw(size_t length, std::true_type) const;

        template <class V>
        V read_raw(std::true_type) const;
    };
    ...
    template <class C, class T>
    basic_Device<C, T>::basic_Device(const std::shared_ptr<InputStream> &default_input_stream)
    ...
    template <class C, class T>
    basic_Device<C, T>::basic_Device(const std::shared_ptr<OutputStream> &default_output_stream)
    ...
    template <class C, class T>
    void basic_Device<C, T>::setInputStream(const std::shared_ptr<InputStream> &stream) {
        if (stream) {
    ...
    template <class C, class T>
    void basic_Device<C, T>::setOutputStream(const std::shared_ptr<OutputStream> &stream) {
        if (stream) {
            stream->setf(::std::ios::boolalpha);
            if (stream->fail())
                throw IOError(strerror(errno));
    ...
    template <class C, class T>
    template <class V>
    void basic_Device<C, T>::write_raw(const std::vector<V> &buffer, std::true_type) const {
    ...
    template <class C, class T>
    template <class V>
    std::vector<V> basic_Device<C, T>::read_raw(size_t length, std::true_type) const {
