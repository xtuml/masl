...
template <class C, class T> class basic_Device;

typedef basic_Device<char, ::std::char_traits<char>> Device;

template <class C, class T = ::std::char_traits<C>> class basic_Device {
  public:
    ...
    basic_Device(const std::shared_ptr<InputStream> &input_stream);

    basic_Device(const std::shared_ptr<OutputStream> &output_stream);
    ...
    void setInputStream(const std::shared_ptr<InputStream> &stream);
    void setOutputStream(const std::shared_ptr<OutputStream> &stream);
    void setInOutStream(const std::shared_ptr<InOutStream> &stream) {
    ...
    std::shared_ptr<StreamType> getInputStream(StreamType *dummy = 0) const {
        return std::dynamic_pointer_cast<StreamType>(input_stream);
    }

    template <class StreamType>
    std::shared_ptr<StreamType> getOutputStream(StreamType *dummy = 0) const {
        return std::dynamic_pointer_cast<StreamType>(output_stream);
    }

    // Functions to return references to the currently active streams.
    const std::shared_ptr<InputStream> &getInputStream() const {
        return input_stream;
    }
    const std::shared_ptr<OutputStream> &getOutputStream() const {
    ...
    template <class D> const basic_Device &operator<<(const D &data) const;

    template <class D> const basic_Device &operator>>(D &data) const;
    ...
    // EOF check - returns true if the next read of the input stream will fail
    // because it is at end of file.
    bool atEOF() const;
    void checkRead() const;
    void checkWrite() const;

    bool inputValid() const { return static_cast<bool>(input_stream); }
    bool outputValid() const { return static_cast<bool>(output_stream); }
    ...
    template <class V> void write_raw(const std::vector<V> &buffer) const;

    template <class V> void write_raw(V buffer) const;
    ...
    template <class V> std::vector<V> read_raw(size_t length) const;

    template <class V> V read_raw() const;

    friend void to_json(nlohmann::json &json,
                        const SWA::basic_Device<C, T> &v) {
        json = nullptr;
    }

    friend void from_json(const nlohmann::json &json,
                          SWA::basic_Device<C, T> &v) {}
  ...
  private:
    ...
    InputStream &In() const {
        if (!input_stream)
            throw IOError("Input Device Invalid");
    ...
    OutputStream &Out() const {
        if (!output_stream)
            throw IOError("Output Device Invalid");

        return *output_stream;
    }

    template <class V>
    void write_raw(const std::vector<V> &buffer, std::true_type) const;

    template <class V> void write_raw(V buffer, std::true_type) const;

    template <class V>
    std::vector<V> read_raw(size_t length, std::true_type) const;

    template <class V> V read_raw(std::true_type) const;
};
...
basic_Device<C, T>::basic_Device() : record_delimiter('\n') {}

template <class C, class T>
basic_Device<C, T>::basic_Device(
    const std::shared_ptr<InputStream> &default_input_stream)
    : input_stream(default_input_stream), record_delimiter('\n') {
    default_input_stream->setf(::std::ios::boolalpha);
    if (default_input_stream->fail())
        throw IOError(strerror(errno));
}

template <class C, class T>
basic_Device<C, T>::basic_Device(
    const std::shared_ptr<OutputStream> &default_output_stream)
    : output_stream(default_output_stream), record_delimiter('\n') {
    default_output_stream->setf(::std::ios::boolalpha);
    if (default_output_stream->fail())
        throw IOError(strerror(errno));
}
...
basic_Device<C, T>::basic_Device(
    const std::shared_ptr<InputStream> &default_input_stream,
    const std::shared_ptr<OutputStream> &default_output_stream)
    : input_stream(default_input_stream), output_stream(default_output_stream),
      record_delimiter('\n')
...
template <class C, class T> bool basic_Device<C, T>::atEOF() const {
...
template <class C, class T>
void basic_Device<C, T>::setInputStream(
...
template <class C, class T>
void basic_Device<C, T>::setOutputStream(
...
template <class C, class T>
const basic_Device<C, T> &
basic_Device<C, T>::operator<<(const basic_Device<C, T> &data) const {
    Out().clear();
    Out() << data.In().rdbuf();
    checkWrite();
    return *this;
...
template <class C, class T>
const basic_Device<C, T> &
basic_Device<C, T>::operator>>(basic_Device<C, T> &data) const {
    In().clear();
    In() >> data.Out().rdbuf();
    checkRead();
    return *this;
...
template <class C, class T>
const basic_Device<C, T> &
basic_Device<C, T>::operator<<(OutputStream &(*op)(OutputStream &)) const {
    Out().clear();
    (*op)(Out());
    checkWrite();
    return *this;
...
template <class C, class T>
const basic_Device<C, T> &
basic_Device<C, T>::operator>>(InputStream &(*op)(InputStream &)) const {
    In().clear();
    (*op)(In());
    checkRead();
    return *this;
...
void basic_Device<C, T>::write_raw(const std::vector<V> &buffer) const {
    write_raw(buffer, typename std::is_fundamental<V>::type());
}
...
template <class C, class T>
template <class V>
void basic_Device<C, T>::write_raw(const std::vector<V> &buffer,
                                   std::true_type) const {
    write_raw(reinterpret_cast<const char *>(&buffer[0]),
              buffer.size() * sizeof(V));
}

template <class C, class T>
template <class V>
void basic_Device<C, T>::write_raw(V buffer, std::true_type) const {
...
template <class C, class T> char basic_Device<C, T>::read_raw_char() const {
    char c;
    In().clear();
    In().get(c);
    checkRead();
...
std::vector<V> basic_Device<C, T>::read_raw(size_t length) const {
    return read_raw<V>(length, typename std::is_fundamental<V>::type());
}

template <class C, class T>
template <class V>
std::vector<V> basic_Device<C, T>::read_raw(size_t length,
                                            std::true_type) const {
...
V basic_Device<C, T>::read_raw() const {
    return read_raw<V>(typename std::is_fundamental<V>::type());
}

template <class C, class T>
template <class V>
V basic_Device<C, T>::read_raw(std::true_type) const {
    V result;
    In().clear();
...
template <class C, class T> void basic_Device<C, T>::checkRead() const {
    if (In().fail()) {
        if (In().eof()) {
            In().clear();
...
template <class C, class T> void basic_Device<C, T>::checkWrite() const {
    if (Out().fail()) {
        Out().clear();
