...
namespace SWA {
    ...
    class Bag
        : private boost::less_than_comparable<
              ...
              boost::equality_comparable<
                  Bag<T>,
                  boost::less_than_comparable<Bag<T>, Sequence<T>, boost::equality_comparable<Bag<T>, Sequence<T>>>>> {
      public:
        //*********************************************************
        ...
        template <class It>
        Bag(It i, It j, bool unique = false, bool sorted = false)
        ...
        Bag(const Sequence<T> &rhs)
            : data(rhs.getData()), unique(rhs.isUnique()), sorted(rhs.isSorted()) {}

        template <class T2>
        Bag(const Sequence<T2> &rhs)
            : data(rhs.begin(), rhs.end()), unique(rhs.isUnique()), sorted(rhs.isSorted()) {}

        Bag &operator=(const Sequence<T> &rhs) {
            data = rhs.getData();
            unique = rhs.isUnique();
            sorted = rhs.isSorted();
            return *this;
        ...
        template <class T2>
        Bag(const Set<T2> &rhs)
            : data(rhs.begin(), rhs.end()), unique(rhs.isUnique()), sorted(rhs.isSorted()) {}

        Bag &operator=(const Set<T> &rhs) {
            data = rhs.getData();
            unique = rhs.isUnique();
            sorted = rhs.isSorted();
            return *this;
        ...
        explicit Bag(const T &element)
            : data(NullCheck<T>::isNull(element) ? 0 : 1, element), unique(true), sorted(true) {}

        template <class T2>
        explicit Bag(const ObjectPtr<T2> &element)
        ...
        T any() const {
            if (!data.size())
                throw ProgramError("Attempt to get element from empty bag");
            return data[0];
        }
    ...
    template <class T>
    ::std::ostream &operator<<(::std::ostream &stream, const Bag<T> &obj) {
