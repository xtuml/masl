...
namespace SWA {
    ...
    class ObjectPtr {
      public:
        ...
        ObjectPtr()
            : ptr(0) {}
        ObjectPtr(NullObjectPtr)
            : ptr(0) {}

        template <class T2>
        explicit ObjectPtr(T2 *ptr)
            : ptr(ptr) {}
        ...
        template <class T2>
        ObjectPtr &operator=(T2 *ptr) {
            this->ptr = ptr;
            return *this;
        }

        template <class T2>
        ObjectPtr(const ObjectPtr<T2> &rhs)
            : ptr(rhs.get()) {}

        template <class T2>
        ObjectPtr &operator=(const ObjectPtr<T2> &rhs) {
            this->ptr = rhs.get();
            return *this;
        }

        template <class T2>
        ObjectPtr<T2> downcast() const {
    ...
    template <class T>
    std::size_t hash_value(const ObjectPtr<T> &p) {
    ...
    class ObjectPtr<void> {
      public:
        ...
        ObjectPtr(const ObjectPtr<T2> &objPtr)
            : ptr(objPtr.get()), archId(objPtr.get() ? objPtr->getArchitectureId() : 0) {}
        ...
        SWA::IdType getArchitectureId() const {
            return archId;
        }

        friend bool operator==(const ObjectPtr &cp, NullObjectPtr) {
            return !cp;
        }
        friend bool operator==(NullObjectPtr, const ObjectPtr &cp) {
            return !cp;
        }
    ...
    template <class NT>
    struct NullCheck {
        static bool isNull(const NT &element) {
            return false;
        }
    };
