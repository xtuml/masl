...
namespace SWA {
    ...
    class EventTimers : public DynamicSingleton<EventTimers> {
      ...
      public:
        ...
        void scheduleTimer(const TimerIdType id, const Timestamp &expiryTime, const std::shared_ptr<Event> &event);
        ...
        boost::signals2::connection registerTimerFiredListener(const std::function<void()> &function);
      ...
      private:
        typedef std::unordered_map<TimerIdType, std::shared_ptr<EventTimer>> TimerStore;
        TimerStore timers;
        bool suspended;
