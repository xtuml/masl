        ...
        output << nlohmann::json(
                      {
                          ...
                          {"timestamp",
                           fmt::format("{:%FT%TZ}", event.getTimestamp())},
...
class JsonLayoutFactory : public log4cplus::spi::LayoutFactory {
  public:
    ...
    createObject(const log4cplus::helpers::Properties &props) override {
...
struct Init {
    Init() {
        ...
        for (const auto &app : log4cplus::Logger::getRoot().getAllAppenders()) {
            app->setLayout(
                std::make_unique<log4cplus::PatternLayout>("%-5p %c - %m%n"));
        }

        // register JSON layout
        log4cplus::spi::getLayoutFactoryRegistry().put(
            std::make_unique<JsonLayoutFactory>());
    }
