...
namespace SQLITE {
        ...
        if (dbName_ != ":memory:") {
            ...
            if (errorCode != SQLITE_OK) {
                throw SqliteException(
                    ::boost::make_tuple("Failed to open SQLITE database :", dbName_, "-", sqlite3_errmsg(database_))
                );
            }
            ...
            if (isEmpty()) {
                ...
                if (executeStatement(schema_) == false) {
                    throw SqliteException(
                        ::boost::make_tuple("Failed to create schema for", dbName_, ":", sqlite3_errmsg(database_))
                    );
                }
            } else {
    ...
    bool Database::openMemoryBasedDb() {
        ...
        if (dbName_ == ":memory:") {
            ...
            if (errorCode != SQLITE_OK) {
                throw SqliteException(
                    ::boost::make_tuple("Failed to open SQLITE in memory database :", sqlite3_errmsg(database_))
                );
            }
            schema_ = ::SQL::Schema::singleton().getSchema();
            if (executeStatement(schema_) == false) {
                throw SqliteException(
                    ::boost::make_tuple("Failed to create schema for in memory database :", sqlite3_errmsg(database_))
                );
            }
        }
        return isMemoryDb;
    ...
    // *****************************************************************
    // *****************************************************************
    std::shared_ptr<::SQL::AbstractStatementFactory> Database::getStatementFormatter() {
        return std::shared_ptr<::SQL::AbstractStatementFactory>(new SqliteAbstractStatementFactory);
    }
    ...
    void Database::commitTransaction() {
        ...
        if (executeStatement(COMMIT_TRANSACTION + SQL_TERMINATOR) == false) {
            throw SqliteException(
                ::boost::make_tuple("Failed to commit transaction for database", dbName_, ":", getCurrentError())
            );
        }
    }
                ...
                for (int columnIndex = 0; columnIndex < columnCount; ++columnIndex) {
                    ...
                    switch (columnType) {
                        case SQLITE_FLOAT: {
                            ...
                            std::string doubleValue = std::format("{}", doubleEntry);
                            currentRow[columnIndex] = doubleValue;
                        } break;
                        case SQLITE_BLOB: {
                            int32_t column2Bytes = sqlite3_column_bytes(ppStmt, columnIndex);
                            const char *column2Blob =
                                reinterpret_cast<const char *>(sqlite3_column_blob(ppStmt, columnIndex));
    ...
    bool Database::isEmpty() {
        ...
        if (executeQuery(schemaQuery, masterSchemaResult) == false) {
            throw SqliteException(
                std::string("Schema::hasSchema : failed to "
    ...
    void Database::validateSchema(const ::SQL::Schema::TableDefinitionType &tableDefinitions) {
        ...
        if (executeQuery(schemaQuery, masterSchemaResult) == false) {
            throw SqliteException(
                std::string("Schema::validateSchema : failed to "
                            "execute query on master table - ") +
                getCurrentError()
            );
        }
        ...
        if (masterSchemaResult.getColumns() != 2) {
            throw SqliteException(::boost::make_tuple(
                ...
                2,
                "found",
                masterSchemaResult.getColumns()
            ));
        }

        for (::SQL::Schema::TableDefinitionType::const_iterator tableItr = tableDefinitions.begin();
             tableItr != tableDefinitions.end();
             ++tableItr) {
    ...
    std::string Database::normalise(const std::string &sql) const {
        ...
        std::copy(
            std::istream_iterator<std::string>(inputTokenStream),
    ...
    // *****************************************************************
    // *****************************************************************
    void Database::checkColumnCount(
        const char *const location, const int32_t actual, const int32_t expected, const std::string &query
    ) {
        if (expected != actual) {
            throw ::SQLITE::SqliteException(
                ::boost::make_tuple(location, "- column count mismatch : expected", expected, "found", actual, query)
            );
        }
    }
