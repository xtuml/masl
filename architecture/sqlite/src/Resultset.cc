...
//********************************************************
//********************************************************
ResultSet::ResultSet() : rows_(0), columns_(0) {}
...
void ResultSet::setColumns(const ColumnType iColumns) {
    ...
    if (matrix_.empty() == false && columns_ != iColumns) {
        throw SqliteException("ResultSet::setColumns - operation failed : "
                              "ResultSet already contains data");
    }
    columns_ = iColumns;
...
//********************************************************
//********************************************************
void ResultSet::addColumnName(const ColumnType iIndex,
                              const EntryType &iColumnName) {
...
//********************************************************
//********************************************************
const ResultSet::EntryContainerType &
ResultSet::getRow(const RowType iRowIndex) const {
...
void ResultSet::display() const {
    ...
    for (; rowItr != rowEnd; ++rowItr) {
        ...
        std::copy(
            currentRow.begin(), currentRow.end(),
            std::ostream_iterator<std::string>(std::cout, delimiter.c_str()));
        std::cout << std::endl;
    }
}
...
bool ResultSet::operator==(const ResultSet &iRhs) const {
    return rows_ == iRhs.rows_ && columns_ == iRhs.columns_ &&
           columnNames_ == iRhs.columnNames_ && matrix_ == iRhs.matrix_;
}
...
//********************************************************
//********************************************************
void ResultSet::convertToLowerCase(const std::string &iSource,
                                   std::string &iDestination) const {
    iDestination.clear();
...
void ResultSet::checkColumnSize(const ColumnType iColumns) const {
    if (iColumns != columns_) {
        throw SqliteException(
            "ResultSet::checkColumnSize : column size mismatch");
    }
}
