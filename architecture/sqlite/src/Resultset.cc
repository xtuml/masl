...
namespace SQLITE {
    ...
    void ResultSet::display() const {
        ...
        std::copy(
            columnNames_.begin(), columnNames_.end(), std::ostream_iterator<std::string>(std::cout, delimiter.c_str())
        );
        std::cout << std::endl;
        ...
        for (; rowItr != rowEnd; ++rowItr) {
            ...
            std::copy(
                currentRow.begin(), currentRow.end(), std::ostream_iterator<std::string>(std::cout, delimiter.c_str())
            );
            std::cout << std::endl;
        }
    }
    ...
    bool ResultSet::operator==(const ResultSet &iRhs) const {
        return rows_ == iRhs.rows_ && columns_ == iRhs.columns_ && columnNames_ == iRhs.columnNames_ &&
