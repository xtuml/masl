...
namespace {
    ...
    bool configureCommandLine() {
        SWA::Process::getInstance().getCommandLine().registerOption(
            SWA::NamedOption(commandLineArg, commandLineUsage, false, commandLineDesc, false)
        );
        return true;
    }
    ...
    bool isMonitorEnabled() {
        static bool enabled =
            SWA::Process::getInstance().getCommandLine().optionPresent(commandLineArg) || getenv("SQL_MONITOR") != 0;
        ;
        return enabled;
    }
...
namespace SQLITE {
    ...
    SqlPreparedStatementMonitor::SqlPreparedStatementMonitor(
        const std::string &statement, const std::string &values, const bool result
    ) {
        if (isEnabled()) {
