...
namespace {
    ...
    const char *const lookupTable[::SQLITE::PreparedStatement::MAX_PARAMETER_COUNT] = {
...
namespace SQLITE {
    ...
    class PreparedStatement::PreparedStatementImpl {
      public:
        ...
        void prepare() {
            ...
            if (sqlite3_prepare(databaseImpl, statement.c_str(), statement.size(), &preparedStatement, 0) !=
                SQLITE_OK) {
        ...
        void execute() {
            ...
            if (sqlite3_step(preparedStatement) != SQLITE_DONE) {
                sqlite3_reset(preparedStatement); // need to reset prepared statement to  get
    ...
    void PreparedStatement::bind(const int32_t position, const std::string &value) const {
        if (sqlite3_bind_text(
                impl->preparedStatement, getPositionIndex(position), value.c_str(), value.size(), SQLITE_TRANSIENT
            ) != SQLITE_OK) {
    ...
    void PreparedStatement::bind(const int32_t position, const char *const value) const {
        if (sqlite3_bind_text(
                impl->preparedStatement, getPositionIndex(position), value, strlen(value), SQLITE_TRANSIENT
            ) != SQLITE_OK) {
