...
namespace {
    ...
    bool registerAssignerIml = ::SQL::AssignerStateFactory::singleton().registerImpl(
        std::shared_ptr<::SQL::AssignerStateImpl>(new ::SQLITE::AssignerStateMapper())
    );
} // namespace
...
namespace SQLITE {
    ...
    void AssignerStateMapper::insertState(const std::string &objectKey, const int32_t currentState) {
        std::ostringstream statement;
        statement << "INSERT INTO ASSIGNER_STATES VALUES('" << objectKey << "','" << currentState << "');";
        executeStatement(statement.str());
    }
