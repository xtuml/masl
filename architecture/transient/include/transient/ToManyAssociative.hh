...
template <class Related, class Associative> class ToManyAssociative {
  private:
  ...
  public:
    ...
    SWA::Set<AssociativePtr> navigateAssociative(Predicate predicate) const {
        ...
        SWA::copy_if(
            assocBegin(), assocEnd(), result.inserter(),
            [predicate](auto &&ptr) { return predicate(ptr.deref()); });
        result.forceUnique();
        return result;
    }
...
void ToManyAssociative<Related, Associative>::link(RelatedPtr rhs,
...
template <class Related, class Associative>
void ToManyAssociative<Related, Associative>::unlink(RelatedPtr rhs,
                                                     AssociativePtr assoc) {
