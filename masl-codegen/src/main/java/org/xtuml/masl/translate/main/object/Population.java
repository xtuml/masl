...
public class Population {
    ...
    public Population(final ObjectTranslator translator) {
        ...
        bodyFile =
                domainTranslator.getLibrary().createBodyFile(Mangler.mangleFile(translator.getObjectDeclaration()) +
                                                             "Population");
        headerFile =
    ...
    Function addCreateInstance() {
        ...
        for (final AttributeDeclaration att : translator.getObjectDeclaration().getAttributes()) {
            if (att.isIdentifier() || !att.isReferential()) {
                ...
                final Variable
                        param =
                        createInstance.createParameter(type.getOptimalParameterType(), Mangler.mangleName(att));
    ...
    Function addGetAssignerState(final EnumerationType stateEnum) {
        ...
        final Function
                getter =
                theClass.createMemberFunction(assignerStateMachine, "getAssignerState", Visibility.PUBLIC);
        getter.setPure(true);
    ...
    Function addSetAssignerState(final EnumerationType stateEnum) {
        ...
        final Function
                setter =
                theClass.createMemberFunction(assignerStateMachine, "setAssignerState", Visibility.PUBLIC);
        setter.setPure(true);
    ...
    Function addFindFunction(final org.xtuml.masl.metamodel.expression.Expression predicate,
        ...
        if (predicate == null) {
            ...
            findFn =
                    theClass.createMemberFunction(retrieval,
            ...
            for (final FindParameterExpression maslParam : predicate.getFindParameters()) {
                findFn.createParameter(domainTranslator.getTypes().getType(maslParam.getType()).getOptimalParameterType(),
                                       maslParam.getName());

            }
            findFn.setComment("MASL find: " + predicate);
        }
    ...
    Function addGetUniqueId(final AttributeDeclaration att) {
        ...
        final Function
                getUniqueId =
                theClass.createMemberFunction(creators, "getUnique_" + Mangler.mangleName(att), Visibility.PUBLIC);
    ...
    Function addUseUniqueId(final AttributeDeclaration att) {
        ...
        final Function
                useUniqueId =
                theClass.createMemberFunction(creators, "useUnique_" + Mangler.mangleName(att), Visibility.PUBLIC);
    ...
    Function addCreateEventFunction(final EventDeclaration event) {
        ...
        final Function
                createFunction =
                theClass.createStaticFunction(generateEvent,
                                              "create_" +
