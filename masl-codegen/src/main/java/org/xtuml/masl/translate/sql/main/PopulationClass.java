...
public class PopulationClass implements GeneratedClass {
    ...
    public PopulationClass(final ObjectTranslator parent,
        ...
        populationClass =
                new Class(className,
                          namespace,
                          parent.getFrameworkTranslator().getLibrary().createPrivateHeader(databaseTraits.getName() +
                                                                                           Mangler.mangleFile(
                                                                                                   objectDeclaration) +
                                                                                           "Population"));
    }
        ...
        for (final AttributeDeclaration attributeDecl : objectDeclaration.getAttributes()) {
            if (attributeDecl.isIdentifier() || !attributeDecl.isReferential()) {
                if (attributeDecl.isUnique()) {
                    final Function
                            uniqueIdGetter =
                            populationClass.redefineFunction(instanceCreation,
                                                             getMainObjectTranslator().getPopulation().getGetUniqueId(
                                                                     attributeDecl),
                                                             Visibility.PUBLIC);
                    final TypeUsage type = getMainDomainTranslator().getTypes().getType(attributeDecl.getType());
                    final Variable
                            nextUniqueId =
                            populationClass.createMemberVariable(populationAttributes,
                    ...
                    final Function
                            uniqueIdUser =
                            populationClass.redefineFunction(instanceCreation,
                                                             getMainObjectTranslator().getPopulation().getUseUniqueId(
                                                                     attributeDecl),
                                                             Visibility.PUBLIC);
                    ...
                    final Expression
                            newId =
                            new BinaryExpression(Std.max(usedId, nextUniqueId.asExpression()),
                                                 BinaryOperator.PLUS,
                                                 Literal.ONE);
                    uniqueIdUser.getCode().appendStatement(new BinaryExpression(nextUniqueId.asExpression(),
                    ...
                    final Expression
                            getMaxUniqueIdFnCall =
                            getMaxUniqueIdFn.asFunctionCall(Database.mapperInstance.asExpression(), true);
                    final Expression
                            uniqueIdValue =
                            new BinaryExpression(getMaxUniqueIdFnCall, BinaryOperator.PLUS, new Literal(1));
                    final Expression
                            uniqueIdAssignment =
                            new BinaryExpression(nextUniqueId.asExpression(), BinaryOperator.ASSIGN, uniqueIdValue);
    ...
    public void translateRelationships() {
        ...
        if (forStatements.size() > 0) {
            ...
            final Variable
                    superTypeSetVar =
                    new Variable(new TypeUsage(objSetClass), "superTypeSet", new Function("findAll").asFunctionCall());
            ...
            final Variable
                    derivedvar =
                    new Variable(new TypeUsage(Database.psObjectPtrClass), "derived", downcastFnCall);
            forStatements.prependStatement(derivedvar.asStatement());

            final ForStatement
                    setItrFor =
                    new ForStatement(new VariableDefinitionStatement(setItrVar), forTest, incrementExpr, forStatements);
    ...
    public void translateFind() {
        for (final Population.FindFunction func : getMainObjectTranslator().getPopulation().getFindFunctions()) {
            ...
            if (func.predicate != null) {
                final Function
                        findFn =
                        populationClass.redefineFunction(findRoutines, func.function, Visibility.PUBLIC);
                ...
                if (identifier == null) {
                    switch (func.type) {
                        ...
                        default:
                            ...
                            final Class
                                    instanceSetType =
                                    Architecture.set(new TypeUsage(Architecture.objectPtr(new TypeUsage(
                                            getMainObjectTranslator().getMainClass()))));
                            final TypeUsage instanceSetTypeUsage = new TypeUsage(instanceSetType);
                            final Expression
                                    initialValue =
                                    findFn.asFunctionCall(Database.mapperInstance.asExpression(), true, params);
                            final Variable
                                    instanceSet =
                                    new Variable(instanceSetTypeUsage, "instanceSet", new Expression[]{initialValue});
    ...
    private void createIdentifierFind(final IdentifierDeclaration identifier,
        ...
        final Expression
                fullCachingCondition =
                new Function("fullCachingEnabled").asFunctionCall(Database.mapperInstance.asExpression(), true);
        ...
        final Expression
                mapperFindFnCall =
                new Function(functionImpl.getName()).asFunctionCall(Database.mapperInstance.asExpression(),
                                                                    true,
    ...
    private Function addIdentifierLookup(final IdentifierDeclaration identifier) {
        ...
        if (identifierFindFn == null) {
            ...
            identifierFindFn =
                    populationClass.createMemberFunction(identiferFindRoutines,
                                                         "find_" + lookupName,
                                                         Visibility.PRIVATE);
            ...
            for (final AttributeDeclaration attDec : identifier.getAttributes()) {
                final Variable
                        param =
                        identifierFindFn.createParameter(getMainDomainTranslator().getTypes().getType(attDec.getType()).getOptimalParameterType(),
            ...
            final Class
                    lookupContainerType =
                    Boost.unordered_map(new TypeUsage(objectKeyType), new TypeUsage(objectPtr));
            final Variable
                    identifierFindAtt =
                    populationClass.createMemberVariable(populationAttributes,
                                                         lookupName + "_cache",
            ...
            final Expression
                    find =
                    new Function("find").asFunctionCall(identifierFindAtt.asExpression(),
                                                        false,
                                                        objectKeyType.callConstructor(params));
            final Variable
                    found =
                    new Variable(new TypeUsage(lookupContainerType.referenceNestedType("const_iterator")),
                                 "result",
                                 find);
            final Statement failReturn = new ReturnStatement(objectPtr.callConstructor());
            final Statement
                    foundReturn =
                    new ReturnStatement(new Variable("second").asMemberReference(found.asExpression(), true));
            final Statement
                    check =
            ...
            if (identifierFindFns.size() == 1) {
                ...
                final Expression
                        findAllFnCall =
                        new Function("findAll").asFunctionCall(Database.mapperInstance.asExpression(), true, findArgs);
                ...
                final Variable
                        objItrVar =
                        new Variable(new TypeUsage(iterType),
                                     "objItr",
                ...
                final Class
                        impObjPtrType =
                        Architecture.objectPtr(new TypeUsage(objectTranslator.getClass(ImplementationClass.KEY_NAME)));
                ...
                final Variable
                        implValueDef =
                        new Variable(new TypeUsage(impObjPtrType), "implObj", new Expression[]{downcastFnCall});
                final Expression keyExpr = objectKeyGetter.asFunctionCall(implValueDef.asExpression(), true);
                final Expression
                        valueTypeExpr =
                        lookupContainerType.referenceNestedType("value_type").callConstructor(keyExpr, ValueExpr);
                final Expression
                        insertFnCall =
                        new Function("insert").asFunctionCall(identifierFindAtt.asExpression(), false, valueTypeExpr);

                final BinaryExpression
                        forTestCondition =
                        new BinaryExpression(objItrVar.asExpression(),
                                             BinaryOperator.NOT_EQUAL,
                                             objEndVar.asExpression());
                final UnaryExpression
                        itrIncrement =
                        new UnaryExpression(UnaryOperator.PREINCREMENT, objItrVar.asExpression());
                ...
                final ForStatement
                        forLoop =
                        new ForStatement(objItrVar.asStatement(), forTestCondition, itrIncrement, cacheInitLoopBlock);
    ...
    private void defineNormalLinkerFns(final NormalRelationshipDeclaration normalRelationshipDecl) {
        ...
        final Class
                relationshipMapperType =
                objectTranslator.getFrameworkTranslator().getRelationshipTranslator(normalRelationshipDecl).getRelationshipMapperClass();
        final Class
                relationshipMapperSqlType =
                objectTranslator.getFrameworkTranslator().getRelationshipTranslator(normalRelationshipDecl).getRelationshipMapperSqlClass();
        ...
        final Function
                objectDeletedFn =
                getDeleteObjectFunction(normalRelationshipDecl.getLeftObject(),
                                        normalRelationshipDecl.getRightObject(),
                                        null);

        final Variable
                relationshipVar =
                addRelationshipDataMember(relationshipMapperType,
                                          relationshipMapperSqlType,
        ...
        org.xtuml.masl.translate.main.object.RelationshipTranslator
                relTrans =
                getMainObjectTranslator().getRelationshipTranslator(leftToRightRelSpec);
        if (relTrans != null) {
    ...
    private void defineAssociativeLinkerFns(final AssociativeRelationshipDeclaration assocRelationshipDecl) {
        ...
        if (relationshipVar == null) {
            ...
            final Class
                    relationshipMapperType =
                    objectTranslator.getFrameworkTranslator().getRelationshipTranslator(assocRelationshipDecl).getRelationshipMapperClass();
            final Class
                    relationshipMapperSqlType =
                    objectTranslator.getFrameworkTranslator().getRelationshipTranslator(assocRelationshipDecl).getRelationshipMapperSqlClass();
    ...
    private String getSubtypeRelationshipName(final SubtypeRelationshipDeclaration subtypeRelationshipDecl,
        ...
        return subtypeRelationshipDecl.getName() +
               "_" +
    ...
    private void defineSubtypeRelationshipInit(final SubtypeRelationshipDeclaration subtypeRelationshipDecl,
        ...
        if (objectDeclaration == superTypeObjDecl) {
            ...
            final Expression
                    setlinkedFnCall =
                    new Function("setLinked" + subtypeRelationshipDecl.getName()).asFunctionCall(derived.asExpression(),
                                                                                                 true);
    ...
    private void defineSubtypeLinkerFns(final SubtypeRelationshipDeclaration subtypeRelationshipDecl) {
        ...
        if (objectDeclaration == superTypeObjDecl) {
            ...
            for (final ObjectDeclaration derivedObjDecl : subtypeRelationshipDecl.getSubtypes()) {
                final Variable
                        relationshipVar =
                        getSubTypeRelationship(subtypeRelationshipDecl,
                                               derivedObjDecl,
    ...
    private Variable getSubTypeRelationship(final SubtypeRelationshipDeclaration subtypeRelationshipDecl,
        ...
        Variable
                relationshipVar =
                relationshipDataMemberList.get(getSubtypeRelationshipName(subtypeRelationshipDecl, derivedObjDecl));
        if (relationshipVar == null) {
            // Each relationship has an associated relationship mapper class and
            // relationship mapper sql class. Need to get
            // the class representations for these so they can be used to form and
            ...
            final Class
                    relationshipMapperType =
                    objectTranslator.getFrameworkTranslator().getRelationshipTranslator(subtypeRelationshipDecl).getSuperToSubtypeRelationshipMapperClass(
                            superTypeObjDecl,
                            derivedObjDecl);
            final Class
                    relationshipMapperSqlType =
                    objectTranslator.getFrameworkTranslator().getRelationshipTranslator(subtypeRelationshipDecl).getSuperToSubtypeRelationshipMapperSqlClass(
                            superTypeObjDecl,
                            derivedObjDecl);
            ...
            final Function
                    objectDeletedFn =
                    (direction == RelationshipDirection.BaseToDerived ?
                     new Function("objectDeletedLhs") :
    ...
    private void implementNormalLinkerFns(final NormalRelationshipDeclaration normRelDecl,
        ...
        final ObjectDeclaration
                paramLhsObjDecl =
                (direction == RelationshipDirection.LeftToRight ? relLhsObject : relRhsObject);
        final ObjectDeclaration
                paramRhsObjDecl =
                (direction == RelationshipDirection.LeftToRight ? relRhsObject : relLhsObject);
        final ObjectTranslator
                paramLhsType =
                objectTranslator.getFrameworkTranslator().getObjectTranslator(paramLhsObjDecl);
        final ObjectTranslator
                paramRhsType =
                objectTranslator.getFrameworkTranslator().getObjectTranslator(paramRhsObjDecl);
        ...
        final Function
                populationNavigateFn =
                defineNavigateFn(mainNavigateFn, relVar, direction, paramLhsType, paramRhsType);
    ...
    private void implementAssociativeNavFns(final ObjectDeclaration ObjectOnLhsOfNav,
        ...
        final ObjectTranslator
                paramLhsType =
                objectTranslator.getFrameworkTranslator().getObjectTranslator(ObjectOnLhsOfNav);
        final ObjectTranslator
                paramRhsType =
                objectTranslator.getFrameworkTranslator().getObjectTranslator(ObjectOnRhsOfNav);

        final Function
                populationNavFn =
                defineNavigateFn(navigateFunction, relationshipVar, direction, paramLhsType, paramRhsType);
        bodyFile.addFunctionDefinition(populationNavFn);

        final Function
                populationCountFn =
                defineCountFn(countFunction, relationshipVar, direction, paramLhsType, paramRhsType);
        bodyFile.addFunctionDefinition(populationCountFn);
    }
    ...
    private void implementAssociativeCorrelateFn(final ObjectDeclaration ObjectOnLhsOfNav,
        ...
        final ObjectTranslator
                paramAssType =
                objectTranslator.getFrameworkTranslator().getObjectTranslator(ObjectOnAssOfNav);

        final Class paramAssClass = getMainDomainTranslator().getObjectTranslator(ObjectOnAssOfNav).getMainClass();

        final Function
                populationCorrelateFn =
                populationClass.createMemberFunction(relationshipLinkers,
                                                     correlateFunction.getName(),
                                                     Visibility.PUBLIC);
        final Variable
                lhsParamVar =
                populationCorrelateFn.createParameter(new TypeUsage(Architecture.objectPtr(new TypeUsage(paramLhsType.getClass(
                        "ImplementationClass"))), TypeUsage.ConstReference), "lhs");
        final Variable
                rhsParamVar =
                populationCorrelateFn.createParameter(new TypeUsage(Architecture.objectPtr(new TypeUsage(paramRhsType.getClass(
                        "ImplementationClass"))), TypeUsage.ConstReference), "rhs");
        ...
        final Expression
                getPopulationFnCall =
                paramAssType.getClass("PopulationClass").callStaticFunction("getPopulation");
        final Function findObjectFn = new Function("findObject");
        final Expression
                findObjectFnCall =
                findObjectFn.asFunctionCall(getPopulationFnCall, false, correlatedVar.asExpression());
    ...
    private void implementAssociativeLinkerFns(final AssociativeRelationshipDeclaration assocRelationshipDecl,
        ...
        final ObjectTranslator
                paramAssoType =
                objectTranslator.getFrameworkTranslator().getObjectTranslator(paramAssoObjDecl);

        // Define the link function
        final Function
                populationLinkFn =
                populationClass.createMemberFunction(relationshipLinkers, linkFunction.getName(), Visibility.PUBLIC);
        populationLinkFn.createParameter(new TypeUsage(Architecture.objectPtr(new TypeUsage(paramLhsType.getClass(
                "ImplementationClass"))), TypeUsage.ConstReference), "lhs");
        populationLinkFn.createParameter(new TypeUsage(Architecture.objectPtr(new TypeUsage(paramRhsType.getClass(
                "ImplementationClass"))), TypeUsage.ConstReference), "rhs");
        populationLinkFn.createParameter(new TypeUsage(Architecture.objectPtr(new TypeUsage(paramAssoType.getClass(
                "ImplementationClass"))), TypeUsage.ConstReference), "asso");
        ...
        final Expression
                mapperLinkFnCall =
                mapperLinkFn.asFunctionCall(relationshipVar.asExpression(),
                                            false,
                                            new Variable("lhs").asExpression(),
                                            new Variable("rhs").asExpression(),
                                            new Variable("asso").asExpression());
        populationLinkFn.getCode().appendExpression(mapperLinkFnCall);

        // Define the unlink function
        final Function
                populationUnlinkFn =
                populationClass.createMemberFunction(relationshipLinkers, unlinkFunction.getName(), Visibility.PUBLIC);
        populationUnlinkFn.createParameter(new TypeUsage(Architecture.objectPtr(new TypeUsage(paramLhsType.getClass(
                "ImplementationClass"))), TypeUsage.ConstReference), "lhs");
        populationUnlinkFn.createParameter(new TypeUsage(Architecture.objectPtr(new TypeUsage(paramRhsType.getClass(
                "ImplementationClass"))), TypeUsage.ConstReference), "rhs");
        populationUnlinkFn.createParameter(new TypeUsage(Architecture.objectPtr(new TypeUsage(paramAssoType.getClass(
                "ImplementationClass"))), TypeUsage.ConstReference), "asso");
        ...
        final Expression
                mapperUnlinkFnCall =
                mapperUnLinkFn.asFunctionCall(relationshipVar.asExpression(),
                                              false,
                                              new Variable("lhs").asExpression(),
                                              new Variable("rhs").asExpression(),
                                              new Variable("asso").asExpression());
    ...
    private Function defineLinkerFn(final String functionName,
        ...
        final Function
                populationLinkFn =
                populationClass.createMemberFunction(relationshipLinkers, mainLinkerFn.getName(), Visibility.PUBLIC);
        populationLinkFn.createParameter(new TypeUsage(Architecture.objectPtr(new TypeUsage(paramLhsType.getClass(
                "ImplementationClass"))), TypeUsage.ConstReference), "lhs");
        populationLinkFn.createParameter(new TypeUsage(Architecture.objectPtr(new TypeUsage(paramRhsType.getClass(
                "ImplementationClass"))), TypeUsage.ConstReference), "rhs");
    ...
    private Function defineNavigateFn(final Function mainNavigateFn,
        ...
        final Function
                populationNavigateFn =
                populationClass.createMemberFunction(relationshipNavs, mainNavigateFn.getName(), Visibility.PUBLIC);
        populationNavigateFn.setReturnType(mainNavigateFn.getReturnType());
        populationNavigateFn.createParameter(new TypeUsage(Architecture.objectPtr(new TypeUsage(paramLhsType.getClass(
                "ImplementationClass"))), TypeUsage.ConstReference), "lhs");
        ...
        final Expression
                ...
                mapperNavigateFn.asFunctionCall(relVar.asExpression(), false, new Variable("lhs").asExpression());

        final TypeUsage navigatedType = getNavigatedType(relVar, direction);
        final Variable navigatedVar = new Variable(navigatedType, "navigated", new Expression[]{mapperNavigateFnCall});
        ...
        final Expression
                getPopulationFnCall =
                paramRhsType.getClass("PopulationClass").callStaticFunction("getPopulation");
        final Function findObjectFn = new Function("findObject");
        final Expression
                findObjectFnCall =
                findObjectFn.asFunctionCall(getPopulationFnCall, false, navigatedVar.asExpression());
    ...
    private Function defineCountFn(final Function countFn,
        ...
        final Function
                populationCountFn =
                populationClass.createMemberFunction(relationshipCounts, countFn.getName(), Visibility.PUBLIC);
        populationCountFn.setReturnType(countFn.getReturnType());
        populationCountFn.createParameter(new TypeUsage(Architecture.objectPtr(new TypeUsage(paramLhsType.getClass(
                "ImplementationClass"))), TypeUsage.ConstReference), "lhs");
        ...
        final Expression
                countImplFnCall =
                countImplFn.asFunctionCall(relVar.asExpression(),
                                           false,
                                           populationCountFn.getParameters().get(0).asExpression());
    ...
    private Function addConstructor() {
        ...
        final List<Expression> constructorArgs = List.of(); // no
        // arg
        // constructor
    ...
    private Function addInitialiseMethod() {
        final Function
                initialiseMethod =
                populationClass.createMemberFunction(intialisation, "initialise", Visibility.PUBLIC);
        bodyFile.addFunctionDefinition(initialiseMethod);
        final Function initFn = new Function("initialise");
        initialiseMethod.getCode().appendStatement(new ExpressionStatement(initFn.asFunctionCall(Database.mapperInstance.asExpression(),
                                                                                                 true)));
    ...
    private void addCreateMethodBody() {
        ...
        if (identifierFindAtts.size() == 0) {
            ...
            final Variable
                    objVar =
                    new Variable(new TypeUsage(objPtr),
                                 "obj",
            ...
            final Variable
                    objImplVar =
                    new Variable(new TypeUsage(objImplPtr), "objImpl", new Expression[]{downcastFnCall});
            ...
            for (final Map.Entry<IdentifierDeclaration, Variable> identifierFind : identifierFindAtts.entrySet()) {
                ...
                final Expression
                        valueTypeExpr =
                        lookupContainerType.referenceNestedType("value_type").callConstructor(keyExpr,
                                                                                              objImplVar.asExpression());

                final Expression
                        insertFnCall =
                        new Function("insert").asFunctionCall(identifierFind.getValue().asExpression(),
                                                              false,
                                                              valueTypeExpr);
                final Expression
                        clearFnCall =
                        new Function("clear").asFunctionCall(identifierFind.getValue().asExpression(), false);
    ...
    private void addDeleteMethodBody() {
        if (identifierFindAtts.size() > 0) {
            ...
            for (final Map.Entry<IdentifierDeclaration, Variable> identifierEntry : identifierFindAtts.entrySet()) {
                ...
                final Expression
                        eraseFnCall =
                        new Function("erase").asFunctionCall(identifierEntry.getValue().asExpression(), false, keyExpr);
        ...
        final Expression
                deleteInstanceFnCall =
                new Function("deleteInstance").asFunctionCall(Database.mapperInstance.asExpression(),
                                                              true,
                                                              deleterFn.getParameters().get(0).asExpression());
    ...
    private Function addGetPopulationMethod() {
        final Function
                getPopulationMethod =
                populationClass.createStaticFunction(singletonRegistration, "getPopulation", Visibility.PUBLIC);
    ...
    private Function addSingletonRegistration(final Function populationMethod) {
        ...
        final Variable
                registered =
                populationClass.createStaticVariable(populationAttributes,
                                                     "registered",
    ...
    private void addFindObjectMethods() {
        ...
        final Function
                findObject =
                populationClass.createMemberFunction(findObjectRoutines, "findObject", Visibility.PUBLIC);
        final Variable
                objVar =
                findObject.createParameter(new TypeUsage(Architecture.ID_TYPE, TypeUsage.ConstReference), "obj");
        findObject.setReturnType(new TypeUsage(objPtrClass));
        findObject.getCode().appendStatement(new ReturnStatement(new Function("find").asFunctionCall(new Variable(
                "mapper").asExpression(), true, objVar.asExpression())));
        ...
        final Variable
                objSetVar =
                findSetObject.createParameter(new TypeUsage(setType, TypeUsage.ConstReference), "obj");
        ...
        final Class
                objectSetType =
                populationClass.referenceNestedType("MapperType").referenceNestedType("PsObjectPtrSet");
        final Variable
                objectSetVar =
                new Variable(new TypeUsage(objectSetType),
                             "objectSet",
        ...
        final Class
                returnSetType =
                Architecture.set(new TypeUsage(Architecture.objectPtr(new TypeUsage(getMainObjectTranslator().getMainClass()))));
        ...
        findSetObject.getCode().appendStatement(new ReturnStatement(returnSetType.callConstructor(beginFnCall,
                                                                                                  endFnCall,
                                                                                                  Literal.TRUE)));
    ...
    private Variable addRelationshipDataMember(final Class implRelationshipMapperType,
        ...
        final Variable
                ...
                populationClass.createMemberVariable(populationAttributes,
                                                     ...
                                                     new TypeUsage(implRelationshipMapperType, TypeUsage.Reference),
                                                     Visibility.PRIVATE);
        ...
        // Create cpp line:
        // relationshipR1.initialise(std::shared_ptr<R1Mapper::RelSqlGeneratorType>(new
        // RelationshipR1SqlGenerator));

        final Function initialiseFn = new Function("initialise");
        final Class
                relSqlGenClass =
                Std.shared_ptr(new TypeUsage(implRelationshipMapperType.referenceNestedType(
                        "RelSqlGeneratorType")));
        final Expression
                relSqlGenConstruct =
                relSqlGenClass.callConstructor(new NewExpression(new TypeUsage(implRelationshipMapperSql),
                                                                 new ArrayList<Expression>()));
        final Expression
                initialiseFnCall =
                initialiseFn.asFunctionCall(relationshipVar.asExpression(), false, relSqlGenConstruct);
        ...
        final Expression
                objectDeletedFnCall =
                objectDeletedFn.asFunctionCall(relationshipVar.asExpression(), false, downCastFnCall);
    ...
    private void addCurrentState() {
        if (objectDeclaration.hasAssignerState()) {
            ...
            final Function
                    getter =
                    populationClass.redefineFunction(assignerStateRoutines,
            ...
            final String
                    assignerStateKey =
                    objectDeclaration.getDomain().getName() + "::" + objectDeclaration.getName();
            ...
            // newState);
            final Expression
                    setAssignerStateFnCall =
                    objectTranslator.getDatabase().setAssignerStateFnCall(assignerStateKey,
                                                                          setter.getParameters().get(0).asExpression());
            setter.getCode().appendExpression(setAssignerStateFnCall);

            // Generate body for getter
            final Expression
                    getAssignerStateFnCall =
                    objectTranslator.getDatabase().getAssignerStateFnCall(currentStateType, assignerStateKey);
            ...
            final BinaryExpression
                    assignerSetTestCondition =
                    new BinaryExpression(isAssignerSetFnCall, BinaryOperator.EQUAL, new Literal("false"));
            final Expression
                    setAssignerToDefaultStateFnCall =
                    objectTranslator.getDatabase().setAssignerStateFnCall(assignerStateKey, startingStateExpr);
            final IfStatement
                    testAssignerIf =
                    new IfStatement(assignerSetTestCondition, setAssignerToDefaultStateFnCall.asStatement());
    ...
    private Function getRelationshipFunction(final String function, final RelationshipDirection direction) {
        ...
        } else if (direction == RelationshipDirection.DerivedToBase ||
                   direction == RelationshipDirection.BaseToDerived) {
            relationshipFunction = new Function(function + "FromRhsToLhs");
    ...
    private TypeUsage getNavigatedType(final Variable relationshipVar, final RelationshipDirection direction) {
        ...
        if (direction == RelationshipDirection.LeftToRight || direction == RelationshipDirection.RightToLeft) {
            navigatedType =
                    new TypeUsage(relationshipClass.referenceNestedType("NavigatedRhsType"), TypeUsage.ConstReference);
            if (direction == RelationshipDirection.RightToLeft) {
                navigatedType =
                        new TypeUsage(relationshipClass.referenceNestedType("NavigatedLhsType"),
                                      TypeUsage.ConstReference);
            }
        } else if (direction == RelationshipDirection.AssocToRight || direction == RelationshipDirection.AssocToLeft) {
            navigatedType =
                    new TypeUsage(relationshipClass.referenceNestedType("NavigatedAssType"), TypeUsage.ConstReference);
            if (direction == RelationshipDirection.AssocToRight) {
                navigatedType =
                        new TypeUsage(relationshipClass.referenceNestedType("NavigatedAssType"),
                                      TypeUsage.ConstReference);
            }
        } else if (direction == RelationshipDirection.DerivedToBase ||
                   direction == RelationshipDirection.BaseToDerived) {
