...
final class SqliteTraits implements DatabaseTraits {
    ...
    @Override
    public TernaryRelationshipToTableTranslator createTernaryRelationshipToTableTranslator(final AssociativeRelationshipDeclaration relationshipDeclaration) {
        return new SqliteTernaryRelationshipToTableTranslator(relationshipDeclaration);
    }
    ...
    private Function createEventEncoder(final Namespace namespace,
        ...
        if (event.getParameters().size() > 0) {
            final TypeUsage eventPtr = new TypeUsage(Std.shared_ptr(new TypeUsage(eventTrans.getEventClass())));

            final Variable
                    thisEventVar =
                    new Variable(eventPtr,
                                 "thisEvent",
                                 Std.dynamic_pointer_cast(new TypeUsage(eventTrans.getEventClass())).asFunctionCall(eventVar.asExpression()));
